...
CoreInternalOutcome BatchPublishMessage::Deserialize(const Value &value)
{
    string requestId = "";
...
void BatchPublishMessage::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_payloadHasBeenSet)
    {
        ...
        string key = "Payload";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_payload.c_str(), allocator).Move(), allocator);
    }
