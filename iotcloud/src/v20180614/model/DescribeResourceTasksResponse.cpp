...
CoreInternalOutcome DescribeResourceTasksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskInfos") && !rsp["TaskInfos"].IsNull())
    {
        if (!rsp["TaskInfos"].IsArray())
            return CoreInternalOutcome(Error("response `TaskInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TaskInfos"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            FirmwareTaskInfo item;
...
string DescribeResourceTasksResponse::ToJsonString() const
{
...
vector<FirmwareTaskInfo> DescribeResourceTasksResponse::GetTaskInfos() const
{
    return m_taskInfos;
}

bool DescribeResourceTasksResponse::TaskInfosHasBeenSet() const
{
    return m_taskInfosHasBeenSet;
}

uint64_t DescribeResourceTasksResponse::GetTotal() const
{
    return m_total;
}

bool DescribeResourceTasksResponse::TotalHasBeenSet() const
{
