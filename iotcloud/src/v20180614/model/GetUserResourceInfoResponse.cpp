...
CoreInternalOutcome GetUserResourceInfoResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["UsedSize"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `UsedSize` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_usedSize = rsp["UsedSize"].GetUint64();
...
string GetUserResourceInfoResponse::ToJsonString() const
{
...
uint64_t GetUserResourceInfoResponse::GetUsedSize() const
{
    return m_usedSize;
}

bool GetUserResourceInfoResponse::UsedSizeHasBeenSet() const
{
    return m_usedSizeHasBeenSet;
}

uint64_t GetUserResourceInfoResponse::GetLimit() const
{
    return m_limit;
}

bool GetUserResourceInfoResponse::LimitHasBeenSet() const
{
