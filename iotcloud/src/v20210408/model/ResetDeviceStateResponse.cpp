...
CoreInternalOutcome ResetDeviceStateResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ResetDeviceResults") && !rsp["ResetDeviceResults"].IsNull())
    {
        if (!rsp["ResetDeviceResults"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ResetDeviceResults` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ResetDeviceResults"];
...
string ResetDeviceStateResponse::ToJsonString() const
{
    ...
    if (m_resetDeviceResultsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ResetDeviceResults";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_resetDeviceResults.begin(); itr != m_resetDeviceResults.end(); ++itr, ++i)
        {
