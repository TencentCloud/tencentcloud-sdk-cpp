...
MarketClient::SyncUserAndOrderInfoOutcome MarketClient::SyncUserAndOrderInfo(const SyncUserAndOrderInfoRequest &request)
{
    auto outcome = MakeRequest(request, "SyncUserAndOrderInfo");
    if (outcome.IsSuccess())
    {
        auto r = outcome.GetResult();
        string payload = string(r.Body(), r.BodySize());
        SyncUserAndOrderInfoResponse rsp = SyncUserAndOrderInfoResponse();
        auto o = rsp.Deserialize(payload);
        if (o.IsSuccess())
            return SyncUserAndOrderInfoOutcome(rsp);
        else
            return SyncUserAndOrderInfoOutcome(o.GetError());
    }
    else
    {
        return SyncUserAndOrderInfoOutcome(outcome.GetError());
    }
}

void MarketClient::SyncUserAndOrderInfoAsync(const SyncUserAndOrderInfoRequest& request, const SyncUserAndOrderInfoAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context)
{
    auto fn = [this, request, handler, context]()
    {
        handler(this, request, this->SyncUserAndOrderInfo(request), context);
    };

    Executor::GetInstance()->Submit(new Runnable(fn));
}

MarketClient::SyncUserAndOrderInfoOutcomeCallable MarketClient::SyncUserAndOrderInfoCallable(const SyncUserAndOrderInfoRequest &request)
{
    auto task = std::make_shared<std::packaged_task<SyncUserAndOrderInfoOutcome()>>(
        [this, request]()
        {
            return this->SyncUserAndOrderInfo(request);
        }
    );
