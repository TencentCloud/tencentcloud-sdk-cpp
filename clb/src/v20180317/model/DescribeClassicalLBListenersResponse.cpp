...
CoreInternalOutcome DescribeClassicalLBListenersResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Listeners") && !rsp["Listeners"].IsNull())
    {
        if (!rsp["Listeners"].IsArray())
            return CoreInternalOutcome(Error("response `Listeners` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Listeners"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ClassicalListener item;
...
string DescribeClassicalLBListenersResponse::ToJsonString() const
{
...
vector<ClassicalListener> DescribeClassicalLBListenersResponse::GetListeners() const
{
    return m_listeners;
}

bool DescribeClassicalLBListenersResponse::ListenersHasBeenSet() const
{
