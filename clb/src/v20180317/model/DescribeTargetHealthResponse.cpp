...
CoreInternalOutcome DescribeTargetHealthResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LoadBalancers") && !rsp["LoadBalancers"].IsNull())
    {
        if (!rsp["LoadBalancers"].IsArray())
            return CoreInternalOutcome(Error("response `LoadBalancers` is not array type"));

        const rapidjson::Value &tmpValue = rsp["LoadBalancers"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            LoadBalancerHealth item;
...
string DescribeTargetHealthResponse::ToJsonString() const
{
...
vector<LoadBalancerHealth> DescribeTargetHealthResponse::GetLoadBalancers() const
{
    return m_loadBalancers;
}

bool DescribeTargetHealthResponse::LoadBalancersHasBeenSet() const
{
