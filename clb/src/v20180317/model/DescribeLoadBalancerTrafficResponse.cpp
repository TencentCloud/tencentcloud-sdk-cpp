...
CoreInternalOutcome DescribeLoadBalancerTrafficResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LoadBalancerTraffic") && !rsp["LoadBalancerTraffic"].IsNull())
    {
        if (!rsp["LoadBalancerTraffic"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LoadBalancerTraffic` is not array type"));
...
string DescribeLoadBalancerTrafficResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_loadBalancerTrafficHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "LoadBalancerTraffic";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_loadBalancerTraffic.begin(); itr != m_loadBalancerTraffic.end(); ++itr, ++i)
        {
