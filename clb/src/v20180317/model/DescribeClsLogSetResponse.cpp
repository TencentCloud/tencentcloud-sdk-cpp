...
CoreInternalOutcome DescribeClsLogSetResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("HealthLogsetId") && !rsp["HealthLogsetId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `HealthLogsetId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeClsLogSetResponse::ToJsonString() const
{
...
string DescribeClsLogSetResponse::GetLogsetId() const
{
    return m_logsetId;
}
