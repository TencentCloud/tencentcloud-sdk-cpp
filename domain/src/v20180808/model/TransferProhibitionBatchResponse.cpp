...
CoreInternalOutcome TransferProhibitionBatchResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LogId") && !rsp["LogId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `LogId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string TransferProhibitionBatchResponse::ToJsonString() const
{
...
uint64_t TransferProhibitionBatchResponse::GetLogId() const
{
    return m_logId;
}

bool TransferProhibitionBatchResponse::LogIdHasBeenSet() const
{
