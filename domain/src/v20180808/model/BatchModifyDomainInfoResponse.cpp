...
CoreInternalOutcome BatchModifyDomainInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LogId") && !rsp["LogId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LogId` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string BatchModifyDomainInfoResponse::ToJsonString() const
{
...
int64_t BatchModifyDomainInfoResponse::GetLogId() const
{
    return m_logId;
}

bool BatchModifyDomainInfoResponse::LogIdHasBeenSet() const
{
