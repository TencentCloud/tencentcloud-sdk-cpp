...
CoreInternalOutcome UploadImageResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AccessUrl") && !rsp["AccessUrl"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `AccessUrl` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string UploadImageResponse::ToJsonString() const
{
...
string UploadImageResponse::GetAccessUrl() const
{
    return m_accessUrl;
}

bool UploadImageResponse::AccessUrlHasBeenSet() const
{
