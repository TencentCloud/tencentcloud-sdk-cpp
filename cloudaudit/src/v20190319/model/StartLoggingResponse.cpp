...
CoreInternalOutcome StartLoggingResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["IsSuccess"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `IsSuccess` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_isSuccess = rsp["IsSuccess"].GetInt64();
...
string StartLoggingResponse::ToJsonString() const
{
...
int64_t StartLoggingResponse::GetIsSuccess() const
{
    return m_isSuccess;
}

bool StartLoggingResponse::IsSuccessHasBeenSet() const
{
