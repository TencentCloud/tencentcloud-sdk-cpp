...
CoreInternalOutcome DescribeEventsResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["NextToken"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `NextToken` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_nextToken = rsp["NextToken"].GetUint64();
...
string DescribeEventsResponse::ToJsonString() const
{
    ...
    if (m_nextTokenHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_nextToken, allocator);
    }
...
bool DescribeEventsResponse::GetListOver() const
{
    return m_listOver;
}

bool DescribeEventsResponse::ListOverHasBeenSet() const
{
    return m_listOverHasBeenSet;
}

uint64_t DescribeEventsResponse::GetNextToken() const
{
    return m_nextToken;
}

bool DescribeEventsResponse::NextTokenHasBeenSet() const
{
    return m_nextTokenHasBeenSet;
}

vector<Event> DescribeEventsResponse::GetEvents() const
{
    return m_events;
}
