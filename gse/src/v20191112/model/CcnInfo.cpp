...
CoreInternalOutcome CcnInfo::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("AccountId") && !value["AccountId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `CcnInfo.AccountId` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("CcnId") && !value["CcnId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `CcnInfo.CcnId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void CcnInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
