...
CoreInternalOutcome CopyFleetResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FleetAttributes") && !rsp["FleetAttributes"].IsNull())
    {
        if (!rsp["FleetAttributes"].IsArray())
            return CoreInternalOutcome(Error("response `FleetAttributes` is not array type"));
...
string CopyFleetResponse::ToJsonString() const
{
...
vector<FleetAttributes> CopyFleetResponse::GetFleetAttributes() const
{
    return m_fleetAttributes;
}

bool CopyFleetResponse::FleetAttributesHasBeenSet() const
{
    return m_fleetAttributesHasBeenSet;
}

int64_t CopyFleetResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool CopyFleetResponse::TotalCountHasBeenSet() const
{
