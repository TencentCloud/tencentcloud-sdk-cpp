...
CoreInternalOutcome GetUploadFederationTokenResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AssetCredentials") && !rsp["AssetCredentials"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AssetCredentials` is not object type").SetRequestId(requestId));
        }
...
string GetUploadFederationTokenResponse::ToJsonString() const
{
    ...
    if (m_assetCredentialsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AssetCredentials";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_assetCredentials.ToJsonObject(value[key.c_str()], allocator);
    }
