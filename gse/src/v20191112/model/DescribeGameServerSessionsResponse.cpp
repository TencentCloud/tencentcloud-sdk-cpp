...
CoreInternalOutcome DescribeGameServerSessionsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("GameServerSessions") && !rsp["GameServerSessions"].IsNull())
    {
        if (!rsp["GameServerSessions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `GameServerSessions` is not array type"));

        const rapidjson::Value &tmpValue = rsp["GameServerSessions"];
...
string DescribeGameServerSessionsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_gameServerSessionsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "GameServerSessions";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_gameServerSessions.begin(); itr != m_gameServerSessions.end(); ++itr, ++i)
        {
...
vector<GameServerSession> DescribeGameServerSessionsResponse::GetGameServerSessions() const
{
    return m_gameServerSessions;
}

bool DescribeGameServerSessionsResponse::GameServerSessionsHasBeenSet() const
{
    return m_gameServerSessionsHasBeenSet;
}

string DescribeGameServerSessionsResponse::GetNextToken() const
{
    return m_nextToken;
}

bool DescribeGameServerSessionsResponse::NextTokenHasBeenSet() const
{
