...
CoreInternalOutcome GetGameServerInstanceLogUrlResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PresignedUrls") && !rsp["PresignedUrls"].IsNull())
    {
        if (!rsp["PresignedUrls"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PresignedUrls` is not array type"));
    ...
    if (rsp.HasMember("HasNext") && !rsp["HasNext"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `HasNext` IsBool=false incorrectly").SetRequestId(requestId));
        }
...
string GetGameServerInstanceLogUrlResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_presignedUrlsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "PresignedUrls";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_presignedUrls.begin(); itr != m_presignedUrls.end(); ++itr)
        {
    ...
    if (m_hasNextHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "HasNext";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_hasNext, allocator);
    }
