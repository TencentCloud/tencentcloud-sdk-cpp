...
CoreInternalOutcome DescribeTimerScalingPoliciesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TimerScalingPolicies") && !rsp["TimerScalingPolicies"].IsNull())
    {
        if (!rsp["TimerScalingPolicies"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TimerScalingPolicies` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TimerScalingPolicies"];
...
string DescribeTimerScalingPoliciesResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_timerScalingPoliciesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TimerScalingPolicies";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_timerScalingPolicies.begin(); itr != m_timerScalingPolicies.end(); ++itr, ++i)
        {
