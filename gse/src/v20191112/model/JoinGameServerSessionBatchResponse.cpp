...
CoreInternalOutcome JoinGameServerSessionBatchResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PlayerSessions") && !rsp["PlayerSessions"].IsNull())
    {
        if (!rsp["PlayerSessions"].IsArray())
            return CoreInternalOutcome(Error("response `PlayerSessions` is not array type"));
...
string JoinGameServerSessionBatchResponse::ToJsonString() const
{
...
vector<PlayerSession> JoinGameServerSessionBatchResponse::GetPlayerSessions() const
{
    return m_playerSessions;
}

bool JoinGameServerSessionBatchResponse::PlayerSessionsHasBeenSet() const
{
