...
CoreInternalOutcome DescribeInstanceTypesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InstanceTypeList") && !rsp["InstanceTypeList"].IsNull())
    {
        if (!rsp["InstanceTypeList"].IsArray())
            return CoreInternalOutcome(Error("response `InstanceTypeList` is not array type"));

        const Value &tmpValue = rsp["InstanceTypeList"];
...
vector<InstanceTypeInfo> DescribeInstanceTypesResponse::GetInstanceTypeList() const
{
    return m_instanceTypeList;
}

bool DescribeInstanceTypesResponse::InstanceTypeListHasBeenSet() const
{
