...
CoreInternalOutcome ProcedurePrivilege::Deserialize(const Value &value)
{
    string requestId = "";
...
void ProcedurePrivilege::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_procedureHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_procedure.c_str(), allocator).Move(), allocator);
    }
