...
CoreInternalOutcome FunctionPrivilege::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Database") && !value["Database"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `FunctionPrivilege.Database` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("FunctionName") && !value["FunctionName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `FunctionPrivilege.FunctionName` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Privileges") && !value["Privileges"].IsNull())
    {
        if (!value["Privileges"].IsArray())
            return CoreInternalOutcome(Error("response `FunctionPrivilege.Privileges` is not array type"));
...
void FunctionPrivilege::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
