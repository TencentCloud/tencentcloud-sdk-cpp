...
CoreInternalOutcome DescribeDatabasesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Databases") && !rsp["Databases"].IsNull())
    {
        if (!rsp["Databases"].IsArray())
            return CoreInternalOutcome(Error("response `Databases` is not array type"));

        const Value &tmpValue = rsp["Databases"];
...
string DescribeDatabasesResponse::GetInstanceId() const
{
    return m_instanceId;
}

bool DescribeDatabasesResponse::InstanceIdHasBeenSet() const
{
