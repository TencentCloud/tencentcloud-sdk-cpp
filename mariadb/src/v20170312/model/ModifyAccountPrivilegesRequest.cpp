...
ModifyAccountPrivilegesRequest::ModifyAccountPrivilegesRequest() :
    ...
    m_functionPrivilegesHasBeenSet(false),
    m_procedurePrivilegesHasBeenSet(false)
{
}
...
string ModifyAccountPrivilegesRequest::ToJsonString() const
{
    ...
    if (m_functionPrivilegesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "FunctionPrivileges";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_functionPrivileges.begin(); itr != m_functionPrivileges.end(); ++itr, ++i)
        {
            d[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(d[key.c_str()][i], allocator);
        }
    ...
    if (m_procedurePrivilegesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ProcedurePrivileges";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_procedurePrivileges.begin(); itr != m_procedurePrivileges.end(); ++itr, ++i)
        {
            d[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(d[key.c_str()][i], allocator);
        }
...
vector<FunctionPrivilege> ModifyAccountPrivilegesRequest::GetFunctionPrivileges() const
{
    return m_functionPrivileges;
}

void ModifyAccountPrivilegesRequest::SetFunctionPrivileges(const vector<FunctionPrivilege>& _functionPrivileges)
{
    m_functionPrivileges = _functionPrivileges;
    m_functionPrivilegesHasBeenSet = true;
}

bool ModifyAccountPrivilegesRequest::FunctionPrivilegesHasBeenSet() const
{
    return m_functionPrivilegesHasBeenSet;
}

vector<ProcedurePrivilege> ModifyAccountPrivilegesRequest::GetProcedurePrivileges() const
{
    return m_procedurePrivileges;
}

void ModifyAccountPrivilegesRequest::SetProcedurePrivileges(const vector<ProcedurePrivilege>& _procedurePrivileges)
{
    m_procedurePrivileges = _procedurePrivileges;
    m_procedurePrivilegesHasBeenSet = true;
}

bool ModifyAccountPrivilegesRequest::ProcedurePrivilegesHasBeenSet() const
{
    return m_procedurePrivilegesHasBeenSet;
}
