...
CoreInternalOutcome OcrTextDetail::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("HitInfos") && !value["HitInfos"].IsNull())
    {
        if (!value["HitInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `OcrTextDetail.HitInfos` is not array type"));
...
vector<OcrHitInfo> OcrTextDetail::GetHitInfos() const
{
    return m_hitInfos;
}

void OcrTextDetail::SetHitInfos(const vector<OcrHitInfo>& _hitInfos)
{
    m_hitInfos = _hitInfos;
    m_hitInfosHasBeenSet = true;
}

bool OcrTextDetail::HitInfosHasBeenSet() const
{
