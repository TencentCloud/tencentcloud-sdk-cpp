...
CoreInternalOutcome User::Deserialize(const Value &value)
{
    ...
    if (value.HasMember("Gender") && !value["Gender"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `User.Gender` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Age") && !value["Age"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `User.Age` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Level") && !value["Level"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `User.Level` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
void User::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_accountTypeHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_accountType.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_headUrlHasBeenSet)
    {
        ...
        string key = "HeadUrl";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_headUrl.c_str(), allocator).Move(), allocator);
    }
