...
CoreInternalOutcome GetVirtualNumResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RefNum") && !rsp["RefNum"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `RefNum` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("VirtualNum") && !rsp["VirtualNum"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `VirtualNum` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string GetVirtualNumResponse::ToJsonString() const
{
    ...
    if (m_refNumHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "RefNum";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_refNum.c_str(), allocator).Move(), allocator);
    }

    if (m_virtualNumHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "VirtualNum";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_virtualNum.c_str(), allocator).Move(), allocator);
    }
