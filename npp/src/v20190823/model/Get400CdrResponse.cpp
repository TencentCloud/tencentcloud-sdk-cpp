...
CoreInternalOutcome Get400CdrResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Offset") && !rsp["Offset"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Offset` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    {
        if (!rsp["Cdr"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Cdr` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Cdr"];
...
string Get400CdrResponse::ToJsonString() const
{
    ...
    if (m_cdrHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_cdr.begin(); itr != m_cdr.end(); ++itr, ++i)
        {
...
string Get400CdrResponse::GetErrorCode() const
{
    return m_errorCode;
}

bool Get400CdrResponse::ErrorCodeHasBeenSet() const
{
    return m_errorCodeHasBeenSet;
}

string Get400CdrResponse::GetMsg() const
{
    return m_msg;
}

bool Get400CdrResponse::MsgHasBeenSet() const
{
    return m_msgHasBeenSet;
}

string Get400CdrResponse::GetOffset() const
{
    return m_offset;
}

bool Get400CdrResponse::OffsetHasBeenSet() const
{
    return m_offsetHasBeenSet;
}

vector<VirturalNumCdr> Get400CdrResponse::GetCdr() const
{
    return m_cdr;
}

bool Get400CdrResponse::CdrHasBeenSet() const
{
