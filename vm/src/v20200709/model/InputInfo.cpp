...
CoreInternalOutcome InputInfo::Deserialize(const Value &value)
{
    ...
    if (value.HasMember("BucketInfo") && !value["BucketInfo"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `InputInfo.BucketInfo` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void InputInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_bucketInfoHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_bucketInfo.c_str(), allocator).Move(), allocator);
    }
