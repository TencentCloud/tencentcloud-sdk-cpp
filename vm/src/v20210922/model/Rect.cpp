...
CoreInternalOutcome Rect::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Rotate") && !value["Rotate"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Rect.Rotate` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t Rect::GetRotate() const
{
    return m_rotate;
}

void Rect::SetRotate(const int64_t& _rotate)
{
    m_rotate = _rotate;
    m_rotateHasBeenSet = true;
}

bool Rect::RotateHasBeenSet() const
{
