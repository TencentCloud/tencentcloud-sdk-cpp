...
CoreInternalOutcome SentenceItem::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Sentence") && !value["Sentence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SentenceItem.Sentence` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ParaID") && !value["ParaID"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SentenceItem.ParaID` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("SentenceID") && !value["SentenceID"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SentenceItem.SentenceID` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void SentenceItem::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_sentenceHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_sentence.c_str(), allocator).Move(), allocator);
    }
