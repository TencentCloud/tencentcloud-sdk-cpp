...
CoreInternalOutcome GetFunctionLogsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SearchContext") && !rsp["SearchContext"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SearchContext` is not object type").SetRequestId(requestId));
        }
...
string GetFunctionLogsResponse::ToJsonString() const
{
    ...
    if (m_searchContextHasBeenSet)
    {
        ...
        m_searchContext.ToJsonObject(value[key.c_str()], allocator);
    }
...
int64_t GetFunctionLogsResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool GetFunctionLogsResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<FunctionLog> GetFunctionLogsResponse::GetData() const
{
    return m_data;
}
