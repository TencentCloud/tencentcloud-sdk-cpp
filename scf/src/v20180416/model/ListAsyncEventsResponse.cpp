...
CoreInternalOutcome ListAsyncEventsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EventList") && !rsp["EventList"].IsNull())
    {
        if (!rsp["EventList"].IsArray())
            return CoreInternalOutcome(Error("response `EventList` is not array type"));
...
string ListAsyncEventsResponse::ToJsonString() const
{
...
int64_t ListAsyncEventsResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool ListAsyncEventsResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<AsyncEvent> ListAsyncEventsResponse::GetEventList() const
{
    return m_eventList;
}

bool ListAsyncEventsResponse::EventListHasBeenSet() const
{
