...
CoreInternalOutcome DescribeDSPASupportedMetasResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Metas") && !rsp["Metas"].IsNull())
    {
        if (!rsp["Metas"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Metas` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Metas"];
...
string DescribeDSPASupportedMetasResponse::ToJsonString() const
{
    ...
    if (m_metasHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_metas.begin(); itr != m_metas.end(); ++itr, ++i)
        {
...
vector<DSPAMetaType> DescribeDSPASupportedMetasResponse::GetMetas() const
{
    return m_metas;
}
