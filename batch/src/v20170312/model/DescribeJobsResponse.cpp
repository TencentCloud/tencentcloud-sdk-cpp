...
CoreInternalOutcome DescribeJobsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("JobSet") && !rsp["JobSet"].IsNull())
    {
        if (!rsp["JobSet"].IsArray())
            return CoreInternalOutcome(Error("response `JobSet` is not array type"));
...
string DescribeJobsResponse::ToJsonString() const
{
...
vector<JobView> DescribeJobsResponse::GetJobSet() const
{
    return m_jobSet;
}

bool DescribeJobsResponse::JobSetHasBeenSet() const
{
    return m_jobSetHasBeenSet;
}

int64_t DescribeJobsResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeJobsResponse::TotalCountHasBeenSet() const
{
