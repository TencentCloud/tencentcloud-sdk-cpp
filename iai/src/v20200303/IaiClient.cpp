...
{
    using Req = const CompareMaskFaceRequest&;
    using Resp = CompareMaskFaceResponse;

    DoRequestAsync<Req, Resp>(
        "CompareMaskFace", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
{
    const auto prom = std::make_shared<std::promise<CompareMaskFaceOutcome>>();
    CompareMaskFaceAsync(
    request,
    [prom](
        const IaiClient*,
        const CompareMaskFaceRequest&,
        CompareMaskFaceOutcome resp,
        const std::shared_ptr<const AsyncCallerContext>&
    )
    {
        prom->set_value(resp);
...
{
    using Req = const DetectFaceSimilarityRequest&;
    using Resp = DetectFaceSimilarityResponse;

    DoRequestAsync<Req, Resp>(
        "DetectFaceSimilarity", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
{
    const auto prom = std::make_shared<std::promise<DetectFaceSimilarityOutcome>>();
    DetectFaceSimilarityAsync(
    request,
    [prom](
        const IaiClient*,
        const DetectFaceSimilarityRequest&,
        DetectFaceSimilarityOutcome resp,
        const std::shared_ptr<const AsyncCallerContext>&
    )
    {
        prom->set_value(resp);
...
{
    using Req = const DetectLiveFaceAccurateRequest&;
    using Resp = DetectLiveFaceAccurateResponse;

    DoRequestAsync<Req, Resp>(
        "DetectLiveFaceAccurate", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
{
    const auto prom = std::make_shared<std::promise<DetectLiveFaceAccurateOutcome>>();
    DetectLiveFaceAccurateAsync(
    request,
    [prom](
        const IaiClient*,
        const DetectLiveFaceAccurateRequest&,
        DetectLiveFaceAccurateOutcome resp,
        const std::shared_ptr<const AsyncCallerContext>&
    )
    {
        prom->set_value(resp);
