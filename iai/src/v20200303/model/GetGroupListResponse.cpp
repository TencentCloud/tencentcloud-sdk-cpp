...
CoreInternalOutcome GetGroupListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("GroupInfos") && !rsp["GroupInfos"].IsNull())
    {
        if (!rsp["GroupInfos"].IsArray())
            return CoreInternalOutcome(Error("response `GroupInfos` is not array type"));

        const Value &tmpValue = rsp["GroupInfos"];
...
uint64_t GetGroupListResponse::GetGroupNum() const
{
    return m_groupNum;
}

bool GetGroupListResponse::GroupNumHasBeenSet() const
{
