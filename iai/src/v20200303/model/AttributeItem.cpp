...
CoreInternalOutcome AttributeItem::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Probability") && !value["Probability"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `AttributeItem.Probability` IsDouble=false incorrectly").SetRequestId(requestId));
        }
...
void AttributeItem::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
