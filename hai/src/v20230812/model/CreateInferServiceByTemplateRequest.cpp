/*
 * Copyright (c) 2017-2025 Tencent. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include <tencentcloud/hai/v20230812/model/CreateInferServiceByTemplateRequest.h>
#include <tencentcloud/core/utils/rapidjson/document.h>
#include <tencentcloud/core/utils/rapidjson/writer.h>
#include <tencentcloud/core/utils/rapidjson/stringbuffer.h>

using namespace TencentCloud::Hai::V20230812::Model;
using namespace std;

CreateInferServiceByTemplateRequest::CreateInferServiceByTemplateRequest() :
    m_templateIdHasBeenSet(false),
    m_serviceNameHasBeenSet(false),
    m_replicasHasBeenSet(false),
    m_serviceChargeTypeHasBeenSet(false),
    m_hyperParamHasBeenSet(false),
    m_networkSettingHasBeenSet(false),
    m_securityTypeHasBeenSet(false),
    m_serviceChargePrepaidHasBeenSet(false)
{
}

string CreateInferServiceByTemplateRequest::ToJsonString() const
{
    rapidjson::Document d;
    d.SetObject();
    rapidjson::Document::AllocatorType& allocator = d.GetAllocator();


    if (m_templateIdHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TemplateId";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(m_templateId.c_str(), allocator).Move(), allocator);
    }

    if (m_serviceNameHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ServiceName";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(m_serviceName.c_str(), allocator).Move(), allocator);
    }

    if (m_replicasHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "Replicas";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, m_replicas, allocator);
    }

    if (m_serviceChargeTypeHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ServiceChargeType";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(m_serviceChargeType.c_str(), allocator).Move(), allocator);
    }

    if (m_hyperParamHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "HyperParam";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_hyperParam.ToJsonObject(d[key.c_str()], allocator);
    }

    if (m_networkSettingHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "NetworkSetting";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_networkSetting.ToJsonObject(d[key.c_str()], allocator);
    }

    if (m_securityTypeHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SecurityType";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(m_securityType.c_str(), allocator).Move(), allocator);
    }

    if (m_serviceChargePrepaidHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ServiceChargePrepaid";
        iKey.SetString(key.c_str(), allocator);
        d.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_serviceChargePrepaid.ToJsonObject(d[key.c_str()], allocator);
    }


    rapidjson::StringBuffer buffer;
    rapidjson::Writer<rapidjson::StringBuffer> writer(buffer);
    d.Accept(writer);
    return buffer.GetString();
}


string CreateInferServiceByTemplateRequest::GetTemplateId() const
{
    return m_templateId;
}

void CreateInferServiceByTemplateRequest::SetTemplateId(const string& _templateId)
{
    m_templateId = _templateId;
    m_templateIdHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::TemplateIdHasBeenSet() const
{
    return m_templateIdHasBeenSet;
}

string CreateInferServiceByTemplateRequest::GetServiceName() const
{
    return m_serviceName;
}

void CreateInferServiceByTemplateRequest::SetServiceName(const string& _serviceName)
{
    m_serviceName = _serviceName;
    m_serviceNameHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::ServiceNameHasBeenSet() const
{
    return m_serviceNameHasBeenSet;
}

int64_t CreateInferServiceByTemplateRequest::GetReplicas() const
{
    return m_replicas;
}

void CreateInferServiceByTemplateRequest::SetReplicas(const int64_t& _replicas)
{
    m_replicas = _replicas;
    m_replicasHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::ReplicasHasBeenSet() const
{
    return m_replicasHasBeenSet;
}

string CreateInferServiceByTemplateRequest::GetServiceChargeType() const
{
    return m_serviceChargeType;
}

void CreateInferServiceByTemplateRequest::SetServiceChargeType(const string& _serviceChargeType)
{
    m_serviceChargeType = _serviceChargeType;
    m_serviceChargeTypeHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::ServiceChargeTypeHasBeenSet() const
{
    return m_serviceChargeTypeHasBeenSet;
}

HyperParam CreateInferServiceByTemplateRequest::GetHyperParam() const
{
    return m_hyperParam;
}

void CreateInferServiceByTemplateRequest::SetHyperParam(const HyperParam& _hyperParam)
{
    m_hyperParam = _hyperParam;
    m_hyperParamHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::HyperParamHasBeenSet() const
{
    return m_hyperParamHasBeenSet;
}

NetworkSetting CreateInferServiceByTemplateRequest::GetNetworkSetting() const
{
    return m_networkSetting;
}

void CreateInferServiceByTemplateRequest::SetNetworkSetting(const NetworkSetting& _networkSetting)
{
    m_networkSetting = _networkSetting;
    m_networkSettingHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::NetworkSettingHasBeenSet() const
{
    return m_networkSettingHasBeenSet;
}

string CreateInferServiceByTemplateRequest::GetSecurityType() const
{
    return m_securityType;
}

void CreateInferServiceByTemplateRequest::SetSecurityType(const string& _securityType)
{
    m_securityType = _securityType;
    m_securityTypeHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::SecurityTypeHasBeenSet() const
{
    return m_securityTypeHasBeenSet;
}

ServiceChargePrepaid CreateInferServiceByTemplateRequest::GetServiceChargePrepaid() const
{
    return m_serviceChargePrepaid;
}

void CreateInferServiceByTemplateRequest::SetServiceChargePrepaid(const ServiceChargePrepaid& _serviceChargePrepaid)
{
    m_serviceChargePrepaid = _serviceChargePrepaid;
    m_serviceChargePrepaidHasBeenSet = true;
}

bool CreateInferServiceByTemplateRequest::ServiceChargePrepaidHasBeenSet() const
{
    return m_serviceChargePrepaidHasBeenSet;
}


