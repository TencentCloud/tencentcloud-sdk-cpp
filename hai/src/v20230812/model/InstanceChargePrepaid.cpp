...
CoreInternalOutcome InstanceChargePrepaid::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TimeUnit") && !value["TimeUnit"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceChargePrepaid.TimeUnit` IsString=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t InstanceChargePrepaid::GetPeriod() const
{
    return m_period;
}

void InstanceChargePrepaid::SetPeriod(const uint64_t& _period)
{
    m_period = _period;
    m_periodHasBeenSet = true;
}
...
string InstanceChargePrepaid::GetTimeUnit() const
{
    return m_timeUnit;
}

void InstanceChargePrepaid::SetTimeUnit(const string& _timeUnit)
{
    m_timeUnit = _timeUnit;
    m_timeUnitHasBeenSet = true;
}

bool InstanceChargePrepaid::TimeUnitHasBeenSet() const
{
