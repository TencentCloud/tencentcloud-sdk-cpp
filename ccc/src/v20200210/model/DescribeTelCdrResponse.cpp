...
CoreInternalOutcome DescribeTelCdrResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TelCdrs") && !rsp["TelCdrs"].IsNull())
    {
        if (!rsp["TelCdrs"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TelCdrs` is not array type"));
...
string DescribeTelCdrResponse::ToJsonString() const
{
...
vector<TelCdrInfo> DescribeTelCdrResponse::GetTelCdrList() const
{
    return m_telCdrList;
}

bool DescribeTelCdrResponse::TelCdrListHasBeenSet() const
{
