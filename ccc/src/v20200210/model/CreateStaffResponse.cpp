...
CoreInternalOutcome CreateStaffResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ErrorStaffList") && !rsp["ErrorStaffList"].IsNull())
    {
        if (!rsp["ErrorStaffList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ErrorStaffList` is not array type"));
...
string CreateStaffResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_errorStaffListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ErrorStaffList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_errorStaffList.begin(); itr != m_errorStaffList.end(); ++itr, ++i)
        {
