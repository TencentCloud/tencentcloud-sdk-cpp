...
CoreInternalOutcome DescribeChatMessagesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Messages") && !rsp["Messages"].IsNull())
    {
        if (!rsp["Messages"].IsArray())
            return CoreInternalOutcome(Error("response `Messages` is not array type"));
...
string DescribeChatMessagesResponse::ToJsonString() const
{
...
int64_t DescribeChatMessagesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeChatMessagesResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<MessageBody> DescribeChatMessagesResponse::GetMessages() const
{
    return m_messages;
}

bool DescribeChatMessagesResponse::MessagesHasBeenSet() const
{
