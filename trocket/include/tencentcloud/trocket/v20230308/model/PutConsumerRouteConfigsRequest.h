...
#ifndef TENCENTCLOUD_TROCKET_V20230308_MODEL_PUTCONSUMERROUTECONFIGSREQUEST_H_
#define TENCENTCLOUD_TROCKET_V20230308_MODEL_PUTCONSUMERROUTECONFIGSREQUEST_H_
    ...
    namespace Trocket
    {
        namespace V20230308
        {
            namespace Model
            {
                ...
                */
                class PutConsumerRouteConfigsRequest : public AbstractModel
                {
                public:
                    PutConsumerRouteConfigsRequest();
                    ~PutConsumerRouteConfigsRequest() = default;
                    ...
                    /**
                     * 获取<p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
                     * @return Configs <p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
                     * 
                     */
                    std::vector<PutConsumerRouteConfigItem> GetConfigs() const;

                    /**
                     * 设置<p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
                     * @param _configs <p>批量写入的消费者路由配置列表</p><p>入参限制：批量上限为 32 条</p><p>覆盖式写入</p>
                     * 
                     */
                    void SetConfigs(const std::vector<PutConsumerRouteConfigItem>& _configs);
                ...
                private:
                     ...
                     */
                    std::vector<PutConsumerRouteConfigItem> m_configs;
...
#endif // !TENCENTCLOUD_TROCKET_V20230308_MODEL_PUTCONSUMERROUTECONFIGSREQUEST_H_
