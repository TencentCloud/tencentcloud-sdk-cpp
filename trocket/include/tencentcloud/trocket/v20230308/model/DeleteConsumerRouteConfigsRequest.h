...
#ifndef TENCENTCLOUD_TROCKET_V20230308_MODEL_DELETECONSUMERROUTECONFIGSREQUEST_H_
#define TENCENTCLOUD_TROCKET_V20230308_MODEL_DELETECONSUMERROUTECONFIGSREQUEST_H_
    ...
    namespace Trocket
    {
        namespace V20230308
        {
            namespace Model
            {
                ...
                */
                class DeleteConsumerRouteConfigsRequest : public AbstractModel
                {
                public:
                    DeleteConsumerRouteConfigsRequest();
                    ~DeleteConsumerRouteConfigsRequest() = default;
                    ...
                    /**
                     * 获取<p>批量删除的消费组路由列表</p><p>入参限制：批量上限为 32 条</p><p>传入 Label 表示只删除该标签路由，不传表示删除完整路由</p>
                     * @return Configs <p>批量删除的消费组路由列表</p><p>入参限制：批量上限为 32 条</p><p>传入 Label 表示只删除该标签路由，不传表示删除完整路由</p>
                     * 
                     */
                    std::vector<ConsumerRouteLabelKey> GetConfigs() const;

                    /**
                     * 设置<p>批量删除的消费组路由列表</p><p>入参限制：批量上限为 32 条</p><p>传入 Label 表示只删除该标签路由，不传表示删除完整路由</p>
                     * @param _configs <p>批量删除的消费组路由列表</p><p>入参限制：批量上限为 32 条</p><p>传入 Label 表示只删除该标签路由，不传表示删除完整路由</p>
                     * 
                     */
                    void SetConfigs(const std::vector<ConsumerRouteLabelKey>& _configs);
                ...
                private:
                     ...
                     */
                    std::vector<ConsumerRouteLabelKey> m_configs;
...
#endif // !TENCENTCLOUD_TROCKET_V20230308_MODEL_DELETECONSUMERROUTECONFIGSREQUEST_H_
