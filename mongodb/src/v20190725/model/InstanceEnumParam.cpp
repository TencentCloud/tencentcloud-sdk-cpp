...
CoreInternalOutcome InstanceEnumParam::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tips") && !value["Tips"].IsNull())
    {
        if (!value["Tips"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InstanceEnumParam.Tips` is not array type"));
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceEnumParam.Status` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
vector<string> InstanceEnumParam::GetTips() const
{
    return m_tips;
}

void InstanceEnumParam::SetTips(const vector<string>& _tips)
{
    m_tips = _tips;
    m_tipsHasBeenSet = true;
}
...
uint64_t InstanceEnumParam::GetStatus() const
{
    return m_status;
}

void InstanceEnumParam::SetStatus(const uint64_t& _status)
{
    m_status = _status;
    m_statusHasBeenSet = true;
}
