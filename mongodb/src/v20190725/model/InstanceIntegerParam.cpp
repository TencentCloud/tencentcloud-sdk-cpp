...
CoreInternalOutcome InstanceIntegerParam::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tips") && !value["Tips"].IsNull())
    {
        if (!value["Tips"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InstanceIntegerParam.Tips` is not array type"));
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceIntegerParam.Status` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
vector<string> InstanceIntegerParam::GetTips() const
{
    return m_tips;
}

void InstanceIntegerParam::SetTips(const vector<string>& _tips)
{
    m_tips = _tips;
    m_tipsHasBeenSet = true;
}
...
uint64_t InstanceIntegerParam::GetStatus() const
{
    return m_status;
}

void InstanceIntegerParam::SetStatus(const uint64_t& _status)
{
    m_status = _status;
    m_statusHasBeenSet = true;
}
