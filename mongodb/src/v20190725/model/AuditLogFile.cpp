...
CoreInternalOutcome AuditLogFile::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["ProgressRate"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `AuditLogFile.ProgressRate` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_progressRate = value["ProgressRate"].GetUint64();
...
uint64_t AuditLogFile::GetProgressRate() const
{
    return m_progressRate;
}

void AuditLogFile::SetProgressRate(const uint64_t& _progressRate)
{
