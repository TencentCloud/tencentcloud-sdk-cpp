...
CoreInternalOutcome Thinking::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Type") && !value["Type"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Thinking.Type` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string Thinking::GetType() const
{
    return m_type;
}

void Thinking::SetType(const string& _type)
{
    m_type = _type;
    m_typeHasBeenSet = true;
}

bool Thinking::TypeHasBeenSet() const
{
