...
CoreInternalOutcome DescribeConsumerOffsetsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TopicPartitionOffsetsInfo") && !rsp["TopicPartitionOffsetsInfo"].IsNull())
    {
        if (!rsp["TopicPartitionOffsetsInfo"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TopicPartitionOffsetsInfo` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TopicPartitionOffsetsInfo"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            TopicPartitionOffsetInfo item;
            ...
            m_topicPartitionOffsetsInfo.push_back(item);
        }
...
string DescribeConsumerOffsetsResponse::ToJsonString() const
{
...
string DescribeConsumerOffsetsResponse::GetConsumerGroup() const
{
    return m_consumerGroup;
}

bool DescribeConsumerOffsetsResponse::ConsumerGroupHasBeenSet() const
{
    return m_consumerGroupHasBeenSet;
}

vector<TopicPartitionOffsetInfo> DescribeConsumerOffsetsResponse::GetTopicPartitionOffsetsInfo() const
{
    return m_topicPartitionOffsetsInfo;
}

bool DescribeConsumerOffsetsResponse::TopicPartitionOffsetsInfoHasBeenSet() const
{
