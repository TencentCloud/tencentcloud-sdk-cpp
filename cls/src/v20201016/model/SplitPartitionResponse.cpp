...
CoreInternalOutcome SplitPartitionResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Partitions") && !rsp["Partitions"].IsNull())
    {
        if (!rsp["Partitions"].IsArray())
            return CoreInternalOutcome(Error("response `Partitions` is not array type"));
...
string SplitPartitionResponse::ToJsonString() const
{
...
vector<PartitionInfo> SplitPartitionResponse::GetPartitions() const
{
    return m_partitions;
}

bool SplitPartitionResponse::PartitionsHasBeenSet() const
{
