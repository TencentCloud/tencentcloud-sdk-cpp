...
CoreInternalOutcome Instance::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Values") && !value["Values"].IsNull())
    {
        if (!value["Values"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Instance.Values` is not array type"));
...
vector<string> Instance::GetValues() const
{
    return m_values;
}

void Instance::SetValues(const vector<string>& _values)
{
    m_values = _values;
    m_valuesHasBeenSet = true;
}

bool Instance::ValuesHasBeenSet() const
{
