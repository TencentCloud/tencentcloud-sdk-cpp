...
CoreInternalOutcome ToolCall::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Index") && !value["Index"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ToolCall.Index` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t ToolCall::GetIndex() const
{
    return m_index;
}

void ToolCall::SetIndex(const uint64_t& _index)
{
    m_index = _index;
    m_indexHasBeenSet = true;
}
