...
CoreInternalOutcome CreateAlarmResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["AlarmId"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `AlarmId` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_alarmId = string(rsp["AlarmId"].GetString());
...
string CreateAlarmResponse::ToJsonString() const
{
...
string CreateAlarmResponse::GetAlarmId() const
{
    return m_alarmId;
}
