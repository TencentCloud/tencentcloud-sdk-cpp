...
CoreInternalOutcome RuleKeyValueInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("CaseSensitive") && !value["CaseSensitive"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `RuleKeyValueInfo.CaseSensitive` IsBool=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("KeyValues") && !value["KeyValues"].IsNull())
    {
        if (!value["KeyValues"].IsArray())
            return CoreInternalOutcome(Error("response `RuleKeyValueInfo.KeyValues` is not array type"));
