...
CoreInternalOutcome ChatCompletionsResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["Created"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `Created` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_created = rsp["Created"].GetUint64();
...
uint64_t ChatCompletionsResponse::GetCreated() const
{
    return m_created;
}
