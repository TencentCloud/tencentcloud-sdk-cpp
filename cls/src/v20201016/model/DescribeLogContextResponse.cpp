...
CoreInternalOutcome DescribeLogContextResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LogContextInfos") && !rsp["LogContextInfos"].IsNull())
    {
        if (!rsp["LogContextInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LogContextInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["LogContextInfos"];
...
string DescribeLogContextResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_logContextInfosHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "LogContextInfos";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_logContextInfos.begin(); itr != m_logContextInfos.end(); ++itr, ++i)
        {
