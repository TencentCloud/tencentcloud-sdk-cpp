...
CoreInternalOutcome ShieldArea::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("DDoSHosts") && !value["DDoSHosts"].IsNull())
    {
        if (!value["DDoSHosts"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ShieldArea.DDoSHosts` is not array type"));

        const rapidjson::Value &tmpValue = value["DDoSHosts"];
...
vector<DDoSHost> ShieldArea::GetDDoSHosts() const
{
    return m_dDoSHosts;
}

void ShieldArea::SetDDoSHosts(const vector<DDoSHost>& _dDoSHosts)
{
    m_dDoSHosts = _dDoSHosts;
    m_dDoSHostsHasBeenSet = true;
}

bool ShieldArea::DDoSHostsHasBeenSet() const
{
