...
CoreInternalOutcome AliasDomain::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("AliasId") && !value["AliasId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AliasDomain.AliasId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string AliasDomain::GetAliasId() const
{
    return m_aliasId;
}

void AliasDomain::SetAliasId(const string& _aliasId)
{
    m_aliasId = _aliasId;
    m_aliasIdHasBeenSet = true;
}

bool AliasDomain::AliasIdHasBeenSet() const
{
