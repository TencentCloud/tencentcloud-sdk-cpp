...
CoreInternalOutcome DescribeSecurityRuleIdResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("WafGroupRules") && !rsp["WafGroupRules"].IsNull())
    {
        if (!rsp["WafGroupRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `WafGroupRules` is not array type"));

        const rapidjson::Value &tmpValue = rsp["WafGroupRules"];
...
string DescribeSecurityRuleIdResponse::ToJsonString() const
{
...
vector<WafGroupRule> DescribeSecurityRuleIdResponse::GetWafGroupRules() const
{
