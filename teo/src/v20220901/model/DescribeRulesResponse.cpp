...
CoreInternalOutcome DescribeRulesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RuleItems") && !rsp["RuleItems"].IsNull())
    {
        if (!rsp["RuleItems"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RuleItems` is not array type"));

        const rapidjson::Value &tmpValue = rsp["RuleItems"];
...
string DescribeRulesResponse::GetZoneId() const
{
    return m_zoneId;
}

bool DescribeRulesResponse::ZoneIdHasBeenSet() const
{
    return m_zoneIdHasBeenSet;
}

vector<RuleItem> DescribeRulesResponse::GetRuleItems() const
{
    return m_ruleItems;
}

bool DescribeRulesResponse::RuleItemsHasBeenSet() const
{
