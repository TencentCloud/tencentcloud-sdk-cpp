...
CoreInternalOutcome ModifyResponseHeaderParameters::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("HeaderActions") && !value["HeaderActions"].IsNull())
    {
        if (!value["HeaderActions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ModifyResponseHeaderParameters.HeaderActions` is not array type"));
...
void ModifyResponseHeaderParameters::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<HeaderAction> ModifyResponseHeaderParameters::GetHeaderActions() const
{
    return m_headerActions;
}

void ModifyResponseHeaderParameters::SetHeaderActions(const vector<HeaderAction>& _headerActions)
{
    m_headerActions = _headerActions;
    m_headerActionsHasBeenSet = true;
}

bool ModifyResponseHeaderParameters::HeaderActionsHasBeenSet() const
{
