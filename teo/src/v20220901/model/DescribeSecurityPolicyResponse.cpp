...
CoreInternalOutcome DescribeSecurityPolicyResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SecurityConfig") && !rsp["SecurityConfig"].IsNull())
    {
        if (!rsp["SecurityConfig"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `SecurityConfig` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_securityConfig.Deserialize(rsp["SecurityConfig"]);
...
SecurityConfig DescribeSecurityPolicyResponse::GetSecurityConfig() const
{
    return m_securityConfig;
}

bool DescribeSecurityPolicyResponse::SecurityConfigHasBeenSet() const
{
