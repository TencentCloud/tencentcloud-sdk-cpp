...
CoreInternalOutcome DescribeSecurityPolicyResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["SecurityPolicy"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `SecurityPolicy` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_securityPolicy.Deserialize(rsp["SecurityPolicy"]);
...
string DescribeSecurityPolicyResponse::ToJsonString() const
{
    ...
    if (m_securityPolicyHasBeenSet)
    {
        ...
        m_securityPolicy.ToJsonObject(value[key.c_str()], allocator);
    }
...
SecurityPolicy DescribeSecurityPolicyResponse::GetSecurityPolicy() const
{
    return m_securityPolicy;
}

bool DescribeSecurityPolicyResponse::SecurityPolicyHasBeenSet() const
{
