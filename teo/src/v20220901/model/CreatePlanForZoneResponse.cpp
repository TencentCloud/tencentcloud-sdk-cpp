...
CoreInternalOutcome CreatePlanForZoneResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ResourceNames") && !rsp["ResourceNames"].IsNull())
    {
        if (!rsp["ResourceNames"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ResourceNames` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ResourceNames"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_resourceNames.push_back((*itr).GetString());
        }
...
string CreatePlanForZoneResponse::ToJsonString() const
{
...
vector<string> CreatePlanForZoneResponse::GetResourceNames() const
{
    return m_resourceNames;
}

bool CreatePlanForZoneResponse::ResourceNamesHasBeenSet() const
{
    return m_resourceNamesHasBeenSet;
}

vector<string> CreatePlanForZoneResponse::GetDealNames() const
{
    return m_dealNames;
}

bool CreatePlanForZoneResponse::DealNamesHasBeenSet() const
{
