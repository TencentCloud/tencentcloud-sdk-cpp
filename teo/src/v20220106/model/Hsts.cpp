...
CoreInternalOutcome Hsts::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Preload") && !value["Preload"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Hsts.Preload` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string Hsts::GetPreload() const
{
    return m_preload;
}

void Hsts::SetPreload(const string& _preload)
{
    m_preload = _preload;
    m_preloadHasBeenSet = true;
}

bool Hsts::PreloadHasBeenSet() const
{
