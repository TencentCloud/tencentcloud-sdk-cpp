...
CoreInternalOutcome ExceptConfig::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("UserRules") && !value["UserRules"].IsNull())
    {
        if (!value["UserRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ExceptConfig.UserRules` is not array type"));
...
vector<ExceptUserRule> ExceptConfig::GetUserRules() const
{
    return m_userRules;
}

void ExceptConfig::SetUserRules(const vector<ExceptUserRule>& _userRules)
{
    m_userRules = _userRules;
    m_userRulesHasBeenSet = true;
}

bool ExceptConfig::UserRulesHasBeenSet() const
{
