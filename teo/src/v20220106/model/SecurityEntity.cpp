...
CoreInternalOutcome SecurityEntity::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("AppId") && !value["AppId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SecurityEntity.AppId` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t SecurityEntity::GetAppId() const
{
    return m_appId;
}

void SecurityEntity::SetAppId(const int64_t& _appId)
{
    m_appId = _appId;
    m_appIdHasBeenSet = true;
}

bool SecurityEntity::AppIdHasBeenSet() const
{
