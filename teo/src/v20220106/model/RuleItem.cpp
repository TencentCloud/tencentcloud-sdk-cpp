...
CoreInternalOutcome RuleItem::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Conditions") && !value["Conditions"].IsNull())
    {
        if (!value["Conditions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RuleItem.Conditions` is not array type"));
    ...
    if (value.HasMember("Actions") && !value["Actions"].IsNull())
    {
        if (!value["Actions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RuleItem.Actions` is not array type"));
...
vector<RuleAndConditions> RuleItem::GetConditions() const
{
    return m_conditions;
}

void RuleItem::SetConditions(const vector<RuleAndConditions>& _conditions)
{
    m_conditions = _conditions;
    m_conditionsHasBeenSet = true;
}

bool RuleItem::ConditionsHasBeenSet() const
{
    return m_conditionsHasBeenSet;
}

vector<RuleAction> RuleItem::GetActions() const
{
    return m_actions;
}

void RuleItem::SetActions(const vector<RuleAction>& _actions)
{
    m_actions = _actions;
    m_actionsHasBeenSet = true;
}

bool RuleItem::ActionsHasBeenSet() const
{
