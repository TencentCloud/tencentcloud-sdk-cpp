...
CoreInternalOutcome IntelligenceRule::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Items") && !value["Items"].IsNull())
    {
        if (!value["Items"].IsArray())
            return CoreInternalOutcome(Core::Error("response `IntelligenceRule.Items` is not array type"));
...
vector<IntelligenceRuleItem> IntelligenceRule::GetItems() const
{
    return m_items;
}

void IntelligenceRule::SetItems(const vector<IntelligenceRuleItem>& _items)
{
    m_items = _items;
    m_itemsHasBeenSet = true;
}

bool IntelligenceRule::ItemsHasBeenSet() const
{
