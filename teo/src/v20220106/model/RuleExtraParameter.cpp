...
#include <tencentcloud/teo/v20220106/model/RuleExtraParameter.h>
...
CoreInternalOutcome RuleExtraParameter::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["Choices"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `RuleExtraParameter.Choices` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_choices = string(value["Choices"].GetString());
...
void RuleExtraParameter::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_choicesHasBeenSet)
    {
        ...
        string key = "Choices";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_choices.c_str(), allocator).Move(), allocator);
    }
...
string RuleExtraParameter::GetChoices() const
{
    return m_choices;
}

void RuleExtraParameter::SetChoices(const string& _choices)
{
    m_choices = _choices;
    m_choicesHasBeenSet = true;
}
