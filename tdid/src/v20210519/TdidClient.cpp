...
TdidClient::VerifyPurchaseOutcome TdidClient::VerifyPurchase(const VerifyPurchaseRequest &request)
{
    auto outcome = MakeRequest(request, "VerifyPurchase");
    if (outcome.IsSuccess())
    {
        auto r = outcome.GetResult();
        string payload = string(r.Body(), r.BodySize());
        VerifyPurchaseResponse rsp = VerifyPurchaseResponse();
        auto o = rsp.Deserialize(payload);
        if (o.IsSuccess())
            return VerifyPurchaseOutcome(rsp);
        else
            return VerifyPurchaseOutcome(o.GetError());
    }
    else
    {
        return VerifyPurchaseOutcome(outcome.GetError());
    }
}

void TdidClient::VerifyPurchaseAsync(const VerifyPurchaseRequest& request, const VerifyPurchaseAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context)
{
    auto fn = [this, request, handler, context]()
    {
        handler(this, request, this->VerifyPurchase(request), context);
    };

    Executor::GetInstance()->Submit(new Runnable(fn));
}

TdidClient::VerifyPurchaseOutcomeCallable TdidClient::VerifyPurchaseCallable(const VerifyPurchaseRequest &request)
{
    auto task = std::make_shared<std::packaged_task<VerifyPurchaseOutcome()>>(
        [this, request]()
        {
            return this->VerifyPurchase(request);
        }
    );
