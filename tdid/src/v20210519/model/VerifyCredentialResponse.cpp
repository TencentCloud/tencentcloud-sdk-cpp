...
CoreInternalOutcome VerifyCredentialResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("VerifyCode") && !rsp["VerifyCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `VerifyCode` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("VerifyMessage") && !rsp["VerifyMessage"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `VerifyMessage` IsString=false incorrectly").SetRequestId(requestId));
        }
