...
CoreInternalOutcome VideoModerationResponse::Deserialize(const string &payload)
{
    Document d;
    ...
    if (rsp.HasMember("BusinessCode") && !rsp["BusinessCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `BusinessCode` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t VideoModerationResponse::GetBusinessCode() const
{
    return m_businessCode;
}

bool VideoModerationResponse::BusinessCodeHasBeenSet() const
{
    return m_businessCodeHasBeenSet;
}

string VideoModerationResponse::GetData() const
{
    return m_data;
}

bool VideoModerationResponse::DataHasBeenSet() const
{
