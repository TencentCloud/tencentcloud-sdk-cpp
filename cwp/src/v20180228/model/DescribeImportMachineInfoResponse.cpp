...
CoreInternalOutcome DescribeImportMachineInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EffectiveMachineInfoList") && !rsp["EffectiveMachineInfoList"].IsNull())
    {
        if (!rsp["EffectiveMachineInfoList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `EffectiveMachineInfoList` is not array type"));
    ...
    if (rsp.HasMember("InvalidMachineList") && !rsp["InvalidMachineList"].IsNull())
    {
        if (!rsp["InvalidMachineList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InvalidMachineList` is not array type"));
...
string DescribeImportMachineInfoResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_effectiveMachineInfoListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "EffectiveMachineInfoList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_effectiveMachineInfoList.begin(); itr != m_effectiveMachineInfoList.end(); ++itr, ++i)
        {
    ...
    if (m_invalidMachineListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "InvalidMachineList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_invalidMachineList.begin(); itr != m_invalidMachineList.end(); ++itr)
        {
