...
CoreInternalOutcome DescribeScanTaskStatusResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["State"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `State` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_state.Deserialize(rsp["State"]);
...
TaskStatus DescribeScanTaskStatusResponse::GetState() const
{
    return m_state;
}

bool DescribeScanTaskStatusResponse::StateHasBeenSet() const
{
