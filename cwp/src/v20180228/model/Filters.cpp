...
CoreInternalOutcome Filters::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExactMatch") && !value["ExactMatch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Filters.ExactMatch` IsBool=false incorrectly").SetRequestId(requestId));
        }
...
bool Filters::GetExactMatch() const
{
    return m_exactMatch;
}

void Filters::SetExactMatch(const bool& _exactMatch)
{
