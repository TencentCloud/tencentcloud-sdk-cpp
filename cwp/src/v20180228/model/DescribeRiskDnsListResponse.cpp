...
CoreInternalOutcome DescribeRiskDnsListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RiskDnsList") && !rsp["RiskDnsList"].IsNull())
    {
        if (!rsp["RiskDnsList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RiskDnsList` is not array type"));
...
string DescribeRiskDnsListResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_riskDnsListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "RiskDnsList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_riskDnsList.begin(); itr != m_riskDnsList.end(); ++itr, ++i)
        {
