...
CoreInternalOutcome ListOrganizationNodesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Nodes") && !rsp["Nodes"].IsNull())
    {
        if (!rsp["Nodes"].IsArray())
            return CoreInternalOutcome(Error("response `Nodes` is not array type"));
...
string ListOrganizationNodesResponse::ToJsonString() const
{
...
vector<OrgNode> ListOrganizationNodesResponse::GetNodes() const
{
    return m_nodes;
}

bool ListOrganizationNodesResponse::NodesHasBeenSet() const
{
