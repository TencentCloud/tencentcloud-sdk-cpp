...
CoreInternalOutcome ShareMember::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("ShareMemberUin") && !value["ShareMemberUin"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ShareMember.ShareMemberUin` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void ShareMember::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
int64_t ShareMember::GetShareMemberUin() const
{
    return m_shareMemberUin;
}

void ShareMember::SetShareMemberUin(const int64_t& _shareMemberUin)
{
    m_shareMemberUin = _shareMemberUin;
    m_shareMemberUinHasBeenSet = true;
}

bool ShareMember::ShareMemberUinHasBeenSet() const
{
