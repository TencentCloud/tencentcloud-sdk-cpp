...
CoreInternalOutcome GetTaskStatusResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["TaskStatus"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `TaskStatus` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_taskStatus.Deserialize(rsp["TaskStatus"]);
...
TaskStatus GetTaskStatusResponse::GetTaskStatus() const
{
    return m_taskStatus;
}

bool GetTaskStatusResponse::TaskStatusHasBeenSet() const
{
