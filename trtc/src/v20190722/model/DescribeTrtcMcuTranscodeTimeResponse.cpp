...
CoreInternalOutcome DescribeTrtcMcuTranscodeTimeResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Usages") && !rsp["Usages"].IsNull())
    {
        if (!rsp["Usages"].IsArray())
            return CoreInternalOutcome(Error("response `Usages` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Usages"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            OneSdkAppIdTranscodeTimeUsagesInfo item;
...
string DescribeTrtcMcuTranscodeTimeResponse::ToJsonString() const
{
...
vector<OneSdkAppIdTranscodeTimeUsagesInfo> DescribeTrtcMcuTranscodeTimeResponse::GetUsages() const
{
    return m_usages;
}

bool DescribeTrtcMcuTranscodeTimeResponse::UsagesHasBeenSet() const
{
