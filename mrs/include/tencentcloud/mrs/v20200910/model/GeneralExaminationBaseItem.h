/*
 * Copyright (c) 2017-2019 THL A29 Limited, a Tencent company. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef TENCENTCLOUD_MRS_V20200910_MODEL_GENERALEXAMINATIONBASEITEM_H_
#define TENCENTCLOUD_MRS_V20200910_MODEL_GENERALEXAMINATIONBASEITEM_H_

#include <string>
#include <vector>
#include <map>
#include <tencentcloud/core/utils/rapidjson/document.h>
#include <tencentcloud/core/utils/rapidjson/writer.h>
#include <tencentcloud/core/utils/rapidjson/stringbuffer.h>
#include <tencentcloud/core/AbstractModel.h>
#include <tencentcloud/mrs/v20200910/model/GeneralExaminationVitalSign.h>
#include <tencentcloud/mrs/v20200910/model/GeneralExaminationOthers.h>
#include <tencentcloud/mrs/v20200910/model/GeneralExaminationBriefSummary.h>


namespace TencentCloud
{
    namespace Mrs
    {
        namespace V20200910
        {
            namespace Model
            {
                /**
                * 体检报告-一般检测信息
                */
                class GeneralExaminationBaseItem : public AbstractModel
                {
                public:
                    GeneralExaminationBaseItem();
                    ~GeneralExaminationBaseItem() = default;
                    void ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const;
                    CoreInternalOutcome Deserialize(const rapidjson::Value &value);


                    /**
                     * 获取生命体征
                     * @return VitalSign 生命体征
                     * 
                     */
                    GeneralExaminationVitalSign GetVitalSign() const;

                    /**
                     * 设置生命体征
                     * @param _vitalSign 生命体征
                     * 
                     */
                    void SetVitalSign(const GeneralExaminationVitalSign& _vitalSign);

                    /**
                     * 判断参数 VitalSign 是否已赋值
                     * @return VitalSign 是否已赋值
                     * 
                     */
                    bool VitalSignHasBeenSet() const;

                    /**
                     * 获取其他
                     * @return Others 其他
                     * 
                     */
                    GeneralExaminationOthers GetOthers() const;

                    /**
                     * 设置其他
                     * @param _others 其他
                     * 
                     */
                    void SetOthers(const GeneralExaminationOthers& _others);

                    /**
                     * 判断参数 Others 是否已赋值
                     * @return Others 是否已赋值
                     * 
                     */
                    bool OthersHasBeenSet() const;

                    /**
                     * 获取小结
                     * @return BriefSummary 小结
                     * 
                     */
                    GeneralExaminationBriefSummary GetBriefSummary() const;

                    /**
                     * 设置小结
                     * @param _briefSummary 小结
                     * 
                     */
                    void SetBriefSummary(const GeneralExaminationBriefSummary& _briefSummary);

                    /**
                     * 判断参数 BriefSummary 是否已赋值
                     * @return BriefSummary 是否已赋值
                     * 
                     */
                    bool BriefSummaryHasBeenSet() const;

                private:

                    /**
                     * 生命体征
                     */
                    GeneralExaminationVitalSign m_vitalSign;
                    bool m_vitalSignHasBeenSet;

                    /**
                     * 其他
                     */
                    GeneralExaminationOthers m_others;
                    bool m_othersHasBeenSet;

                    /**
                     * 小结
                     */
                    GeneralExaminationBriefSummary m_briefSummary;
                    bool m_briefSummaryHasBeenSet;

                };
            }
        }
    }
}

#endif // !TENCENTCLOUD_MRS_V20200910_MODEL_GENERALEXAMINATIONBASEITEM_H_
