...
CoreInternalOutcome TextToClassResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TextTypeList") && !rsp["TextTypeList"].IsNull())
    {
        if (!rsp["TextTypeList"].IsArray())
            return CoreInternalOutcome(Error("response `TextTypeList` is not array type"));
...
string TextToClassResponse::ToJsonString() const
{
...
vector<TextType> TextToClassResponse::GetTextTypeList() const
{
    return m_textTypeList;
}

bool TextToClassResponse::TextTypeListHasBeenSet() const
{
