...
CoreInternalOutcome AspectRatio::Deserialize(const Value &value)
{
    string requestId = "";
...
void AspectRatio::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_numberHasBeenSet)
    {
        ...
        string key = "Number";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_number.c_str(), allocator).Move(), allocator);
    }
