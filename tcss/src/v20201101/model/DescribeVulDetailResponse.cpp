...
CoreInternalOutcome DescribeVulDetailResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["VulInfo"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `VulInfo` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_vulInfo.Deserialize(rsp["VulInfo"]);
...
VulDetailInfo DescribeVulDetailResponse::GetVulInfo() const
{
    return m_vulInfo;
}

bool DescribeVulDetailResponse::VulInfoHasBeenSet() const
{
