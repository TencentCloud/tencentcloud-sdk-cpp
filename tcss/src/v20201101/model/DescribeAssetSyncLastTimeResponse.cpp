...
CoreInternalOutcome DescribeAssetSyncLastTimeResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AssetSyncLastTime") && !rsp["AssetSyncLastTime"].IsNull())
    {
        if (!rsp["AssetSyncLastTime"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `AssetSyncLastTime` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_assetSyncLastTime = string(rsp["AssetSyncLastTime"].GetString());
...
string DescribeAssetSyncLastTimeResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_assetSyncLastTimeHasBeenSet)
    {
...
string DescribeAssetSyncLastTimeResponse::GetAssetSyncLastTime() const
{
    return m_assetSyncLastTime;
}
