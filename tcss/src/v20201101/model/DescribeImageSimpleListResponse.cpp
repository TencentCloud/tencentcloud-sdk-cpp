...
CoreInternalOutcome DescribeImageSimpleListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ImageList") && !rsp["ImageList"].IsNull())
    {
        if (!rsp["ImageList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ImageList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ImageList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ImageSimpleInfo item;
...
string DescribeImageSimpleListResponse::ToJsonString() const
{
...
vector<ImageSimpleInfo> DescribeImageSimpleListResponse::GetImageList() const
{
    return m_imageList;
}

bool DescribeImageSimpleListResponse::ImageListHasBeenSet() const
{
    return m_imageListHasBeenSet;
}

uint64_t DescribeImageSimpleListResponse::GetImageCnt() const
{
    return m_imageCnt;
}

bool DescribeImageSimpleListResponse::ImageCntHasBeenSet() const
{
