...
<li> FALSE，不开启该功能，则 AS 等待竞价实例被销毁后才会去扩容补齐伸缩组期望实例数。</li> 
                    ...
                    /**
                     * 获取实例主机名序号相关设置。开启后为伸缩组内自动创建的实例主机名添加递增的数字序号。
                    ...
                    /**
                     * 设置实例主机名序号相关设置。开启后为伸缩组内自动创建的实例主机名添加递增的数字序号。
                    ...
                    /**
                     * 获取匹配期望数并发扩容功能，不能在InstanceAllocationPolicy为竞价混合模式时设置，也不能在ScalingMode为扩容优先开机模式时设置。目前仅支持两个匹配期望数扩容活动并发进行，不支持指定数量扩容、缩容等其他类型活动并发。设置为FALSE表示不开启。
                    ...
                    /**
                     * 设置匹配期望数并发扩容功能，不能在InstanceAllocationPolicy为竞价混合模式时设置，也不能在ScalingMode为扩容优先开机模式时设置。目前仅支持两个匹配期望数扩容活动并发进行，不支持指定数量扩容、缩容等其他类型活动并发。设置为FALSE表示不开启。
