...
CoreInternalOutcome PaiInstance::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
...
void PaiInstance::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_paiMateUrlHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_paiMateUrl.c_str(), allocator).Move(), allocator);
    }
