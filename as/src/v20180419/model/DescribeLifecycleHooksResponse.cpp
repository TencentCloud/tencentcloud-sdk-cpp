...
CoreInternalOutcome DescribeLifecycleHooksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LifecycleHookSet") && !rsp["LifecycleHookSet"].IsNull())
    {
        if (!rsp["LifecycleHookSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LifecycleHookSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["LifecycleHookSet"];
...
string DescribeLifecycleHooksResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_lifecycleHookSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "LifecycleHookSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_lifecycleHookSet.begin(); itr != m_lifecycleHookSet.end(); ++itr, ++i)
        {
