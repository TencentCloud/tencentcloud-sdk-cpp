...
CoreInternalOutcome DescribeNotificationConfigurationsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AutoScalingNotificationSet") && !rsp["AutoScalingNotificationSet"].IsNull())
    {
        if (!rsp["AutoScalingNotificationSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `AutoScalingNotificationSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["AutoScalingNotificationSet"];
...
string DescribeNotificationConfigurationsResponse::ToJsonString() const
{
    ...
    if (m_autoScalingNotificationSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AutoScalingNotificationSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_autoScalingNotificationSet.begin(); itr != m_autoScalingNotificationSet.end(); ++itr, ++i)
        {
