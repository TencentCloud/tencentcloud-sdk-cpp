...
CoreInternalOutcome FNRRequest::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("PhoneNumber") && !value["PhoneNumber"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `FNRRequest.PhoneNumber` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void FNRRequest::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
