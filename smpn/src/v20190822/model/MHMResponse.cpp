...
CoreInternalOutcome MHMResponse::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TagType") && !value["TagType"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MHMResponse.TagType` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("TagCount") && !value["TagCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MHMResponse.TagCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void MHMResponse::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
