...
CoreInternalOutcome StripTaskResult::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `StripTaskResult.Status` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ErrCode") && !value["ErrCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `StripTaskResult.ErrCode` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ErrMsg") && !value["ErrMsg"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `StripTaskResult.ErrMsg` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ItemSet") && !value["ItemSet"].IsNull())
    {
        if (!value["ItemSet"].IsArray())
            return CoreInternalOutcome(Error("response `StripTaskResult.ItemSet` is not array type"));
...
void StripTaskResult::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
