...
CoreInternalOutcome TagTaskResult::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TagTaskResult.Status` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ErrCode") && !value["ErrCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TagTaskResult.ErrCode` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ErrMsg") && !value["ErrMsg"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TagTaskResult.ErrMsg` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ItemSet") && !value["ItemSet"].IsNull())
    {
        if (!value["ItemSet"].IsArray())
            return CoreInternalOutcome(Error("response `TagTaskResult.ItemSet` is not array type"));
...
void TagTaskResult::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
