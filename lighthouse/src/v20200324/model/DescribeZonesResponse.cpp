...
CoreInternalOutcome DescribeZonesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ZoneInfoSet") && !rsp["ZoneInfoSet"].IsNull())
    {
        if (!rsp["ZoneInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ZoneInfoSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ZoneInfoSet"];
...
vector<ZoneInfo> DescribeZonesResponse::GetZoneInfoSet() const
{
    return m_zoneInfoSet;
}

bool DescribeZonesResponse::ZoneInfoSetHasBeenSet() const
{
