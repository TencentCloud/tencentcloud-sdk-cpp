...
CoreInternalOutcome DiskBackup::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tags") && !value["Tags"].IsNull())
    {
        if (!value["Tags"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DiskBackup.Tags` is not array type"));
...
vector<Tag> DiskBackup::GetTags() const
{
    return m_tags;
}

void DiskBackup::SetTags(const vector<Tag>& _tags)
{
    m_tags = _tags;
    m_tagsHasBeenSet = true;
}

bool DiskBackup::TagsHasBeenSet() const
{
