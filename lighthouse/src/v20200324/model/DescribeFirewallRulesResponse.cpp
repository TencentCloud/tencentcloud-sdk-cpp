...
CoreInternalOutcome DescribeFirewallRulesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FirewallVersion") && !rsp["FirewallVersion"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `FirewallVersion` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeFirewallRulesResponse::ToJsonString() const
{
    ...
    if (m_firewallVersionHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_firewallVersion, allocator);
    }
