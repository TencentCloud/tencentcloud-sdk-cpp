...
CoreInternalOutcome CreateInstanceSnapshotResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SnapshotId") && !rsp["SnapshotId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SnapshotId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CreateInstanceSnapshotResponse::ToJsonString() const
{
...
string CreateInstanceSnapshotResponse::GetSnapshotId() const
{
    return m_snapshotId;
}

bool CreateInstanceSnapshotResponse::SnapshotIdHasBeenSet() const
{
