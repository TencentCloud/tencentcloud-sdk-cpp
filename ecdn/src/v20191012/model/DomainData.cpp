...
CoreInternalOutcome DomainData::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Resource") && !value["Resource"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `DomainData.Resource` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("DetailData") && !value["DetailData"].IsNull())
    {
        if (!value["DetailData"].IsArray())
            return CoreInternalOutcome(Error("response `DomainData.DetailData` is not array type"));
...
void DomainData::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
