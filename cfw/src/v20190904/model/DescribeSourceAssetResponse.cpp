...
CoreInternalOutcome DescribeSourceAssetResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ZoneList") && !rsp["ZoneList"].IsNull())
    {
        if (!rsp["ZoneList"].IsArray())
            return CoreInternalOutcome(Error("response `ZoneList` is not array type"));
...
string DescribeSourceAssetResponse::ToJsonString() const
{
...
bool DescribeSourceAssetResponse::DataHasBeenSet() const
{
    return m_dataHasBeenSet;
}

int64_t DescribeSourceAssetResponse::GetTotal() const
{
    return m_total;
}
