...
CoreInternalOutcome CommonFilter::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Name") && !value["Name"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CommonFilter.Name` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Values") && !value["Values"].IsNull())
    {
        if (!value["Values"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CommonFilter.Values` is not array type"));
...
void CommonFilter::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
string CommonFilter::GetName() const
{
    return m_name;
}

void CommonFilter::SetName(const string& _name)
{
    m_name = _name;
    m_nameHasBeenSet = true;
}

bool CommonFilter::NameHasBeenSet() const
{
...
void CommonFilter::SetOperatorType(const int64_t& _operatorType)
{
