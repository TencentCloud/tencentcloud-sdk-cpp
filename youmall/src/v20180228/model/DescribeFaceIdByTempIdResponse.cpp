...
CoreInternalOutcome DescribeFaceIdByTempIdResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TempId") && !rsp["TempId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TempId` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("FaceId") && !rsp["FaceId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `FaceId` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("PersonInfo") && !rsp["PersonInfo"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `PersonInfo` is not object type").SetRequestId(requestId));
        }
