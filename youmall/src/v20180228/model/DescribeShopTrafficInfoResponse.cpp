...
CoreInternalOutcome DescribeShopTrafficInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ShopDayTrafficInfoSet") && !rsp["ShopDayTrafficInfoSet"].IsNull())
    {
        if (!rsp["ShopDayTrafficInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ShopDayTrafficInfoSet` is not array type"));
...
string DescribeShopTrafficInfoResponse::ToJsonString() const
{
    ...
    if (m_shopDayTrafficInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ShopDayTrafficInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_shopDayTrafficInfoSet.begin(); itr != m_shopDayTrafficInfoSet.end(); ++itr, ++i)
        {
