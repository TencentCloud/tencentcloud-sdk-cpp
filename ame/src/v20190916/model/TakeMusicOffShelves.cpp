...
CoreInternalOutcome TakeMusicOffShelves::Deserialize(const Value &value)
{
    string requestId = "";
...
void TakeMusicOffShelves::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_musicIdsHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_musicIds.c_str(), allocator).Move(), allocator);
    }
