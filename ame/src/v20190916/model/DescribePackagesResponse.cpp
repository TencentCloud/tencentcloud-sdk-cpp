...
CoreInternalOutcome DescribePackagesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Packages") && !rsp["Packages"].IsNull())
    {
        if (!rsp["Packages"].IsArray())
            return CoreInternalOutcome(Error("response `Packages` is not array type"));

        const Value &tmpValue = rsp["Packages"];
...
vector<Package> DescribePackagesResponse::GetPackages() const
{
    return m_packages;
}
