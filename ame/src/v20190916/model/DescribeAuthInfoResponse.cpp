...
CoreInternalOutcome DescribeAuthInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AuthInfo") && !rsp["AuthInfo"].IsNull())
    {
        if (!rsp["AuthInfo"].IsArray())
            return CoreInternalOutcome(Error("response `AuthInfo` is not array type"));
...
string DescribeAuthInfoResponse::ToJsonString() const
{
...
vector<AuthInfo> DescribeAuthInfoResponse::GetAuthInfo() const
{
    return m_authInfo;
}

bool DescribeAuthInfoResponse::AuthInfoHasBeenSet() const
{
    return m_authInfoHasBeenSet;
}

int64_t DescribeAuthInfoResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeAuthInfoResponse::TotalCountHasBeenSet() const
{
