...
CoreInternalOutcome Device::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("DeviceType") && !value["DeviceType"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Device.DeviceType` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t Device::GetDeviceType() const
{
    return m_deviceType;
}

void Device::SetDeviceType(const int64_t& _deviceType)
{
