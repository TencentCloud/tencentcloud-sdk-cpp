...
CoreInternalOutcome ParameterDetail::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ParameterDetail.Status` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t ParameterDetail::GetStatus() const
{
    return m_status;
}

void ParameterDetail::SetStatus(const int64_t& _status)
{
    m_status = _status;
    m_statusHasBeenSet = true;
}

bool ParameterDetail::StatusHasBeenSet() const
{
