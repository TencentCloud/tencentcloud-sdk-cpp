...
CoreInternalOutcome DescribeBackupFilesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("BackupFiles") && !rsp["BackupFiles"].IsNull())
    {
        if (!rsp["BackupFiles"].IsArray())
            return CoreInternalOutcome(Core::Error("response `BackupFiles` is not array type"));

        const rapidjson::Value &tmpValue = rsp["BackupFiles"];
...
string DescribeBackupFilesResponse::ToJsonString() const
{
    ...
    if (m_backupFilesHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_backupFiles.begin(); itr != m_backupFiles.end(); ++itr, ++i)
        {
...
vector<BackupFile> DescribeBackupFilesResponse::GetBackupFiles() const
{
    return m_backupFiles;
}

bool DescribeBackupFilesResponse::BackupFilesHasBeenSet() const
{
