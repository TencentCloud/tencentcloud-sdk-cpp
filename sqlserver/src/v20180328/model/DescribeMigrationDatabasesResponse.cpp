...
CoreInternalOutcome DescribeMigrationDatabasesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Amount") && !rsp["Amount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Amount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeMigrationDatabasesResponse::ToJsonString() const
{
    ...
    if (m_migrateDBSetHasBeenSet)
    {
        ...
        for (auto itr = m_migrateDBSet.begin(); itr != m_migrateDBSet.end(); ++itr)
        {
