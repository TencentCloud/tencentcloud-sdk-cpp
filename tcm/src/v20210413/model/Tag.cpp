...
CoreInternalOutcome Tag::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["Passthrough"].IsBool())
        {
            return CoreInternalOutcome(Core::Error("response `Tag.Passthrough` IsBool=false incorrectly").SetRequestId(requestId));
        }
        m_passthrough = value["Passthrough"].GetBool();
...
void Tag::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_passthroughHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_passthrough, allocator);
    }
...
bool Tag::GetPassthrough() const
{
    return m_passthrough;
}

void Tag::SetPassthrough(const bool& _passthrough)
{
    m_passthrough = _passthrough;
    m_passthroughHasBeenSet = true;
}

bool Tag::PassthroughHasBeenSet() const
{
