...
CoreInternalOutcome DescribeWeeklyReportBruteAttacksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("WeeklyReportBruteAttacks") && !rsp["WeeklyReportBruteAttacks"].IsNull())
    {
        if (!rsp["WeeklyReportBruteAttacks"].IsArray())
            return CoreInternalOutcome(Core::Error("response `WeeklyReportBruteAttacks` is not array type"));
...
string DescribeWeeklyReportBruteAttacksResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_weeklyReportBruteAttacksHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "WeeklyReportBruteAttacks";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_weeklyReportBruteAttacks.begin(); itr != m_weeklyReportBruteAttacks.end(); ++itr, ++i)
        {
