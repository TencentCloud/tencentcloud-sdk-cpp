...
CoreInternalOutcome DescribeWeeklyReportVulsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("WeeklyReportVuls") && !rsp["WeeklyReportVuls"].IsNull())
    {
        if (!rsp["WeeklyReportVuls"].IsArray())
            return CoreInternalOutcome(Core::Error("response `WeeklyReportVuls` is not array type"));
...
string DescribeWeeklyReportVulsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_weeklyReportVulsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "WeeklyReportVuls";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_weeklyReportVuls.begin(); itr != m_weeklyReportVuls.end(); ++itr, ++i)
        {
