...
CoreInternalOutcome DescribeWeeklyReportNonlocalLoginPlacesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("WeeklyReportNonlocalLoginPlaces") && !rsp["WeeklyReportNonlocalLoginPlaces"].IsNull())
    {
        if (!rsp["WeeklyReportNonlocalLoginPlaces"].IsArray())
            return CoreInternalOutcome(Core::Error("response `WeeklyReportNonlocalLoginPlaces` is not array type"));

        const rapidjson::Value &tmpValue = rsp["WeeklyReportNonlocalLoginPlaces"];
...
string DescribeWeeklyReportNonlocalLoginPlacesResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_weeklyReportNonlocalLoginPlacesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "WeeklyReportNonlocalLoginPlaces";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_weeklyReportNonlocalLoginPlaces.begin(); itr != m_weeklyReportNonlocalLoginPlaces.end(); ++itr, ++i)
        {
