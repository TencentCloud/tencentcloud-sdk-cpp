...
CoreInternalOutcome MatchKBPURLListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PURLList") && !rsp["PURLList"].IsNull())
    {
        if (!rsp["PURLList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PURLList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["PURLList"];
...
string MatchKBPURLListResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_pURLListHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_pURLList.begin(); itr != m_pURLList.end(); ++itr, ++i)
        {
...
vector<PURL> MatchKBPURLListResponse::GetPURLList() const
{
    return m_pURLList;
}
