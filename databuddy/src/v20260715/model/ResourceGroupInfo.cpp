...
ResourceGroupInfo::ResourceGroupInfo() :
    ...
    m_resourceGroupStatusHasBeenSet(false)
{
}
...
CoreInternalOutcome ResourceGroupInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ResourceGroupStatus") && !value["ResourceGroupStatus"].IsNull())
    {
        if (!value["ResourceGroupStatus"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `ResourceGroupInfo.ResourceGroupStatus` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_resourceGroupStatus = string(value["ResourceGroupStatus"].GetString());
        m_resourceGroupStatusHasBeenSet = true;
    }
...
void ResourceGroupInfo::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_resourceGroupStatusHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ResourceGroupStatus";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_resourceGroupStatus.c_str(), allocator).Move(), allocator);
    }
...
string ResourceGroupInfo::GetResourceGroupStatus() const
{
    return m_resourceGroupStatus;
}

void ResourceGroupInfo::SetResourceGroupStatus(const string& _resourceGroupStatus)
{
    m_resourceGroupStatus = _resourceGroupStatus;
    m_resourceGroupStatusHasBeenSet = true;
}

bool ResourceGroupInfo::ResourceGroupStatusHasBeenSet() const
{
    return m_resourceGroupStatusHasBeenSet;
}
