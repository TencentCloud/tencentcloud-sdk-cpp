...
CoreInternalOutcome DescribeUsgRuleResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SgRules") && !rsp["SgRules"].IsNull())
    {
        if (!rsp["SgRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SgRules` is not array type"));
...
string DescribeUsgRuleResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_sgRulesHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_sgRules.begin(); itr != m_sgRules.end(); ++itr, ++i)
        {
