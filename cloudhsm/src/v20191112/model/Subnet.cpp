...
CoreInternalOutcome Subnet::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("AvailableIpAddressCount") && !value["AvailableIpAddressCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Subnet.AvailableIpAddressCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("TotalIpAddressCount") && !value["TotalIpAddressCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Subnet.TotalIpAddressCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void Subnet::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
