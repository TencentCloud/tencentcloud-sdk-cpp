...
CoreInternalOutcome DescribeUserBaseInfoInstanceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("UserUin") && !rsp["UserUin"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UserUin` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("UserAppid") && !rsp["UserAppid"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UserAppid` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("TimeStamp") && !rsp["TimeStamp"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TimeStamp` IsUint64=false incorrectly").SetRequestId(requestId));
        }
