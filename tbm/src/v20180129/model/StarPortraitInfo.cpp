...
CoreInternalOutcome StarPortraitInfo::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("PortraitSet") && !value["PortraitSet"].IsNull())
    {
        if (!value["PortraitSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `StarPortraitInfo.PortraitSet` is not array type"));
...
void StarPortraitInfo::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<StarPortrait> StarPortraitInfo::GetPortraitSet() const
{
    return m_portraitSet;
}

void StarPortraitInfo::SetPortraitSet(const vector<StarPortrait>& _portraitSet)
{
    m_portraitSet = _portraitSet;
    m_portraitSetHasBeenSet = true;
}

bool StarPortraitInfo::PortraitSetHasBeenSet() const
{
