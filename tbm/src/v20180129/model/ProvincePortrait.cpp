...
CoreInternalOutcome ProvincePortrait::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Percent") && !value["Percent"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ProvincePortrait.Percent` IsDouble=false incorrectly").SetRequestId(requestId));
        }
...
void ProvincePortrait::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
