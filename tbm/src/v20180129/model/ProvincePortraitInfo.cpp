...
CoreInternalOutcome ProvincePortraitInfo::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("PortraitSet") && !value["PortraitSet"].IsNull())
    {
        if (!value["PortraitSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ProvincePortraitInfo.PortraitSet` is not array type"));

        const rapidjson::Value &tmpValue = value["PortraitSet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ProvincePortrait item;
...
void ProvincePortraitInfo::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<ProvincePortrait> ProvincePortraitInfo::GetPortraitSet() const
{
    return m_portraitSet;
}

void ProvincePortraitInfo::SetPortraitSet(const vector<ProvincePortrait>& _portraitSet)
{
    m_portraitSet = _portraitSet;
    m_portraitSetHasBeenSet = true;
}

bool ProvincePortraitInfo::PortraitSetHasBeenSet() const
{
