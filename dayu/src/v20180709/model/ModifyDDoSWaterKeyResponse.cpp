...
CoreInternalOutcome ModifyDDoSWaterKeyResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("KeyList") && !rsp["KeyList"].IsNull())
    {
        if (!rsp["KeyList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `KeyList` is not array type"));
...
string ModifyDDoSWaterKeyResponse::ToJsonString() const
{
...
vector<WaterPrintKey> ModifyDDoSWaterKeyResponse::GetKeyList() const
{
    return m_keyList;
}

bool ModifyDDoSWaterKeyResponse::KeyListHasBeenSet() const
{
