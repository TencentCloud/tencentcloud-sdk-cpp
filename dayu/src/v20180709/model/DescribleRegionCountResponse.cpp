...
CoreInternalOutcome DescribleRegionCountResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RegionList") && !rsp["RegionList"].IsNull())
    {
        if (!rsp["RegionList"].IsArray())
            return CoreInternalOutcome(Error("response `RegionList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["RegionList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            RegionInstanceCount item;
...
string DescribleRegionCountResponse::ToJsonString() const
{
...
vector<RegionInstanceCount> DescribleRegionCountResponse::GetRegionList() const
{
    return m_regionList;
}

bool DescribleRegionCountResponse::RegionListHasBeenSet() const
{
