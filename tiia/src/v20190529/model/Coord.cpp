...
CoreInternalOutcome Coord::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("X") && !value["X"].IsNull())
    {
        if (!value["X"].IsInt64())
        {
            return CoreInternalOutcome(Error("response `Coord.X` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Y") && !value["Y"].IsNull())
    {
        if (!value["Y"].IsInt64())
        {
            return CoreInternalOutcome(Error("response `Coord.Y` IsInt64=false incorrectly").SetRequestId(requestId));
        }
