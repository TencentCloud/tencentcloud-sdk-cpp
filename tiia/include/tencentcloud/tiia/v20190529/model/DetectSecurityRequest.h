...
#ifndef TENCENTCLOUD_TIIA_V20190529_MODEL_DETECTSECURITYREQUEST_H_
#define TENCENTCLOUD_TIIA_V20190529_MODEL_DETECTSECURITYREQUEST_H_
    ...
    namespace Tiia
    {
        namespace V20190529
        {
            namespace Model
            {
                ...
                */
                class DetectSecurityRequest : public AbstractModel
                {
                public:
                    DetectSecurityRequest();
                    ~DetectSecurityRequest() = default;
...
支持的图片大小：所下载图片经Base64编码后不超过5M。
                     * @return ImageBase64 图片经过base64编码的内容。
最大不超过4M。与ImageUrl同时存在时优先使用ImageUrl字段。
注意：图片需要base64编码，并且要去掉编码头部。
支持的图片格式：PNG、JPG、JPEG、暂不支持GIF格式。
支持的图片大小：所下载图片经Base64编码后不超过5M。
...
#endif // !TENCENTCLOUD_TIIA_V20190529_MODEL_DETECTSECURITYREQUEST_H_
