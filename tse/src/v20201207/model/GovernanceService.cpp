...
CoreInternalOutcome GovernanceService::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExtendedMetadata") && !value["ExtendedMetadata"].IsNull())
    {
        if (!value["ExtendedMetadata"].IsArray())
            return CoreInternalOutcome(Core::Error("response `GovernanceService.ExtendedMetadata` is not array type"));
...
vector<ExtendedMetadata> GovernanceService::GetExtendedMetadata() const
{
    return m_extendedMetadata;
}

void GovernanceService::SetExtendedMetadata(const vector<ExtendedMetadata>& _extendedMetadata)
{
    m_extendedMetadata = _extendedMetadata;
    m_extendedMetadataHasBeenSet = true;
}

bool GovernanceService::ExtendedMetadataHasBeenSet() const
{
