...
CoreInternalOutcome ApplyUserCertResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CertId") && !rsp["CertId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CertId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("CertDn") && !rsp["CertDn"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CertDn` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ApplyUserCertResponse::ToJsonString() const
{
    ...
    if (m_certDnHasBeenSet)
    {
        ...
        string key = "CertDn";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_certDn.c_str(), allocator).Move(), allocator);
    }
