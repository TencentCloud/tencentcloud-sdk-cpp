...
CoreInternalOutcome PeerSet::Deserialize(const Value &value)
{
    string requestId = "";
...
void PeerSet::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_orgNameHasBeenSet)
    {
        ...
        string key = "OrgName";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_orgName.c_str(), allocator).Move(), allocator);
    }
