...
CoreInternalOutcome GroupDetailForUser::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("GroupName") && !value["GroupName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `GroupDetailForUser.GroupName` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("GroupMSPId") && !value["GroupMSPId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `GroupDetailForUser.GroupMSPId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void GroupDetailForUser::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_groupMSPIdHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_groupMSPId.c_str(), allocator).Move(), allocator);
    }
