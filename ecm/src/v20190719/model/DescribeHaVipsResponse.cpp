...
CoreInternalOutcome DescribeHaVipsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("HaVipSet") && !rsp["HaVipSet"].IsNull())
    {
        if (!rsp["HaVipSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `HaVipSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["HaVipSet"];
...
string DescribeHaVipsResponse::ToJsonString() const
{
    ...
    if (m_haVipSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_haVipSet.begin(); itr != m_haVipSet.end(); ++itr, ++i)
        {
