...
CoreInternalOutcome DescribeModuleDetailResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Module") && !rsp["Module"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Module` is not object type").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("ModuleCounter") && !rsp["ModuleCounter"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ModuleCounter` is not object type").SetRequestId(requestId));
        }
...
string DescribeModuleDetailResponse::ToJsonString() const
{
    ...
    if (m_moduleCounterHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ModuleCounter";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_moduleCounter.ToJsonObject(value[key.c_str()], allocator);
    }
