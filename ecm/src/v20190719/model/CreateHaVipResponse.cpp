...
CoreInternalOutcome CreateHaVipResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("HaVip") && !rsp["HaVip"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `HaVip` is not object type").SetRequestId(requestId));
        }
...
string CreateHaVipResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_haVipHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "HaVip";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_haVip.ToJsonObject(value[key.c_str()], allocator);
    }
