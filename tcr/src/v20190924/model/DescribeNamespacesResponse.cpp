...
CoreInternalOutcome DescribeNamespacesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NamespaceList") && !rsp["NamespaceList"].IsNull())
    {
        if (!rsp["NamespaceList"].IsArray())
            return CoreInternalOutcome(Error("response `NamespaceList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["NamespaceList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            TcrNamespaceInfo item;
...
string DescribeNamespacesResponse::ToJsonString() const
{
...
vector<TcrNamespaceInfo> DescribeNamespacesResponse::GetNamespaceList() const
{
    return m_namespaceList;
}

bool DescribeNamespacesResponse::NamespaceListHasBeenSet() const
{
    return m_namespaceListHasBeenSet;
}

int64_t DescribeNamespacesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeNamespacesResponse::TotalCountHasBeenSet() const
{
