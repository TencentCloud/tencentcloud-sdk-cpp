...
CoreInternalOutcome TriggerInvokeCondition::Deserialize(const Value &value)
{
    string requestId = "";
...
void TriggerInvokeCondition::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_invokeMethodHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_invokeMethod.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_invokeExprHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_invokeExpr.c_str(), allocator).Move(), allocator);
    }
