...
CoreInternalOutcome DescribeRepositoriesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RepositoryList") && !rsp["RepositoryList"].IsNull())
    {
        if (!rsp["RepositoryList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RepositoryList` is not array type"));
...
string DescribeRepositoriesResponse::ToJsonString() const
{
    ...
    if (m_repositoryListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "RepositoryList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_repositoryList.begin(); itr != m_repositoryList.end(); ++itr, ++i)
        {
