...
CoreInternalOutcome DescribeInstancesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Registries") && !rsp["Registries"].IsNull())
    {
        if (!rsp["Registries"].IsArray())
            return CoreInternalOutcome(Error("response `Registries` is not array type"));

        const Value &tmpValue = rsp["Registries"];
...
vector<Registry> DescribeInstancesResponse::GetRegistries() const
{
    return m_registries;
}

bool DescribeInstancesResponse::RegistriesHasBeenSet() const
{
