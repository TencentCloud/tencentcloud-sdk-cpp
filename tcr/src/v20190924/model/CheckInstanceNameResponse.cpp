...
CoreInternalOutcome CheckInstanceNameResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("IsValidated") && !rsp["IsValidated"].IsNull())
    {
        if (!rsp["IsValidated"].IsBool())
        {
            return CoreInternalOutcome(Core::Error("response `IsValidated` IsBool=false incorrectly").SetRequestId(requestId));
        }
        m_isValidated = rsp["IsValidated"].GetBool();
...
string CheckInstanceNameResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_isValidatedHasBeenSet)
    {
...
bool CheckInstanceNameResponse::GetIsValidated() const
{
