...
CoreInternalOutcome DescribeImageManifestsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Manifest") && !rsp["Manifest"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Manifest` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("Config") && !rsp["Config"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Config` IsString=false incorrectly").SetRequestId(requestId));
        }
