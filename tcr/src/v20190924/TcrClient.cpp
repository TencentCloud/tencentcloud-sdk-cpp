...
TcrClient::ModifyReplicationOutcome TcrClient::ModifyReplication(const ModifyReplicationRequest &request)
{
    auto outcome = MakeRequest(request, "ModifyReplication");
    if (outcome.IsSuccess())
    {
        auto r = outcome.GetResult();
        string payload = string(r.Body(), r.BodySize());
        ModifyReplicationResponse rsp = ModifyReplicationResponse();
        auto o = rsp.Deserialize(payload);
        if (o.IsSuccess())
            return ModifyReplicationOutcome(rsp);
        else
            return ModifyReplicationOutcome(o.GetError());
    }
    else
    {
        return ModifyReplicationOutcome(outcome.GetError());
    }
}

void TcrClient::ModifyReplicationAsync(const ModifyReplicationRequest& request, const ModifyReplicationAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context)
{
    using Req = const ModifyReplicationRequest&;
    using Resp = ModifyReplicationResponse;

    DoRequestAsync<Req, Resp>(
        "ModifyReplication", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
TcrClient::ModifyReplicationOutcomeCallable TcrClient::ModifyReplicationCallable(const ModifyReplicationRequest &request)
{
    const auto prom = std::make_shared<std::promise<ModifyReplicationOutcome>>();
    ModifyReplicationAsync(
    request,
    [prom](
        const TcrClient*,
        const ModifyReplicationRequest&,
        ModifyReplicationOutcome resp,
        const std::shared_ptr<const AsyncCallerContext>&
    )
    {
        prom->set_value(resp);
