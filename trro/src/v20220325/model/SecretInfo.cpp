...
CoreInternalOutcome SecretInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("SecretId") && !value["SecretId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SecretInfo.SecretId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string SecretInfo::GetSecretId() const
{
    return m_secretId;
}

void SecretInfo::SetSecretId(const string& _secretId)
{
    m_secretId = _secretId;
    m_secretIdHasBeenSet = true;
}

bool SecretInfo::SecretIdHasBeenSet() const
{
