...
CoreInternalOutcome CloudStorage::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Vendor") && !value["Vendor"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CloudStorage.Vendor` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t CloudStorage::GetVendor() const
{
    return m_vendor;
}

void CloudStorage::SetVendor(const int64_t& _vendor)
{
    m_vendor = _vendor;
    m_vendorHasBeenSet = true;
}
