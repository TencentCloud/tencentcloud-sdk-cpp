...
CoreInternalOutcome CallbackInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Secret") && !value["Secret"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CallbackInfo.Secret` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CallbackInfo::GetSecret() const
{
    return m_secret;
}

void CallbackInfo::SetSecret(const string& _secret)
{
    m_secret = _secret;
    m_secretHasBeenSet = true;
}

bool CallbackInfo::SecretHasBeenSet() const
{
