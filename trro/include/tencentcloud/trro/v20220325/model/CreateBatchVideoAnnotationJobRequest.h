...
#ifndef TENCENTCLOUD_TRRO_V20220325_MODEL_CREATEBATCHVIDEOANNOTATIONJOBREQUEST_H_
#define TENCENTCLOUD_TRRO_V20220325_MODEL_CREATEBATCHVIDEOANNOTATIONJOBREQUEST_H_
    ...
    namespace Trro
    {
        namespace V20220325
        {
            namespace Model
            {
                ...
                */
                class CreateBatchVideoAnnotationJobRequest : public AbstractModel
                {
                public:
                    CreateBatchVideoAnnotationJobRequest();
                    ~CreateBatchVideoAnnotationJobRequest() = default;
                     ...
                     * 
                     */
                    BatchS3SourceInfo GetInputStorage() const;
                     ...
                     * 
                     */
                    void SetInputStorage(const BatchS3SourceInfo& _inputStorage);

                    /**
                     * 判断参数 InputStorage 是否已赋值
                     * @return InputStorage 是否已赋值
                     * 
                     */
                    bool InputStorageHasBeenSet() const;
                    ...
                    /**
                     * 设置<p>回调信息，配置后当任务下子处理项状态从处理中变为其他状态时，服务端会向回调地址发送请求（退避重试三次，不保证回调一定送达，需保证目标地址接收服务有效），建议接收方做好幂等处理。回调请求格式如下：<br><strong>请求头</strong></p><table><thead><tr><th>名称</th><th>值</th></tr></thead><tbody><tr><td>X-Annotation-Signature</td><td>hex(HMAC-SHA256(请求体原始字节, CallbackInfo.Secret))</td></tr></tbody></table><p><strong>请求体</strong>（application/json）</p><table><thead><tr><th>参数名</th><th>类型</th><th>必选</th><th>描述</th></tr></thead><tbody><tr><td>JobId</td><td>string</td><td>是</td><td>任务 ID</td></tr><tr><td>TaskId</td><td>string</td><td>是</td><td>处理项 ID</td></tr><tr><td>FileName</td><td>string</td><td>是</td><td>视频文件名</td></tr><tr><td>Status</td><td>int</td><td>是</td><td>触发本次回调的处理项状态：3 超时，4 异常，5 待确认，6 成功</td></tr><tr><td>StatusChangedAt</td><td>int</td><td>是</td><td>状态变更时间，Unix 时间戳（秒）</td></tr><tr><td>RawResult</td><td>string</td><td>否</td><td>当前生效的结果 JSON 原文：成功=标注产物；待确认=原始标注；确认后=确认版内容。超时/异常无内容</td></tr></tbody></table>
                ...
                private:
                     ...
                     * <p>批量输入源信息（目录前缀）</p>
                     */
                    BatchS3SourceInfo m_inputStorage;
                    bool m_inputStorageHasBeenSet;
...
#endif // !TENCENTCLOUD_TRRO_V20220325_MODEL_CREATEBATCHVIDEOANNOTATIONJOBREQUEST_H_
