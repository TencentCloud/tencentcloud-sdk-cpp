...
DnspodClient::DescribeDomainLogFilterFileOutcome DnspodClient::DescribeDomainLogFilterFile(const DescribeDomainLogFilterFileRequest &request)
{
    auto outcome = MakeRequest(request, "DescribeDomainLogFilterFile");
    if (outcome.IsSuccess())
    {
        auto r = outcome.GetResult();
        string payload = string(r.Body(), r.BodySize());
        DescribeDomainLogFilterFileResponse rsp = DescribeDomainLogFilterFileResponse();
        auto o = rsp.Deserialize(payload);
        if (o.IsSuccess())
            return DescribeDomainLogFilterFileOutcome(rsp);
        else
            return DescribeDomainLogFilterFileOutcome(o.GetError());
    }
    else
    {
        return DescribeDomainLogFilterFileOutcome(outcome.GetError());
    }
}

void DnspodClient::DescribeDomainLogFilterFileAsync(const DescribeDomainLogFilterFileRequest& request, const DescribeDomainLogFilterFileAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context)
{
    using Req = const DescribeDomainLogFilterFileRequest&;
    using Resp = DescribeDomainLogFilterFileResponse;

    DoRequestAsync<Req, Resp>(
        "DescribeDomainLogFilterFile", request, {{{"Content-Type", "application/json"}}},
        [this, context, handler](Req req, Outcome<Core::Error, Resp> resp)
        {
            handler(this, req, std::move(resp), context);
        });
...
DnspodClient::DescribeDomainLogFilterFileOutcomeCallable DnspodClient::DescribeDomainLogFilterFileCallable(const DescribeDomainLogFilterFileRequest &request)
{
    const auto prom = std::make_shared<std::promise<DescribeDomainLogFilterFileOutcome>>();
    DescribeDomainLogFilterFileAsync(
    request,
    [prom](
        const DnspodClient*,
        const DescribeDomainLogFilterFileRequest&,
        DescribeDomainLogFilterFileOutcome resp,
        const std::shared_ptr<const AsyncCallerContext>&
    )
    {
        prom->set_value(resp);
