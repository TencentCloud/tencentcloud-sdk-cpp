...
CoreInternalOutcome DescribeRecordResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["RecordInfo"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `RecordInfo` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_recordInfo.Deserialize(rsp["RecordInfo"]);
...
string DescribeRecordResponse::ToJsonString() const
{
    ...
    if (m_recordInfoHasBeenSet)
    {
        ...
        m_recordInfo.ToJsonObject(value[key.c_str()], allocator);
    }
...
RecordInfo DescribeRecordResponse::GetRecordInfo() const
{
    return m_recordInfo;
}
