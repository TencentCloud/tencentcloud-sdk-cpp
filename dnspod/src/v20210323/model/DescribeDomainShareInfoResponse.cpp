...
CoreInternalOutcome DescribeDomainShareInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ShareList") && !rsp["ShareList"].IsNull())
    {
        if (!rsp["ShareList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ShareList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ShareList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DomainShareInfo item;
...
string DescribeDomainShareInfoResponse::ToJsonString() const
{
...
vector<DomainShareInfo> DescribeDomainShareInfoResponse::GetShareList() const
{
    return m_shareList;
}

bool DescribeDomainShareInfoResponse::ShareListHasBeenSet() const
{
    return m_shareListHasBeenSet;
}

string DescribeDomainShareInfoResponse::GetOwner() const
{
    return m_owner;
}

bool DescribeDomainShareInfoResponse::OwnerHasBeenSet() const
{
