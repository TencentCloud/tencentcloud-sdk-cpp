...
CoreInternalOutcome KVPair::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("K") && !value["K"].IsNull())
    {
        if (!value["K"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `KVPair.K` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("V") && !value["V"].IsNull())
    {
        if (!value["V"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `KVPair.V` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string KVPair::GetK() const
{
    return m_k;
}

void KVPair::SetK(const string& _k)
{
    m_k = _k;
    m_kHasBeenSet = true;
}

bool KVPair::KHasBeenSet() const
{
    return m_kHasBeenSet;
}

string KVPair::GetV() const
{
    return m_v;
}

void KVPair::SetV(const string& _v)
{
    m_v = _v;
    m_vHasBeenSet = true;
}

bool KVPair::VHasBeenSet() const
{
