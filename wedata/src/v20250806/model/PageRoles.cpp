...
#include <tencentcloud/wedata/v20250806/model/PageRoles.h>
...
CoreInternalOutcome PageRoles::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Items") && !value["Items"].IsNull())
    {
        if (!value["Items"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PageRoles.Items` is not array type"));
...
vector<SystemRole> PageRoles::GetItems() const
{
    return m_items;
}

void PageRoles::SetItems(const vector<SystemRole>& _items)
{
    m_items = _items;
    m_itemsHasBeenSet = true;
}

bool PageRoles::ItemsHasBeenSet() const
{
