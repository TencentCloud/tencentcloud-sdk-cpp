...
CoreInternalOutcome DeleteTaskResult::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Status") && !value["Status"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DeleteTaskResult.Status` IsBool=false incorrectly").SetRequestId(requestId));
        }
...
bool DeleteTaskResult::GetStatus() const
{
    return m_status;
}

void DeleteTaskResult::SetStatus(const bool& _status)
{
    m_status = _status;
    m_statusHasBeenSet = true;
}
