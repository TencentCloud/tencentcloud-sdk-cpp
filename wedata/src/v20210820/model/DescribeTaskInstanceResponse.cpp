...
CoreInternalOutcome DescribeTaskInstanceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskInstanceDetail") && !rsp["TaskInstanceDetail"].IsNull())
    {
        if (!rsp["TaskInstanceDetail"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `TaskInstanceDetail` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_taskInstanceDetail.Deserialize(rsp["TaskInstanceDetail"]);
...
string DescribeTaskInstanceResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_taskInstanceDetailHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TaskInstanceDetail";
...
TaskInstanceDetail DescribeTaskInstanceResponse::GetTaskInstanceDetail() const
{
    return m_taskInstanceDetail;
}
