...
CoreInternalOutcome NotificationFatigue::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("NotifyCount") && !value["NotifyCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `NotificationFatigue.NotifyCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    {
        if (!value["NotifyInterval"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `NotificationFatigue.NotifyInterval` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_notifyInterval = value["NotifyInterval"].GetInt64();
...
int64_t NotificationFatigue::GetNotifyCount() const
{
    return m_notifyCount;
}

void NotificationFatigue::SetNotifyCount(const int64_t& _notifyCount)
{
...
int64_t NotificationFatigue::GetNotifyInterval() const
{
    return m_notifyInterval;
}

void NotificationFatigue::SetNotifyInterval(const int64_t& _notifyInterval)
{
