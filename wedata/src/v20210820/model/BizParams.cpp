...
BizParams::BizParams() :
    ...
    m_bizStandCodeHasBeenSet(false)
{
}
...
CoreInternalOutcome BizParams::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("BizStandCode") && !value["BizStandCode"].IsNull())
    {
        if (!value["BizStandCode"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `BizParams.BizStandCode` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_bizStandCode = string(value["BizStandCode"].GetString());
        m_bizStandCodeHasBeenSet = true;
    }
...
void BizParams::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_bizStandCodeHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "BizStandCode";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_bizStandCode.c_str(), allocator).Move(), allocator);
    }
...
string BizParams::GetBizStandCode() const
{
    return m_bizStandCode;
}

void BizParams::SetBizStandCode(const string& _bizStandCode)
{
    m_bizStandCode = _bizStandCode;
    m_bizStandCodeHasBeenSet = true;
}

bool BizParams::BizStandCodeHasBeenSet() const
{
    return m_bizStandCodeHasBeenSet;
}
