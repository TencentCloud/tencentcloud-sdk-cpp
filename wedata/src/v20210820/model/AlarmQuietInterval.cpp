...
CoreInternalOutcome AlarmQuietInterval::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("DaysOfWeek") && !value["DaysOfWeek"].IsNull())
    {
        ...
        {
            m_daysOfWeek.push_back((*itr).GetInt64());
        }
...
vector<int64_t> AlarmQuietInterval::GetDaysOfWeek() const
{
    return m_daysOfWeek;
}

void AlarmQuietInterval::SetDaysOfWeek(const vector<int64_t>& _daysOfWeek)
{
