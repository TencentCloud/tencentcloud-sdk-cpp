...
CoreInternalOutcome CanvasInfo::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TasksList") && !value["TasksList"].IsNull())
    {
        if (!value["TasksList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CanvasInfo.TasksList` is not array type"));
    ...
    if (value.HasMember("LinksList") && !value["LinksList"].IsNull())
    {
        if (!value["LinksList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CanvasInfo.LinksList` is not array type"));
...
void CanvasInfo::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<TaskCanvasInfo> CanvasInfo::GetTasksList() const
{
    return m_tasksList;
}

void CanvasInfo::SetTasksList(const vector<TaskCanvasInfo>& _tasksList)
{
    m_tasksList = _tasksList;
    m_tasksListHasBeenSet = true;
}

bool CanvasInfo::TasksListHasBeenSet() const
{
    return m_tasksListHasBeenSet;
}

vector<TaskLinkInfo> CanvasInfo::GetLinksList() const
{
    return m_linksList;
}

void CanvasInfo::SetLinksList(const vector<TaskLinkInfo>& _linksList)
{
    m_linksList = _linksList;
    m_linksListHasBeenSet = true;
}

bool CanvasInfo::LinksListHasBeenSet() const
{
