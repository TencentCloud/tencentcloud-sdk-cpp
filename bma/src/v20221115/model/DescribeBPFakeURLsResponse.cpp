...
CoreInternalOutcome DescribeBPFakeURLsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FakeURLs") && !rsp["FakeURLs"].IsNull())
    {
        if (!rsp["FakeURLs"].IsArray())
            return CoreInternalOutcome(Core::Error("response `FakeURLs` is not array type"));

        const rapidjson::Value &tmpValue = rsp["FakeURLs"];
...
string DescribeBPFakeURLsResponse::ToJsonString() const
{
    ...
    if (m_fakeURLsHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_fakeURLs.begin(); itr != m_fakeURLs.end(); ++itr, ++i)
        {
...
vector<FakeURLData> DescribeBPFakeURLsResponse::GetFakeURLs() const
{
    return m_fakeURLs;
}

bool DescribeBPFakeURLsResponse::FakeURLsHasBeenSet() const
{
