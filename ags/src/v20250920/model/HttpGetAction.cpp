...
CoreInternalOutcome HttpGetAction::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Scheme") && !value["Scheme"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `HttpGetAction.Scheme` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string HttpGetAction::GetScheme() const
{
    return m_scheme;
}

void HttpGetAction::SetScheme(const string& _scheme)
{
    m_scheme = _scheme;
    m_schemeHasBeenSet = true;
}

bool HttpGetAction::SchemeHasBeenSet() const
{
