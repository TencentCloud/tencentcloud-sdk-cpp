...
CoreInternalOutcome EipRsMap::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("EipId") && !value["EipId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `EipRsMap.EipId` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("InstanceId") && !value["InstanceId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `EipRsMap.InstanceId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void EipRsMap::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_eipIdHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_eipId.c_str(), allocator).Move(), allocator);
    }
