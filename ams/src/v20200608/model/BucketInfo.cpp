...
CoreInternalOutcome BucketInfo::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Bucket") && !value["Bucket"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `BucketInfo.Bucket` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Region") && !value["Region"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `BucketInfo.Region` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Object") && !value["Object"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `BucketInfo.Object` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void BucketInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_objectHasBeenSet)
    {
        ...
        string key = "Object";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_object.c_str(), allocator).Move(), allocator);
    }
