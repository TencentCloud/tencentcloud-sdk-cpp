...
CoreInternalOutcome CreateAddressTemplateResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AddressTemplate") && !rsp["AddressTemplate"].IsNull())
    {
        if (!rsp["AddressTemplate"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `AddressTemplate` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_addressTemplate.Deserialize(rsp["AddressTemplate"]);
...
AddressTemplate CreateAddressTemplateResponse::GetAddressTemplate() const
{
    return m_addressTemplate;
}

bool CreateAddressTemplateResponse::AddressTemplateHasBeenSet() const
{
