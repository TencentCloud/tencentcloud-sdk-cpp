...
CoreInternalOutcome CreateVpnGatewayRoutesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Routes") && !rsp["Routes"].IsNull())
    {
        if (!rsp["Routes"].IsArray())
            return CoreInternalOutcome(Error("response `Routes` is not array type"));
...
string CreateVpnGatewayRoutesResponse::ToJsonString() const
{
...
vector<VpnGatewayRoute> CreateVpnGatewayRoutesResponse::GetRoutes() const
{
    return m_routes;
}

bool CreateVpnGatewayRoutesResponse::RoutesHasBeenSet() const
{
