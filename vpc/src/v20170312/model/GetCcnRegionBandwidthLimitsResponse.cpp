...
CoreInternalOutcome GetCcnRegionBandwidthLimitsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CcnBandwidthSet") && !rsp["CcnBandwidthSet"].IsNull())
    {
        if (!rsp["CcnBandwidthSet"].IsArray())
            return CoreInternalOutcome(Error("response `CcnBandwidthSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["CcnBandwidthSet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            CcnBandwidthInfo item;
...
string GetCcnRegionBandwidthLimitsResponse::ToJsonString() const
{
...
vector<CcnBandwidthInfo> GetCcnRegionBandwidthLimitsResponse::GetCcnBandwidthSet() const
{
    return m_ccnBandwidthSet;
}

bool GetCcnRegionBandwidthLimitsResponse::CcnBandwidthSetHasBeenSet() const
{
    return m_ccnBandwidthSetHasBeenSet;
}

uint64_t GetCcnRegionBandwidthLimitsResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool GetCcnRegionBandwidthLimitsResponse::TotalCountHasBeenSet() const
{
