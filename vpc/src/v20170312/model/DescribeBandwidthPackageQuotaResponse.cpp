...
CoreInternalOutcome DescribeBandwidthPackageQuotaResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("QuotaSet") && !rsp["QuotaSet"].IsNull())
    {
        if (!rsp["QuotaSet"].IsArray())
            return CoreInternalOutcome(Error("response `QuotaSet` is not array type"));
...
string DescribeBandwidthPackageQuotaResponse::ToJsonString() const
{
...
vector<Quota> DescribeBandwidthPackageQuotaResponse::GetQuotaSet() const
{
    return m_quotaSet;
}

bool DescribeBandwidthPackageQuotaResponse::QuotaSetHasBeenSet() const
{
