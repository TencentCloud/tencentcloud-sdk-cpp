...
CoreInternalOutcome SecurityGroupPolicySet::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Version") && !value["Version"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SecurityGroupPolicySet.Version` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Egress") && !value["Egress"].IsNull())
    {
        if (!value["Egress"].IsArray())
            return CoreInternalOutcome(Error("response `SecurityGroupPolicySet.Egress` is not array type"));
    ...
    if (value.HasMember("Ingress") && !value["Ingress"].IsNull())
    {
        if (!value["Ingress"].IsArray())
            return CoreInternalOutcome(Error("response `SecurityGroupPolicySet.Ingress` is not array type"));
...
void SecurityGroupPolicySet::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
