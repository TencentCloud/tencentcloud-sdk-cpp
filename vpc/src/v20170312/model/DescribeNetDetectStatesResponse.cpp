...
CoreInternalOutcome DescribeNetDetectStatesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NetDetectStateSet") && !rsp["NetDetectStateSet"].IsNull())
    {
        if (!rsp["NetDetectStateSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `NetDetectStateSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["NetDetectStateSet"];
...
string DescribeNetDetectStatesResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_netDetectStateSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "NetDetectStateSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_netDetectStateSet.begin(); itr != m_netDetectStateSet.end(); ++itr, ++i)
        {
