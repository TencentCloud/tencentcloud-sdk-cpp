...
CoreInternalOutcome ReplaceRoutesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("OldRouteSet") && !rsp["OldRouteSet"].IsNull())
    {
        if (!rsp["OldRouteSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `OldRouteSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["OldRouteSet"];
    ...
    if (rsp.HasMember("NewRouteSet") && !rsp["NewRouteSet"].IsNull())
    {
        if (!rsp["NewRouteSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `NewRouteSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["NewRouteSet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            Route item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
...
string ReplaceRoutesResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_oldRouteSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "OldRouteSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_oldRouteSet.begin(); itr != m_oldRouteSet.end(); ++itr, ++i)
        {
    ...
    if (m_newRouteSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "NewRouteSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_newRouteSet.begin(); itr != m_newRouteSet.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
