...
CoreInternalOutcome InquirePriceCreateDirectConnectGatewayResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TotalCost") && !rsp["TotalCost"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TotalCost` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("RealTotalCost") && !rsp["RealTotalCost"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `RealTotalCost` IsInt64=false incorrectly").SetRequestId(requestId));
        }
