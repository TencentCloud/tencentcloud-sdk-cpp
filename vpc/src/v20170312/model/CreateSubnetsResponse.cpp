...
CoreInternalOutcome CreateSubnetsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SubnetSet") && !rsp["SubnetSet"].IsNull())
    {
        if (!rsp["SubnetSet"].IsArray())
            return CoreInternalOutcome(Error("response `SubnetSet` is not array type"));
...
string CreateSubnetsResponse::ToJsonString() const
{
...
vector<Subnet> CreateSubnetsResponse::GetSubnetSet() const
{
    return m_subnetSet;
}

bool CreateSubnetsResponse::SubnetSetHasBeenSet() const
{
