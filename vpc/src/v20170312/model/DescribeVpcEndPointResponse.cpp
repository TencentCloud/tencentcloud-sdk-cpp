...
CoreInternalOutcome DescribeVpcEndPointResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EndPointSet") && !rsp["EndPointSet"].IsNull())
    {
        if (!rsp["EndPointSet"].IsArray())
            return CoreInternalOutcome(Error("response `EndPointSet` is not array type"));
...
string DescribeVpcEndPointResponse::ToJsonString() const
{
...
vector<EndPoint> DescribeVpcEndPointResponse::GetEndPointSet() const
{
    return m_endPointSet;
}

bool DescribeVpcEndPointResponse::EndPointSetHasBeenSet() const
{
    return m_endPointSetHasBeenSet;
}

uint64_t DescribeVpcEndPointResponse::GetTotalCount() const
{
    return m_totalCount;
}
