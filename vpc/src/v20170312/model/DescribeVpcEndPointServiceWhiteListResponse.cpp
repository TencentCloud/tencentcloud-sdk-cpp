...
CoreInternalOutcome DescribeVpcEndPointServiceWhiteListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("VpcEndpointServiceUserSet") && !rsp["VpcEndpointServiceUserSet"].IsNull())
    {
        if (!rsp["VpcEndpointServiceUserSet"].IsArray())
            return CoreInternalOutcome(Error("response `VpcEndpointServiceUserSet` is not array type"));
...
string DescribeVpcEndPointServiceWhiteListResponse::ToJsonString() const
{
...
bool DescribeVpcEndPointServiceWhiteListResponse::TotalCountHasBeenSet() const
{
