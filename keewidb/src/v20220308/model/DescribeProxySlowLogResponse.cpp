...
CoreInternalOutcome DescribeProxySlowLogResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InstanceProxySlowLogDetail") && !rsp["InstanceProxySlowLogDetail"].IsNull())
    {
        if (!rsp["InstanceProxySlowLogDetail"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InstanceProxySlowLogDetail` is not array type"));
...
string DescribeProxySlowLogResponse::ToJsonString() const
{
    ...
    if (m_instanceProxySlowLogDetailHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "InstanceProxySlowLogDetail";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_instanceProxySlowLogDetail.begin(); itr != m_instanceProxySlowLogDetail.end(); ++itr, ++i)
        {
