...
CoreInternalOutcome InstanceNodeInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("NodeRole") && !value["NodeRole"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceNodeInfo.NodeRole` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ZoneId") && !value["ZoneId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceNodeInfo.ZoneId` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string InstanceNodeInfo::GetNodeRole() const
{
    return m_nodeRole;
}

void InstanceNodeInfo::SetNodeRole(const string& _nodeRole)
{
    m_nodeRole = _nodeRole;
    m_nodeRoleHasBeenSet = true;
}

bool InstanceNodeInfo::NodeRoleHasBeenSet() const
{
    return m_nodeRoleHasBeenSet;
}

int64_t InstanceNodeInfo::GetZoneId() const
{
    return m_zoneId;
}

void InstanceNodeInfo::SetZoneId(const int64_t& _zoneId)
{
    m_zoneId = _zoneId;
    m_zoneIdHasBeenSet = true;
}

bool InstanceNodeInfo::ZoneIdHasBeenSet() const
{
