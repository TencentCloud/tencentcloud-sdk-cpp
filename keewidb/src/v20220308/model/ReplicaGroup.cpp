...
ReplicaGroup::ReplicaGroup() :
    ...
    m_keeWiDBNodesHasBeenSet(false)
{
}
...
CoreInternalOutcome ReplicaGroup::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("KeeWiDBNodes") && !value["KeeWiDBNodes"].IsNull())
    {
        if (!value["KeeWiDBNodes"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ReplicaGroup.KeeWiDBNodes` is not array type"));

        const rapidjson::Value &tmpValue = value["KeeWiDBNodes"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ...
            m_keeWiDBNodes.push_back(item);
        }
        m_keeWiDBNodesHasBeenSet = true;
    }
...
void ReplicaGroup::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_keeWiDBNodesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "KeeWiDBNodes";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_keeWiDBNodes.begin(); itr != m_keeWiDBNodes.end(); ++itr, ++i)
        {
...
vector<KeeWiDBNode> ReplicaGroup::GetKeeWiDBNodes() const
{
    return m_keeWiDBNodes;
}

void ReplicaGroup::SetKeeWiDBNodes(const vector<KeeWiDBNode>& _keeWiDBNodes)
{
    m_keeWiDBNodes = _keeWiDBNodes;
    m_keeWiDBNodesHasBeenSet = true;
}

bool ReplicaGroup::KeeWiDBNodesHasBeenSet() const
{
    return m_keeWiDBNodesHasBeenSet;
}
