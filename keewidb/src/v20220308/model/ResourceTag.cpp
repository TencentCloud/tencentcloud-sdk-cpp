...
CoreInternalOutcome ResourceTag::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TagKey") && !value["TagKey"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ResourceTag.TagKey` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("TagValue") && !value["TagValue"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ResourceTag.TagValue` IsString=false incorrectly").SetRequestId(requestId));
        }
