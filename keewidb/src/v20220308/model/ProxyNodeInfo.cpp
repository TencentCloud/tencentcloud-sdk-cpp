...
CoreInternalOutcome ProxyNodeInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("NodeId") && !value["NodeId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ProxyNodeInfo.NodeId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ProxyNodeInfo::GetNodeId() const
{
    return m_nodeId;
}

void ProxyNodeInfo::SetNodeId(const string& _nodeId)
{
    m_nodeId = _nodeId;
    m_nodeIdHasBeenSet = true;
}
