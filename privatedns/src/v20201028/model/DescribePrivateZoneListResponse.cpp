...
CoreInternalOutcome DescribePrivateZoneListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PrivateZoneSet") && !rsp["PrivateZoneSet"].IsNull())
    {
        if (!rsp["PrivateZoneSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PrivateZoneSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["PrivateZoneSet"];
...
string DescribePrivateZoneListResponse::ToJsonString() const
{
    ...
    if (m_privateZoneSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "PrivateZoneSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_privateZoneSet.begin(); itr != m_privateZoneSet.end(); ++itr, ++i)
        {
