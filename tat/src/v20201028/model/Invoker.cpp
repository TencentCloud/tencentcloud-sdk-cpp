...
CoreInternalOutcome Invoker::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tags") && !value["Tags"].IsNull())
    {
        if (!value["Tags"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Invoker.Tags` is not array type"));
...
vector<Tag> Invoker::GetTags() const
{
    return m_tags;
}

void Invoker::SetTags(const vector<Tag>& _tags)
{
    m_tags = _tags;
    m_tagsHasBeenSet = true;
}

bool Invoker::TagsHasBeenSet() const
{
