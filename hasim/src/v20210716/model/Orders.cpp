...
CoreInternalOutcome Orders::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Total") && !value["Total"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Orders.Total` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("List") && !value["List"].IsNull())
    {
        if (!value["List"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Orders.List` is not array type"));
...
int64_t Orders::GetTotal() const
{
    return m_total;
}

void Orders::SetTotal(const int64_t& _total)
{
    m_total = _total;
    m_totalHasBeenSet = true;
}

bool Orders::TotalHasBeenSet() const
{
    return m_totalHasBeenSet;
}

vector<OrderInfo> Orders::GetList() const
{
    return m_list;
}

void Orders::SetList(const vector<OrderInfo>& _list)
{
    m_list = _list;
    m_listHasBeenSet = true;
}

bool Orders::ListHasBeenSet() const
{
