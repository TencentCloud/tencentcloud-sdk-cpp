...
CoreInternalOutcome GetPublicKeyResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PublicKeyPem") && !rsp["PublicKeyPem"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `PublicKeyPem` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string GetPublicKeyResponse::ToJsonString() const
{
    ...
    if (m_publicKeyPemHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "PublicKeyPem";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_publicKeyPem.c_str(), allocator).Move(), allocator);
    }
