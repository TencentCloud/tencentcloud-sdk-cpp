...
CoreInternalOutcome GetRegionsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Regions") && !rsp["Regions"].IsNull())
    {
        if (!rsp["Regions"].IsArray())
            return CoreInternalOutcome(Error("response `Regions` is not array type"));
...
string GetRegionsResponse::ToJsonString() const
{
...
vector<string> GetRegionsResponse::GetRegions() const
{
    return m_regions;
}

bool GetRegionsResponse::RegionsHasBeenSet() const
{
