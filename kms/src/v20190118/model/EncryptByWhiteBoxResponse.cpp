...
CoreInternalOutcome EncryptByWhiteBoxResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InitializationVector") && !rsp["InitializationVector"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `InitializationVector` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("CipherText") && !rsp["CipherText"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `CipherText` IsString=false incorrectly").SetRequestId(requestId));
        }
