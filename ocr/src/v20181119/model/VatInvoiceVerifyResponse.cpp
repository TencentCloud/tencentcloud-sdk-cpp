...
CoreInternalOutcome VatInvoiceVerifyResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Invoice") && !rsp["Invoice"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Invoice` is not object type").SetRequestId(requestId));
        }
...
string VatInvoiceVerifyResponse::ToJsonString() const
{
...
VatInvoice VatInvoiceVerifyResponse::GetInvoice() const
{
    return m_invoice;
}
