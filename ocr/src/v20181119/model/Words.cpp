...
CoreInternalOutcome Words::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Confidence") && !value["Confidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Words.Confidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Character") && !value["Character"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Words.Character` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void Words::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_characterHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_character.c_str(), allocator).Move(), allocator);
    }
