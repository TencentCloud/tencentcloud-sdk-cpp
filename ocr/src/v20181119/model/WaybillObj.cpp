...
CoreInternalOutcome WaybillObj::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Text") && !value["Text"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `WaybillObj.Text` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string WaybillObj::GetText() const
{
    return m_text;
}

void WaybillObj::SetText(const string& _text)
{
    m_text = _text;
    m_textHasBeenSet = true;
}

bool WaybillObj::TextHasBeenSet() const
{
