...
CoreInternalOutcome DutyPaidProofOCRResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DutyPaidProofInfos") && !rsp["DutyPaidProofInfos"].IsNull())
    {
        if (!rsp["DutyPaidProofInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DutyPaidProofInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["DutyPaidProofInfos"];
...
string DutyPaidProofOCRResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_dutyPaidProofInfosHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DutyPaidProofInfos";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_dutyPaidProofInfos.begin(); itr != m_dutyPaidProofInfos.end(); ++itr, ++i)
        {
