...
CoreInternalOutcome InvoiceGeneralOCRResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InvoiceGeneralInfos") && !rsp["InvoiceGeneralInfos"].IsNull())
    {
        if (!rsp["InvoiceGeneralInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InvoiceGeneralInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["InvoiceGeneralInfos"];
...
string InvoiceGeneralOCRResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_invoiceGeneralInfosHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "InvoiceGeneralInfos";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_invoiceGeneralInfos.begin(); itr != m_invoiceGeneralInfos.end(); ++itr, ++i)
        {
