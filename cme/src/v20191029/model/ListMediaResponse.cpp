...
CoreInternalOutcome ListMediaResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("MaterialTotalCount") && !rsp["MaterialTotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MaterialTotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("MaterialInfoSet") && !rsp["MaterialInfoSet"].IsNull())
    {
        if (!rsp["MaterialInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `MaterialInfoSet` is not array type"));
    ...
    if (rsp.HasMember("ClassInfoSet") && !rsp["ClassInfoSet"].IsNull())
    {
        if (!rsp["ClassInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ClassInfoSet` is not array type"));
...
string ListMediaResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_materialTotalCountHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "MaterialTotalCount";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_materialTotalCount, allocator);
    }

    if (m_materialInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "MaterialInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_materialInfoSet.begin(); itr != m_materialInfoSet.end(); ++itr, ++i)
        {
    ...
    if (m_classInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ClassInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_classInfoSet.begin(); itr != m_classInfoSet.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
