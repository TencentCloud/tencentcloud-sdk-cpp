...
CoreInternalOutcome SearchScope::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Owner") && !value["Owner"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SearchScope.Owner` is not object type").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ClassPath") && !value["ClassPath"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SearchScope.ClassPath` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void SearchScope::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
