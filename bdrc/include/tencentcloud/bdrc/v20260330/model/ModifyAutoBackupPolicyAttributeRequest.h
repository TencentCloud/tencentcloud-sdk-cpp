...
#ifndef TENCENTCLOUD_BDRC_V20260330_MODEL_MODIFYAUTOBACKUPPOLICYATTRIBUTEREQUEST_H_
#define TENCENTCLOUD_BDRC_V20260330_MODEL_MODIFYAUTOBACKUPPOLICYATTRIBUTEREQUEST_H_
    ...
    namespace Bdrc
    {
        namespace V20260330
        {
            namespace Model
            {
                ...
                */
                class ModifyAutoBackupPolicyAttributeRequest : public AbstractModel
                {
                public:
                    ModifyAutoBackupPolicyAttributeRequest();
                    ~ModifyAutoBackupPolicyAttributeRequest() = default;
                    ...
                    /**
                     * 获取备份存储类型。SNAPSHOT表示走快照（不需要备份库），VAULT表示走备份库（必须关联一个备份库）。默认为SNAPSHOT
                     * @return StorageType 备份存储类型。SNAPSHOT表示走快照（不需要备份库），VAULT表示走备份库（必须关联一个备份库）。默认为SNAPSHOT
                     * 
                     */
                    std::string GetStorageType() const;

                    /**
                     * 设置备份存储类型。SNAPSHOT表示走快照（不需要备份库），VAULT表示走备份库（必须关联一个备份库）。默认为SNAPSHOT
...
#endif // !TENCENTCLOUD_BDRC_V20260330_MODEL_MODIFYAUTOBACKUPPOLICYATTRIBUTEREQUEST_H_
