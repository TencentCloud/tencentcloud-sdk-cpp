...
CoreInternalOutcome ApiKeysStatus::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ApiKeysStatus.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ApiKeySet") && !value["ApiKeySet"].IsNull())
    {
        if (!value["ApiKeySet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ApiKeysStatus.ApiKeySet` is not array type"));

        const rapidjson::Value &tmpValue = value["ApiKeySet"];
...
void ApiKeysStatus::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
