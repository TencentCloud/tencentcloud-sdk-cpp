...
CoreInternalOutcome IPStrategyApiStatus::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `IPStrategyApiStatus.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ApiIdStatusSet") && !value["ApiIdStatusSet"].IsNull())
    {
        if (!value["ApiIdStatusSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `IPStrategyApiStatus.ApiIdStatusSet` is not array type"));
...
int64_t IPStrategyApiStatus::GetTotalCount() const
{
    return m_totalCount;
}

void IPStrategyApiStatus::SetTotalCount(const int64_t& _totalCount)
{
    m_totalCount = _totalCount;
    m_totalCountHasBeenSet = true;
}

bool IPStrategyApiStatus::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<IPStrategyApi> IPStrategyApiStatus::GetApiIdStatusSet() const
{
    return m_apiIdStatusSet;
}

void IPStrategyApiStatus::SetApiIdStatusSet(const vector<IPStrategyApi>& _apiIdStatusSet)
{
    m_apiIdStatusSet = _apiIdStatusSet;
    m_apiIdStatusSetHasBeenSet = true;
}

bool IPStrategyApiStatus::ApiIdStatusSetHasBeenSet() const
{
