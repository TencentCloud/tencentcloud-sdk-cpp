...
CoreInternalOutcome ApiAppInfos::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ApiAppInfos.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ApiAppSet") && !value["ApiAppSet"].IsNull())
    {
        if (!value["ApiAppSet"].IsArray())
            return CoreInternalOutcome(Error("response `ApiAppInfos.ApiAppSet` is not array type"));
