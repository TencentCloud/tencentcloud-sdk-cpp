...
CoreInternalOutcome ApiInfoSummary::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ApiInfoSummary.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ApiSet") && !value["ApiSet"].IsNull())
    {
        if (!value["ApiSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ApiInfoSummary.ApiSet` is not array type"));
