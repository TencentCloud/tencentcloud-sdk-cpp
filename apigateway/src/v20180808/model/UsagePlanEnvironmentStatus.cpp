...
CoreInternalOutcome UsagePlanEnvironmentStatus::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UsagePlanEnvironmentStatus.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("EnvironmentList") && !value["EnvironmentList"].IsNull())
    {
        if (!value["EnvironmentList"].IsArray())
            return CoreInternalOutcome(Error("response `UsagePlanEnvironmentStatus.EnvironmentList` is not array type"));

        const Value &tmpValue = value["EnvironmentList"];
...
void UsagePlanEnvironmentStatus::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
