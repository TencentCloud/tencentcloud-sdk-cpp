...
CoreInternalOutcome ApiAppApiInfos::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ApiAppApiInfos.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ApiAppApiSet") && !value["ApiAppApiSet"].IsNull())
    {
        if (!value["ApiAppApiSet"].IsArray())
            return CoreInternalOutcome(Error("response `ApiAppApiInfos.ApiAppApiSet` is not array type"));
