...
CoreInternalOutcome DescribeCfsFileSystemsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FileSystems") && !rsp["FileSystems"].IsNull())
    {
        if (!rsp["FileSystems"].IsArray())
            return CoreInternalOutcome(Core::Error("response `FileSystems` is not array type"));

        const rapidjson::Value &tmpValue = rsp["FileSystems"];
...
string DescribeCfsFileSystemsResponse::ToJsonString() const
{
...
vector<FileSystemInfo> DescribeCfsFileSystemsResponse::GetFileSystems() const
{
