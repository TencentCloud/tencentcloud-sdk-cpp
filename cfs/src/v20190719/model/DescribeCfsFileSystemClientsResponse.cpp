...
CoreInternalOutcome DescribeCfsFileSystemClientsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ClientList") && !rsp["ClientList"].IsNull())
    {
        if (!rsp["ClientList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ClientList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ClientList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            FileSystemClient item;
...
string DescribeCfsFileSystemClientsResponse::ToJsonString() const
{
...
vector<FileSystemClient> DescribeCfsFileSystemClientsResponse::GetClientList() const
{
    return m_clientList;
}
