...
CoreInternalOutcome BucketInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Name") && !value["Name"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `BucketInfo.Name` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string BucketInfo::GetName() const
{
    return m_name;
}

void BucketInfo::SetName(const string& _name)
{
    m_name = _name;
    m_nameHasBeenSet = true;
}

bool BucketInfo::NameHasBeenSet() const
{
