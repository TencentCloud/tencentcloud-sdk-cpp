...
CoreInternalOutcome CreateSignUrlsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SignUrlInfos") && !rsp["SignUrlInfos"].IsNull())
    {
        if (!rsp["SignUrlInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SignUrlInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["SignUrlInfos"];
...
string CreateSignUrlsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_signUrlInfosHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SignUrlInfos";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_signUrlInfos.begin(); itr != m_signUrlInfos.end(); ++itr, ++i)
        {
...
vector<SignUrlInfo> CreateSignUrlsResponse::GetSignUrlInfos() const
{
    return m_signUrlInfos;
}
