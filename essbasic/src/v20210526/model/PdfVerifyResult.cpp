...
CoreInternalOutcome PdfVerifyResult::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("SignType") && !value["SignType"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `PdfVerifyResult.SignType` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t PdfVerifyResult::GetSignType() const
{
    return m_signType;
}

void PdfVerifyResult::SetSignType(const int64_t& _signType)
{
    m_signType = _signType;
    m_signTypeHasBeenSet = true;
}

bool PdfVerifyResult::SignTypeHasBeenSet() const
{
