...
#include <tencentcloud/essbasic/v20210526/model/HasAuthUser.h>
...
CoreInternalOutcome HasAuthUser::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("OpenId") && !value["OpenId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `HasAuthUser.OpenId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string HasAuthUser::GetOpenId() const
{
    return m_openId;
}

void HasAuthUser::SetOpenId(const string& _openId)
{
    m_openId = _openId;
    m_openIdHasBeenSet = true;
}

bool HasAuthUser::OpenIdHasBeenSet() const
{
