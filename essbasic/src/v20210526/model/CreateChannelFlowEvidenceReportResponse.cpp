...
CoreInternalOutcome CreateChannelFlowEvidenceReportResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ReportId") && !rsp["ReportId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ReportId` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_reportId = string(rsp["ReportId"].GetString());
...
bool CreateChannelFlowEvidenceReportResponse::ReportIdHasBeenSet() const
{
    return m_reportIdHasBeenSet;
}

string CreateChannelFlowEvidenceReportResponse::GetStatus() const
{
    return m_status;
}
