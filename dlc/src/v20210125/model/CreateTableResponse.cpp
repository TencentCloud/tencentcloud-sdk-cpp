...
CoreInternalOutcome CreateTableResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Execution") && !rsp["Execution"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Execution` is not object type").SetRequestId(requestId));
        }
...
string CreateTableResponse::ToJsonString() const
{
...
Execution CreateTableResponse::GetExecution() const
{
    return m_execution;
}

bool CreateTableResponse::ExecutionHasBeenSet() const
{
