...
CoreInternalOutcome DescribeDatabasesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DatabaseList") && !rsp["DatabaseList"].IsNull())
    {
        if (!rsp["DatabaseList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DatabaseList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["DatabaseList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DatabaseResponseInfo item;
...
string DescribeDatabasesResponse::ToJsonString() const
{
    ...
    if (m_databaseListHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_databaseList.begin(); itr != m_databaseList.end(); ++itr, ++i)
        {
...
vector<DatabaseResponseInfo> DescribeDatabasesResponse::GetDatabaseList() const
{
