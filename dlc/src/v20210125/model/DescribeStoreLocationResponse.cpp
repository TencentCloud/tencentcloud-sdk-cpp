...
CoreInternalOutcome DescribeStoreLocationResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("StoreLocation") && !rsp["StoreLocation"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `StoreLocation` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeStoreLocationResponse::ToJsonString() const
{
...
string DescribeStoreLocationResponse::GetStoreLocation() const
{
    return m_storeLocation;
}

bool DescribeStoreLocationResponse::StoreLocationHasBeenSet() const
{
