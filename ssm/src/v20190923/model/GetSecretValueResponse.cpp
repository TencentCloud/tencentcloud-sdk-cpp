...
CoreInternalOutcome GetSecretValueResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SecretBinary") && !rsp["SecretBinary"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SecretBinary` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("SecretString") && !rsp["SecretString"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SecretString` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string GetSecretValueResponse::ToJsonString() const
{
    ...
    if (m_secretBinaryHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_secretBinary.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_secretStringHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_secretString.c_str(), allocator).Move(), allocator);
    }
