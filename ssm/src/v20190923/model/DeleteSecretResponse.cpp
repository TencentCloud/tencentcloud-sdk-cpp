...
CoreInternalOutcome DeleteSecretResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DeleteTime") && !rsp["DeleteTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `DeleteTime` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string DeleteSecretResponse::ToJsonString() const
{
...
string DeleteSecretResponse::GetSecretName() const
{
    return m_secretName;
}

bool DeleteSecretResponse::SecretNameHasBeenSet() const
{
    return m_secretNameHasBeenSet;
}

int64_t DeleteSecretResponse::GetDeleteTime() const
{
    return m_deleteTime;
}
