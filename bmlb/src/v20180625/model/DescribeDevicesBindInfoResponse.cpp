...
CoreInternalOutcome DescribeDevicesBindInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LoadBalancerSet") && !rsp["LoadBalancerSet"].IsNull())
    {
        if (!rsp["LoadBalancerSet"].IsArray())
            return CoreInternalOutcome(Error("response `LoadBalancerSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["LoadBalancerSet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DevicesBindInfoLoadBalancer item;
...
string DescribeDevicesBindInfoResponse::ToJsonString() const
{
...
vector<DevicesBindInfoLoadBalancer> DescribeDevicesBindInfoResponse::GetLoadBalancerSet() const
{
    return m_loadBalancerSet;
}

bool DescribeDevicesBindInfoResponse::LoadBalancerSetHasBeenSet() const
{
