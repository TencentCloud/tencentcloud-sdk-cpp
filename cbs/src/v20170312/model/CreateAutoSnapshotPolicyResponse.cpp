...
CoreInternalOutcome CreateAutoSnapshotPolicyResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NextTriggerTime") && !rsp["NextTriggerTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `NextTriggerTime` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CreateAutoSnapshotPolicyResponse::ToJsonString() const
{
