...
CoreInternalOutcome ApplyDisk::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("SnapshotId") && !value["SnapshotId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ApplyDisk.SnapshotId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ApplyDisk::GetSnapshotId() const
{
    return m_snapshotId;
}

void ApplyDisk::SetSnapshotId(const string& _snapshotId)
{
    m_snapshotId = _snapshotId;
    m_snapshotIdHasBeenSet = true;
}

bool ApplyDisk::SnapshotIdHasBeenSet() const
{
