...
#ifndef TENCENTCLOUD_RUM_V20210622_MODEL_DESCRIBEDATAPVURLINFOREQUEST_H_
#define TENCENTCLOUD_RUM_V20210622_MODEL_DESCRIBEDATAPVURLINFOREQUEST_H_
    ...
    namespace Rum
    {
        namespace V20210622
        {
            namespace Model
            {
                ...
                */
                class DescribeDataPvUrlInfoRequest : public AbstractModel
                {
                public:
                    DescribeDataPvUrlInfoRequest();
                    ~DescribeDataPvUrlInfoRequest() = default;
                    ...
                    /**
                     * 获取查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
                     * @return Type 查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
                     * 
                     */
                    std::string GetType() const;

                    /**
                     * 设置查询类型（string）：当前后端实现固定按 from 聚合统计，未使用该字段（保留字段，传值不会影响结果）。
...
#endif // !TENCENTCLOUD_RUM_V20210622_MODEL_DESCRIBEDATAPVURLINFOREQUEST_H_
