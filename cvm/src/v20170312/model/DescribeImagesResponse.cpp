...
CoreInternalOutcome DescribeImagesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ImageSet") && !rsp["ImageSet"].IsNull())
    {
        if (!rsp["ImageSet"].IsArray())
            return CoreInternalOutcome(Error("response `ImageSet` is not array type"));

        const Value &tmpValue = rsp["ImageSet"];
...
vector<Image> DescribeImagesResponse::GetImageSet() const
{
    return m_imageSet;
}

bool DescribeImagesResponse::ImageSetHasBeenSet() const
{
    return m_imageSetHasBeenSet;
}

int64_t DescribeImagesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeImagesResponse::TotalCountHasBeenSet() const
{
