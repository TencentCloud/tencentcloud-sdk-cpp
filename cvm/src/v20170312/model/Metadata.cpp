...
CoreInternalOutcome Metadata::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Items") && !value["Items"].IsNull())
    {
        if (!value["Items"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Metadata.Items` is not array type"));
...
vector<MetadataItem> Metadata::GetItems() const
{
    return m_items;
}

void Metadata::SetItems(const vector<MetadataItem>& _items)
{
    m_items = _items;
    m_itemsHasBeenSet = true;
}

bool Metadata::ItemsHasBeenSet() const
{
