...
CoreInternalOutcome DescribeDisasterRecoverGroupsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DisasterRecoverGroupSet") && !rsp["DisasterRecoverGroupSet"].IsNull())
    {
        if (!rsp["DisasterRecoverGroupSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DisasterRecoverGroupSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["DisasterRecoverGroupSet"];
...
string DescribeDisasterRecoverGroupsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_disasterRecoverGroupSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DisasterRecoverGroupSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_disasterRecoverGroupSet.begin(); itr != m_disasterRecoverGroupSet.end(); ++itr, ++i)
        {
