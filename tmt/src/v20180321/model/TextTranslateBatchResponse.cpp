...
CoreInternalOutcome TextTranslateBatchResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TargetTextList") && !rsp["TargetTextList"].IsNull())
    {
        if (!rsp["TargetTextList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TargetTextList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TargetTextList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_targetTextList.push_back((*itr).GetString());
        }
...
string TextTranslateBatchResponse::ToJsonString() const
{
    ...
    if (m_targetTextListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TargetTextList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_targetTextList.begin(); itr != m_targetTextList.end(); ++itr)
        {
...
string TextTranslateBatchResponse::GetSource() const
{
    return m_source;
}

bool TextTranslateBatchResponse::SourceHasBeenSet() const
{
    return m_sourceHasBeenSet;
}

string TextTranslateBatchResponse::GetTarget() const
{
    return m_target;
}

bool TextTranslateBatchResponse::TargetHasBeenSet() const
{
    return m_targetHasBeenSet;
}

vector<string> TextTranslateBatchResponse::GetTargetTextList() const
{
    return m_targetTextList;
}
