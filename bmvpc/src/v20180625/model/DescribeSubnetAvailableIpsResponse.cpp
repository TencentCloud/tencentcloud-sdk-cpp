...
CoreInternalOutcome DescribeSubnetAvailableIpsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("IpSet") && !rsp["IpSet"].IsNull())
    {
        if (!rsp["IpSet"].IsArray())
            return CoreInternalOutcome(Error("response `IpSet` is not array type"));
...
string DescribeSubnetAvailableIpsResponse::ToJsonString() const
{
...
vector<string> DescribeSubnetAvailableIpsResponse::GetIpSet() const
{
    return m_ipSet;
}

bool DescribeSubnetAvailableIpsResponse::IpSetHasBeenSet() const
{
