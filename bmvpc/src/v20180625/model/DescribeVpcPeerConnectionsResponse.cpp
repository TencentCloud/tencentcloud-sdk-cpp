...
CoreInternalOutcome DescribeVpcPeerConnectionsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("VpcPeerConnectionSet") && !rsp["VpcPeerConnectionSet"].IsNull())
    {
        if (!rsp["VpcPeerConnectionSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `VpcPeerConnectionSet` is not array type"));
...
string DescribeVpcPeerConnectionsResponse::ToJsonString() const
{
    ...
    if (m_vpcPeerConnectionSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "VpcPeerConnectionSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_vpcPeerConnectionSet.begin(); itr != m_vpcPeerConnectionSet.end(); ++itr, ++i)
        {
