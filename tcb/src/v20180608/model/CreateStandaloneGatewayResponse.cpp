...
CoreInternalOutcome CreateStandaloneGatewayResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("GatewayName") && !rsp["GatewayName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `GatewayName` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CreateStandaloneGatewayResponse::ToJsonString() const
{
...
string CreateStandaloneGatewayResponse::GetGatewayName() const
{
    return m_gatewayName;
}

bool CreateStandaloneGatewayResponse::GatewayNameHasBeenSet() const
{
