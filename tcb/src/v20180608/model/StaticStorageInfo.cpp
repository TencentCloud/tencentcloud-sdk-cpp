...
CoreInternalOutcome StaticStorageInfo::Deserialize(const Value &value)
{
    string requestId = "";
...
void StaticStorageInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_staticDomainHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_staticDomain.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_defaultDirNameHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_defaultDirName.c_str(), allocator).Move(), allocator);
    }
