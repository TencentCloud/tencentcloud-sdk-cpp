...
CoreInternalOutcome CloudBaseRunVpcSubnet::Deserialize(const Value &value)
{
    string requestId = "";
...
void CloudBaseRunVpcSubnet::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_targetHasBeenSet)
    {
        ...
        string key = "Target";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_target.c_str(), allocator).Move(), allocator);
    }
