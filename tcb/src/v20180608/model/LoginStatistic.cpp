...
CoreInternalOutcome LoginStatistic::Deserialize(const Value &value)
{
    string requestId = "";
...
void LoginStatistic::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_statisticalTypeHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_statisticalType.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_statisticalCycleHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_statisticalCycle.c_str(), allocator).Move(), allocator);
    }
