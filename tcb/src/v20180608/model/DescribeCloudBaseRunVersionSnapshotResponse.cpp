...
CoreInternalOutcome DescribeCloudBaseRunVersionSnapshotResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Snapshots") && !rsp["Snapshots"].IsNull())
    {
        if (!rsp["Snapshots"].IsArray())
            return CoreInternalOutcome(Error("response `Snapshots` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Snapshots"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            CloudRunServiceSimpleVersionSnapshot item;
...
string DescribeCloudBaseRunVersionSnapshotResponse::ToJsonString() const
{
...
vector<CloudRunServiceSimpleVersionSnapshot> DescribeCloudBaseRunVersionSnapshotResponse::GetSnapshots() const
{
    return m_snapshots;
}

bool DescribeCloudBaseRunVersionSnapshotResponse::SnapshotsHasBeenSet() const
{
