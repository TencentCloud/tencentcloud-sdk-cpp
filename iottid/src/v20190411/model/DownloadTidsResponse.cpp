...
CoreInternalOutcome DownloadTidsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TidSet") && !rsp["TidSet"].IsNull())
    {
        if (!rsp["TidSet"].IsArray())
            return CoreInternalOutcome(Error("response `TidSet` is not array type"));
...
string DownloadTidsResponse::ToJsonString() const
{
...
vector<TidKeysInfo> DownloadTidsResponse::GetTidSet() const
{
    return m_tidSet;
}

bool DownloadTidsResponse::TidSetHasBeenSet() const
{
