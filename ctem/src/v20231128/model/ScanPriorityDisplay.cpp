...
ScanPriorityDisplay::ScanPriorityDisplay() :
    m_onlyScanNewAssetHasBeenSet(false),
    m_priorityRulesHasBeenSet(false)
{
}

CoreInternalOutcome ScanPriorityDisplay::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("OnlyScanNewAsset") && !value["OnlyScanNewAsset"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ScanPriorityDisplay.OnlyScanNewAsset` IsBool=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("PriorityRules") && !value["PriorityRules"].IsNull())
    {
        if (!value["PriorityRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ScanPriorityDisplay.PriorityRules` is not array type"));
...
void ScanPriorityDisplay::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
bool ScanPriorityDisplay::GetOnlyScanNewAsset() const
{
    return m_onlyScanNewAsset;
}

void ScanPriorityDisplay::SetOnlyScanNewAsset(const bool& _onlyScanNewAsset)
{
    m_onlyScanNewAsset = _onlyScanNewAsset;
    m_onlyScanNewAssetHasBeenSet = true;
}

bool ScanPriorityDisplay::OnlyScanNewAssetHasBeenSet() const
{
    return m_onlyScanNewAssetHasBeenSet;
}

vector<string> ScanPriorityDisplay::GetPriorityRules() const
{
    return m_priorityRules;
}

void ScanPriorityDisplay::SetPriorityRules(const vector<string>& _priorityRules)
{
    m_priorityRules = _priorityRules;
    m_priorityRulesHasBeenSet = true;
}

bool ScanPriorityDisplay::PriorityRulesHasBeenSet() const
{
