...
CoreInternalOutcome GetAllFirmwareVersionResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Version") && !rsp["Version"].IsNull())
    {
        if (!rsp["Version"].IsArray())
            return CoreInternalOutcome(Error("response `Version` is not array type"));
...
string GetAllFirmwareVersionResponse::ToJsonString() const
{
...
vector<string> GetAllFirmwareVersionResponse::GetVersion() const
{
    return m_version;
}

bool GetAllFirmwareVersionResponse::VersionHasBeenSet() const
{
