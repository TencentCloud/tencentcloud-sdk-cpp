...
CoreInternalOutcome DescribeDeviceDataHistoryResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FieldName") && !rsp["FieldName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `FieldName` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeDeviceDataHistoryResponse::ToJsonString() const
{
