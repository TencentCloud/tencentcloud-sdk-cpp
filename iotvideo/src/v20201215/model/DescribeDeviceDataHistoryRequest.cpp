...
string DescribeDeviceDataHistoryRequest::ToJsonString() const
{
    ...
    if (m_limitHasBeenSet)
    {
        ...
        for (auto itr = m_limit.begin(); itr != m_limit.end(); ++itr)
        {
...
vector<uint64_t> DescribeDeviceDataHistoryRequest::GetLimit() const
{
    return m_limit;
}

void DescribeDeviceDataHistoryRequest::SetLimit(const vector<uint64_t>& _limit)
{
    m_limit = _limit;
