...
CoreInternalOutcome DeliverStorageServiceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SrcServiceId") && !rsp["SrcServiceId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SrcServiceId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DeliverStorageServiceResponse::ToJsonString() const
{
    ...
    if (m_srcServiceIdHasBeenSet)
    {
        ...
        value.AddMember(iKey, rapidjson::Value(m_srcServiceId.c_str(), allocator).Move(), allocator);
    }
