...
CoreInternalOutcome DescribeSSLStatusResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("IsOpenSSL") && !rsp["IsOpenSSL"].IsNull())
    {
        if (!rsp["IsOpenSSL"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `IsOpenSSL` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_isOpenSSL = string(rsp["IsOpenSSL"].GetString());
        m_isOpenSSLHasBeenSet = true;
    }
...
string DescribeSSLStatusResponse::ToJsonString() const
{
    ...
    if (m_isOpenSSLHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "IsOpenSSL";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_isOpenSSL.c_str(), allocator).Move(), allocator);
    }
...
string DescribeSSLStatusResponse::GetIsOpenSSL() const
{
    return m_isOpenSSL;
}

bool DescribeSSLStatusResponse::IsOpenSSLHasBeenSet() const
{
    return m_isOpenSSLHasBeenSet;
}

string DescribeSSLStatusResponse::GetDownloadUrl() const
{
    return m_downloadUrl;
}

bool DescribeSSLStatusResponse::DownloadUrlHasBeenSet() const
{
