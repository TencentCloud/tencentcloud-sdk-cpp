...
CoreInternalOutcome ExportInstanceErrorLogsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ErrorLogItems") && !rsp["ErrorLogItems"].IsNull())
    {
        if (!rsp["ErrorLogItems"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ErrorLogItems` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ErrorLogItems"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ErrorLogItemExport item;
            ...
            m_errorLogItems.push_back(item);
        }
...
string ExportInstanceErrorLogsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_errorLogItemsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ErrorLogItems";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_errorLogItems.begin(); itr != m_errorLogItems.end(); ++itr, ++i)
        {
...
vector<ErrorLogItemExport> ExportInstanceErrorLogsResponse::GetErrorLogItems() const
{
    return m_errorLogItems;
}
