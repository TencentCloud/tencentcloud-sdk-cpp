...
CoreInternalOutcome LogFilter::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Compare") && !value["Compare"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LogFilter.Compare` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Value") && !value["Value"].IsNull())
    {
        if (!value["Value"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LogFilter.Value` is not array type"));
...
string LogFilter::GetCompare() const
{
    return m_compare;
}

void LogFilter::SetCompare(const string& _compare)
{
    m_compare = _compare;
    m_compareHasBeenSet = true;
}

bool LogFilter::CompareHasBeenSet() const
{
    return m_compareHasBeenSet;
}

vector<string> LogFilter::GetValue() const
{
    return m_value;
}

void LogFilter::SetValue(const vector<string>& _value)
{
    m_value = _value;
    m_valueHasBeenSet = true;
}
