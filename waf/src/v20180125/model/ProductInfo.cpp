...
CoreInternalOutcome ProductInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Value") && !value["Value"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ProductInfo.Value` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ProductInfo::GetValue() const
{
    return m_value;
}

void ProductInfo::SetValue(const string& _value)
{
    m_value = _value;
    m_valueHasBeenSet = true;
}

bool ProductInfo::ValueHasBeenSet() const
{
