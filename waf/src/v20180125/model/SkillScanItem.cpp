...
CoreInternalOutcome SkillScanItem::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ScanType") && !value["ScanType"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SkillScanItem.ScanType` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("RuleList") && !value["RuleList"].IsNull())
    {
        if (!value["RuleList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SkillScanItem.RuleList` is not array type"));
...
string SkillScanItem::GetScanType() const
{
    return m_scanType;
}

void SkillScanItem::SetScanType(const string& _scanType)
{
    m_scanType = _scanType;
    m_scanTypeHasBeenSet = true;
}

bool SkillScanItem::ScanTypeHasBeenSet() const
{
    return m_scanTypeHasBeenSet;
}

vector<SkillScanRuleHit> SkillScanItem::GetRuleList() const
{
    return m_ruleList;
}

void SkillScanItem::SetRuleList(const vector<SkillScanRuleHit>& _ruleList)
{
    m_ruleList = _ruleList;
    m_ruleListHasBeenSet = true;
}

bool SkillScanItem::RuleListHasBeenSet() const
{
