...
CoreInternalOutcome CreateAccessExportResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ExportId") && !rsp["ExportId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ExportId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CreateAccessExportResponse::ToJsonString() const
{
...
string CreateAccessExportResponse::GetExportId() const
{
    return m_exportId;
}

bool CreateAccessExportResponse::ExportIdHasBeenSet() const
{
