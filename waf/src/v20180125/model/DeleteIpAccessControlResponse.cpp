...
CoreInternalOutcome DeleteIpAccessControlResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["FailedItems"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `FailedItems` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_failedItems = string(rsp["FailedItems"].GetString());
...
string DeleteIpAccessControlResponse::ToJsonString() const
{
    ...
    if (m_failedItemsHasBeenSet)
    {
        ...
        value.AddMember(iKey, rapidjson::Value(m_failedItems.c_str(), allocator).Move(), allocator);
    }
...
string DeleteIpAccessControlResponse::GetFailedItems() const
{
    return m_failedItems;
}

bool DeleteIpAccessControlResponse::FailedItemsHasBeenSet() const
{
    return m_failedItemsHasBeenSet;
}

int64_t DeleteIpAccessControlResponse::GetFailedCount() const
{
    return m_failedCount;
}

bool DeleteIpAccessControlResponse::FailedCountHasBeenSet() const
{
