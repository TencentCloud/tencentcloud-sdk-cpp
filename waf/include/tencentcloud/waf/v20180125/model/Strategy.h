...
strsuffix （ 后缀匹配）
 ...
 numneq （ 数值不等于）
    ...
    各匹配字段对应的逻辑符号不同，详见上述匹配字段表格
                    ...
                    /**
                     * 获取匹配内容。验证码字段按 Field 表列出的控制台开放选项配置：CAPTCHA_RISK 支持 numeq、numneq、belong_to、not_belong_to、null、nnull；CAPTCHA_DEVICE_RISK 支持 belong_to、not_belong_to、null、nnull；CAPTCHAR_SCORE 支持 numeq、numgt、numlt、numge、numle、null、nnull。null/nnull 时 Content 必须为空字符串；数值比较传单个整数字符串；集合比较传 JSON 字符串数组的序列化字符串，元素范围和数量限制详见 Field。
                     * @return Content 匹配内容。验证码字段按 Field 表列出的控制台开放选项配置：CAPTCHA_RISK 支持 numeq、numneq、belong_to、not_belong_to、null、nnull；CAPTCHA_DEVICE_RISK 支持 belong_to、not_belong_to、null、nnull；CAPTCHAR_SCORE 支持 numeq、numgt、numlt、numge、numle、null、nnull。null/nnull 时 Content 必须为空字符串；数值比较传单个整数字符串；集合比较传 JSON 字符串数组的序列化字符串，元素范围和数量限制详见 Field。
                     * 
                     */
                    std::string GetContent() const;

                    /**
                     * 设置匹配内容。验证码字段按 Field 表列出的控制台开放选项配置：CAPTCHA_RISK 支持 numeq、numneq、belong_to、not_belong_to、null、nnull；CAPTCHA_DEVICE_RISK 支持 belong_to、not_belong_to、null、nnull；CAPTCHAR_SCORE 支持 numeq、numgt、numlt、numge、numle、null、nnull。null/nnull 时 Content 必须为空字符串；数值比较传单个整数字符串；集合比较传 JSON 字符串数组的序列化字符串，元素范围和数量限制详见 Field。
