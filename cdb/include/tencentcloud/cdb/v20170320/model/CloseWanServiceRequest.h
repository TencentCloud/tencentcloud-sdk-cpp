...
#ifndef TENCENTCLOUD_CDB_V20170320_MODEL_CLOSEWANSERVICEREQUEST_H_
#define TENCENTCLOUD_CDB_V20170320_MODEL_CLOSEWANSERVICEREQUEST_H_
    ...
    namespace Cdb
    {
        namespace V20170320
        {
            namespace Model
            {
                ...
                */
                class CloseWanServiceRequest : public AbstractModel
                {
                public:
                    CloseWanServiceRequest();
                    ~CloseWanServiceRequest() = default;
                    std::string ToJsonString() const;


                    /**
                     * 获取实例 ID，格式如：cdb-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同，可使用 [查询实例列表](https://cloud.tencent.com/document/api/236/15872) 接口获取，其值为输出参数中字段 InstanceId 的值。可传入只读组 ID 关闭只读组外网访问。 
                    ...
                    /**
                     * 设置实例 ID，格式如：cdb-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同，可使用 [查询实例列表](https://cloud.tencent.com/document/api/236/15872) 接口获取，其值为输出参数中字段 InstanceId 的值。可传入只读组 ID 关闭只读组外网访问。 
...
#endif // !TENCENTCLOUD_CDB_V20170320_MODEL_CLOSEWANSERVICEREQUEST_H_
