...
CoreInternalOutcome InstanceChargePrepaid::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("RenewFlag") && !value["RenewFlag"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InstanceChargePrepaid.RenewFlag` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t InstanceChargePrepaid::GetRenewFlag() const
{
    return m_renewFlag;
}

void InstanceChargePrepaid::SetRenewFlag(const int64_t& _renewFlag)
{
