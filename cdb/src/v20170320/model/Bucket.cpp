...
CoreInternalOutcome Bucket::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Count") && !value["Count"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Bucket.Count` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t Bucket::GetCount() const
{
    return m_count;
}

void Bucket::SetCount(const uint64_t& _count)
{
    m_count = _count;
    m_countHasBeenSet = true;
}
