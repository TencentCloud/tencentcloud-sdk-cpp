...
CoreInternalOutcome CreateBackupResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["BackupId"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `BackupId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_backupId = rsp["BackupId"].GetUint64();
...
uint64_t CreateBackupResponse::GetBackupId() const
{
    return m_backupId;
}

bool CreateBackupResponse::BackupIdHasBeenSet() const
{
