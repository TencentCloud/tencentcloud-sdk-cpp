...
CoreInternalOutcome DescribeInstanceNodesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InstanceNodesList") && !rsp["InstanceNodesList"].IsNull())
    {
        if (!rsp["InstanceNodesList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InstanceNodesList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["InstanceNodesList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ...
            m_instanceNodesList.push_back(item);
        }
...
string DescribeInstanceNodesResponse::ToJsonString() const
{
    ...
    if (m_instanceNodesListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "InstanceNodesList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_instanceNodesList.begin(); itr != m_instanceNodesList.end(); ++itr, ++i)
        {
...
int64_t DescribeInstanceNodesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeInstanceNodesResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<InstanceNode> DescribeInstanceNodesResponse::GetInstanceNodesList() const
{
    return m_instanceNodesList;
}
