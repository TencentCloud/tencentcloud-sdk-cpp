...
CoreInternalOutcome DescribeAlarmsByTaskResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AlarmInfos") && !rsp["AlarmInfos"].IsNull())
    {
        if (!rsp["AlarmInfos"].IsArray())
            return CoreInternalOutcome(Core::Error("response `AlarmInfos` is not array type"));

        const rapidjson::Value &tmpValue = rsp["AlarmInfos"];
    ...
    if (rsp.HasMember("FaultRatio") && !rsp["FaultRatio"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `FaultRatio` IsLosslessDouble=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("FaultTimeSpec") && !rsp["FaultTimeSpec"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `FaultTimeSpec` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeAlarmsByTaskResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_alarmInfosHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AlarmInfos";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_alarmInfos.begin(); itr != m_alarmInfos.end(); ++itr, ++i)
        {
    ...
    if (m_faultRatioHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "FaultRatio";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_faultRatio, allocator);
    }

    if (m_faultTimeSpecHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "FaultTimeSpec";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_faultTimeSpec.c_str(), allocator).Move(), allocator);
    }
