...
CoreInternalOutcome GetResultSummaryResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RealData") && !rsp["RealData"].IsNull())
    {
        if (!rsp["RealData"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RealData` is not array type"));
    ...
    if (rsp.HasMember("DayData") && !rsp["DayData"].IsNull())
    {
        if (!rsp["DayData"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DayData` is not array type"));
...
string GetResultSummaryResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_realDataHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_realData.begin(); itr != m_realData.end(); ++itr, ++i)
        {
    ...
    if (m_dayDataHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_dayData.begin(); itr != m_dayData.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
