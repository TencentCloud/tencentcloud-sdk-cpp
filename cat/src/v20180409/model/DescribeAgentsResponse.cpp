...
CoreInternalOutcome DescribeAgentsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Agents") && !rsp["Agents"].IsNull())
    {
        if (!rsp["Agents"].IsArray())
            return CoreInternalOutcome(Error("response `Agents` is not array type"));
...
vector<CatAgent> DescribeAgentsResponse::GetAgents() const
{
    return m_agents;
}

bool DescribeAgentsResponse::AgentsHasBeenSet() const
{
