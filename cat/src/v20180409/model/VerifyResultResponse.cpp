...
CoreInternalOutcome VerifyResultResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ErrorReason") && !rsp["ErrorReason"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ErrorReason` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string VerifyResultResponse::ToJsonString() const
{
