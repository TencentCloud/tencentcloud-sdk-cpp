...
CoreInternalOutcome GetAvailRatioHistoryResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("LowestIsp") && !rsp["LowestIsp"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LowestIsp` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("LowestIsp2") && !rsp["LowestIsp2"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LowestIsp2` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string GetAvailRatioHistoryResponse::ToJsonString() const
{
