...
CoreInternalOutcome DescribeEnvironmentRolesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EnvironmentRoleSets") && !rsp["EnvironmentRoleSets"].IsNull())
    {
        if (!rsp["EnvironmentRoleSets"].IsArray())
            return CoreInternalOutcome(Error("response `EnvironmentRoleSets` is not array type"));

        const rapidjson::Value &tmpValue = rsp["EnvironmentRoleSets"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            EnvironmentRole item;
...
string DescribeEnvironmentRolesResponse::ToJsonString() const
{
...
int64_t DescribeEnvironmentRolesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeEnvironmentRolesResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<EnvironmentRole> DescribeEnvironmentRolesResponse::GetEnvironmentRoleSets() const
{
    return m_environmentRoleSets;
}

bool DescribeEnvironmentRolesResponse::EnvironmentRoleSetsHasBeenSet() const
{
