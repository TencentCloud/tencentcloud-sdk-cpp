...
CoreInternalOutcome DescribeNamespaceBundlesOptResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("BundleSet") && !rsp["BundleSet"].IsNull())
    {
        if (!rsp["BundleSet"].IsArray())
            return CoreInternalOutcome(Error("response `BundleSet` is not array type"));
...
string DescribeNamespaceBundlesOptResponse::ToJsonString() const
{
...
int64_t DescribeNamespaceBundlesOptResponse::GetTotalCount() const
{
    return m_totalCount;
}
