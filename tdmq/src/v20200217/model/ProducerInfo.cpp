...
CoreInternalOutcome ProducerInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("LastUpdateTimestamp") && !value["LastUpdateTimestamp"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ProducerInfo.LastUpdateTimestamp` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t ProducerInfo::GetLastUpdateTimestamp() const
{
    return m_lastUpdateTimestamp;
}

void ProducerInfo::SetLastUpdateTimestamp(const int64_t& _lastUpdateTimestamp)
{
