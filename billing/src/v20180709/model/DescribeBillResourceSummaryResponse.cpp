...
CoreInternalOutcome DescribeBillResourceSummaryResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ResourceSummarySet") && !rsp["ResourceSummarySet"].IsNull())
    {
        if (!rsp["ResourceSummarySet"].IsArray())
            return CoreInternalOutcome(Error("response `ResourceSummarySet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ResourceSummarySet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            BillResourceSummary item;
...
string DescribeBillResourceSummaryResponse::ToJsonString() const
{
...
vector<BillResourceSummary> DescribeBillResourceSummaryResponse::GetResourceSummarySet() const
{
