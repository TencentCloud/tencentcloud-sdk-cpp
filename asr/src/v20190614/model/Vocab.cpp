...
CoreInternalOutcome Vocab::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("WordWeights") && !value["WordWeights"].IsNull())
    {
        ...
        const Value &tmpValue = value["WordWeights"];
...
void Vocab::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_vocabIdHasBeenSet)
    {
        ...
        string key = "VocabId";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_vocabId.c_str(), allocator).Move(), allocator);
    }
