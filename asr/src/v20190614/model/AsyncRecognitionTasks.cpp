...
CoreInternalOutcome AsyncRecognitionTasks::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Tasks") && !value["Tasks"].IsNull())
    {
        if (!value["Tasks"].IsArray())
            return CoreInternalOutcome(Error("response `AsyncRecognitionTasks.Tasks` is not array type"));
...
void AsyncRecognitionTasks::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
