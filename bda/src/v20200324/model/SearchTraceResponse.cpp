...
CoreInternalOutcome SearchTraceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Candidates") && !rsp["Candidates"].IsNull())
    {
        if (!rsp["Candidates"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Candidates` is not array type"));
    ...
    if (rsp.HasMember("InputRetCode") && !rsp["InputRetCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `InputRetCode` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("InputRetCodeDetails") && !rsp["InputRetCodeDetails"].IsNull())
    {
        if (!rsp["InputRetCodeDetails"].IsArray())
            return CoreInternalOutcome(Core::Error("response `InputRetCodeDetails` is not array type"));
...
string SearchTraceResponse::ToJsonString() const
{
