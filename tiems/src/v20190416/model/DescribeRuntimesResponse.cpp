...
CoreInternalOutcome DescribeRuntimesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("UserAccess") && !rsp["UserAccess"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `UserAccess` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeRuntimesResponse::ToJsonString() const
{
    ...
    if (m_userAccessHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "UserAccess";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, m_userAccess, allocator);
    }
