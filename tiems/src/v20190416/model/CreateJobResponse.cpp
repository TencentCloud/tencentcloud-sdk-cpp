...
CoreInternalOutcome CreateJobResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Job") && !rsp["Job"].IsNull())
    {
        if (!rsp["Job"].IsObject())
        {
            return CoreInternalOutcome(Error("response `Job` is not object type").SetRequestId(requestId));
        }
...
string CreateJobResponse::ToJsonString() const
{
...
Job CreateJobResponse::GetJob() const
{
    return m_job;
}

bool CreateJobResponse::JobHasBeenSet() const
{
