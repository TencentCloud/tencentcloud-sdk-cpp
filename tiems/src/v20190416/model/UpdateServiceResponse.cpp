...
CoreInternalOutcome UpdateServiceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Service") && !rsp["Service"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Service` is not object type").SetRequestId(requestId));
        }
...
string UpdateServiceResponse::ToJsonString() const
{
...
ModelService UpdateServiceResponse::GetService() const
{
    return m_service;
}

bool UpdateServiceResponse::ServiceHasBeenSet() const
{
