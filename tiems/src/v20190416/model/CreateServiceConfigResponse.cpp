...
CoreInternalOutcome CreateServiceConfigResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ServiceConfig") && !rsp["ServiceConfig"].IsNull())
    {
        if (!rsp["ServiceConfig"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `ServiceConfig` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_serviceConfig.Deserialize(rsp["ServiceConfig"]);
...
string CreateServiceConfigResponse::ToJsonString() const
{
...
bool CreateServiceConfigResponse::ServiceConfigHasBeenSet() const
{
