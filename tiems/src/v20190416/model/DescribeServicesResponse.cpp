...
CoreInternalOutcome DescribeServicesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Services") && !rsp["Services"].IsNull())
    {
        if (!rsp["Services"].IsArray())
            return CoreInternalOutcome(Error("response `Services` is not array type"));
...
string DescribeServicesResponse::ToJsonString() const
{
...
bool DescribeServicesResponse::ServicesHasBeenSet() const
{
    return m_servicesHasBeenSet;
}

uint64_t DescribeServicesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeServicesResponse::TotalCountHasBeenSet() const
{
