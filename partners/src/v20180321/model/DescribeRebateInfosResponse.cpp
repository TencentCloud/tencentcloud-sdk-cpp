...
CoreInternalOutcome DescribeRebateInfosResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RebateInfoSet") && !rsp["RebateInfoSet"].IsNull())
    {
        if (!rsp["RebateInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RebateInfoSet` is not array type"));
...
string DescribeRebateInfosResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_rebateInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "RebateInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_rebateInfoSet.begin(); itr != m_rebateInfoSet.end(); ++itr, ++i)
        {
