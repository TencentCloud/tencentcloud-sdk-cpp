...
CoreInternalOutcome DescribeVideoListResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["VideoList"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `VideoList` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_videoList.Deserialize(rsp["VideoList"]);
...
string DescribeVideoListResponse::ToJsonString() const
{
    ...
    if (m_videoListHasBeenSet)
    {
        ...
        m_videoList.ToJsonObject(value[key.c_str()], allocator);
    }
...
int64_t DescribeVideoListResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeVideoListResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

RecordTaskItem DescribeVideoListResponse::GetVideoList() const
{
    return m_videoList;
}
