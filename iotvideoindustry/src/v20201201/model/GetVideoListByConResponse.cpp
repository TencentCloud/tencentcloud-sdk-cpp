...
CoreInternalOutcome GetVideoListByConResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("VideoList") && !rsp["VideoList"].IsNull())
    {
        if (!rsp["VideoList"].IsArray())
            return CoreInternalOutcome(Error("response `VideoList` is not array type"));
...
string GetVideoListByConResponse::ToJsonString() const
{
...
vector<RecordTaskItem> GetVideoListByConResponse::GetVideoList() const
{
    return m_videoList;
}

bool GetVideoListByConResponse::VideoListHasBeenSet() const
{
    return m_videoListHasBeenSet;
}

int64_t GetVideoListByConResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool GetVideoListByConResponse::TotalCountHasBeenSet() const
{
