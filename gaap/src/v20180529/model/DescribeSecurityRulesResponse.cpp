...
CoreInternalOutcome DescribeSecurityRulesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SecurityRuleSet") && !rsp["SecurityRuleSet"].IsNull())
    {
        if (!rsp["SecurityRuleSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SecurityRuleSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["SecurityRuleSet"];
...
string DescribeSecurityRulesResponse::ToJsonString() const
{
    ...
    if (m_securityRuleSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SecurityRuleSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_securityRuleSet.begin(); itr != m_securityRuleSet.end(); ++itr, ++i)
        {
