...
CoreInternalOutcome CreateSecurityRulesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RuleIdList") && !rsp["RuleIdList"].IsNull())
    {
        if (!rsp["RuleIdList"].IsArray())
            return CoreInternalOutcome(Error("response `RuleIdList` is not array type"));
...
string CreateSecurityRulesResponse::ToJsonString() const
{
...
vector<string> CreateSecurityRulesResponse::GetRuleIdList() const
{
    return m_ruleIdList;
}

bool CreateSecurityRulesResponse::RuleIdListHasBeenSet() const
{
