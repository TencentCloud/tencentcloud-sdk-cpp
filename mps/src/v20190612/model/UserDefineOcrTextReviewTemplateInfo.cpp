...
CoreInternalOutcome UserDefineOcrTextReviewTemplateInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Switch") && !value["Switch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `UserDefineOcrTextReviewTemplateInfo.Switch` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("LabelSet") && !value["LabelSet"].IsNull())
    {
        if (!value["LabelSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `UserDefineOcrTextReviewTemplateInfo.LabelSet` is not array type"));
    ...
    if (value.HasMember("BlockConfidence") && !value["BlockConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `UserDefineOcrTextReviewTemplateInfo.BlockConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ReviewConfidence") && !value["ReviewConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `UserDefineOcrTextReviewTemplateInfo.ReviewConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
