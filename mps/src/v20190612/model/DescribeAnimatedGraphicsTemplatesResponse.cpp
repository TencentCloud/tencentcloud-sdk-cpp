...
CoreInternalOutcome DescribeAnimatedGraphicsTemplatesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AnimatedGraphicsTemplateSet") && !rsp["AnimatedGraphicsTemplateSet"].IsNull())
    {
        if (!rsp["AnimatedGraphicsTemplateSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `AnimatedGraphicsTemplateSet` is not array type"));
...
string DescribeAnimatedGraphicsTemplatesResponse::ToJsonString() const
{
    ...
    if (m_animatedGraphicsTemplateSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AnimatedGraphicsTemplateSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_animatedGraphicsTemplateSet.begin(); itr != m_animatedGraphicsTemplateSet.end(); ++itr, ++i)
        {
