...
CoreInternalOutcome DescribeUserInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PaymentType") && !rsp["PaymentType"].IsNull())
    {
        if (!rsp["PaymentType"].IsString())
        {
            return CoreInternalOutcome(Error("response `PaymentType` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_paymentType = string(rsp["PaymentType"].GetString());
...
string DescribeUserInfoResponse::GetPaymentType() const
{
