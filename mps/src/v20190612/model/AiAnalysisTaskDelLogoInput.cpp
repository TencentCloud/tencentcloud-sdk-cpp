...
CoreInternalOutcome AiAnalysisTaskDelLogoInput::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ExtendedParameter") && !value["ExtendedParameter"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AiAnalysisTaskDelLogoInput.ExtendedParameter` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string AiAnalysisTaskDelLogoInput::GetExtendedParameter() const
{
    return m_extendedParameter;
}

void AiAnalysisTaskDelLogoInput::SetExtendedParameter(const string& _extendedParameter)
{
    m_extendedParameter = _extendedParameter;
    m_extendedParameterHasBeenSet = true;
}

bool AiAnalysisTaskDelLogoInput::ExtendedParameterHasBeenSet() const
{
