...
CoreInternalOutcome CreatePersonSampleResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FailFaceInfoSet") && !rsp["FailFaceInfoSet"].IsNull())
    {
        if (!rsp["FailFaceInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `FailFaceInfoSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["FailFaceInfoSet"];
...
string CreatePersonSampleResponse::ToJsonString() const
{
    ...
    if (m_failFaceInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "FailFaceInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_failFaceInfoSet.begin(); itr != m_failFaceInfoSet.end(); ++itr, ++i)
        {
...
AiSamplePerson CreatePersonSampleResponse::GetPerson() const
{
    return m_person;
}

bool CreatePersonSampleResponse::PersonHasBeenSet() const
{
    return m_personHasBeenSet;
}

vector<AiSampleFailFaceInfo> CreatePersonSampleResponse::GetFailFaceInfoSet() const
{
    return m_failFaceInfoSet;
}

bool CreatePersonSampleResponse::FailFaceInfoSetHasBeenSet() const
{
