...
CoreInternalOutcome LiveStreamAiRecognitionResultInfo::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("ResultSet") && !value["ResultSet"].IsNull())
    {
        if (!value["ResultSet"].IsArray())
            return CoreInternalOutcome(Error("response `LiveStreamAiRecognitionResultInfo.ResultSet` is not array type"));
...
void LiveStreamAiRecognitionResultInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
