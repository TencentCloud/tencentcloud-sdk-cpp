...
CoreInternalOutcome AiAnalysisTaskTagOutput::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TagSet") && !value["TagSet"].IsNull())
    {
        if (!value["TagSet"].IsArray())
            return CoreInternalOutcome(Error("response `AiAnalysisTaskTagOutput.TagSet` is not array type"));
...
void AiAnalysisTaskTagOutput::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
