...
CoreInternalOutcome CreateInputRTMPPullSettings::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("SourceAddresses") && !value["SourceAddresses"].IsNull())
    {
        if (!value["SourceAddresses"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CreateInputRTMPPullSettings.SourceAddresses` is not array type"));

        const rapidjson::Value &tmpValue = value["SourceAddresses"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            RTMPPullSourceAddress item;
...
void CreateInputRTMPPullSettings::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<RTMPPullSourceAddress> CreateInputRTMPPullSettings::GetSourceAddresses() const
{
    return m_sourceAddresses;
}

void CreateInputRTMPPullSettings::SetSourceAddresses(const vector<RTMPPullSourceAddress>& _sourceAddresses)
{
    m_sourceAddresses = _sourceAddresses;
    m_sourceAddressesHasBeenSet = true;
}

bool CreateInputRTMPPullSettings::SourceAddressesHasBeenSet() const
{
