...
CoreInternalOutcome CreateVerifyRecordResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SubDomain") && !rsp["SubDomain"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `SubDomain` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("RecordType") && !rsp["RecordType"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `RecordType` IsString=false incorrectly").SetRequestId(requestId));
        }
