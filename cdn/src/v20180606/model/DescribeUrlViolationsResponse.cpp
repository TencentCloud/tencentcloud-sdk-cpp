...
CoreInternalOutcome DescribeUrlViolationsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("UrlRecordList") && !rsp["UrlRecordList"].IsNull())
    {
        if (!rsp["UrlRecordList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `UrlRecordList` is not array type"));
...
string DescribeUrlViolationsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_urlRecordListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "UrlRecordList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_urlRecordList.begin(); itr != m_urlRecordList.end(); ++itr, ++i)
        {
