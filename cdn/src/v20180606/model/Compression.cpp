...
CoreInternalOutcome Compression::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("CompressionRules") && !value["CompressionRules"].IsNull())
    {
        ...
        const Value &tmpValue = value["CompressionRules"];
...
void Compression::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
