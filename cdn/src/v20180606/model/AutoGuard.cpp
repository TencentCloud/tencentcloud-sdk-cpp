...
CoreInternalOutcome AutoGuard::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Switch") && !value["Switch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AutoGuard.Switch` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void AutoGuard::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
string AutoGuard::GetSwitch() const
{
    return m_switch;
}

void AutoGuard::SetSwitch(const string& _switch)
{
    m_switch = _switch;
    m_switchHasBeenSet = true;
}
