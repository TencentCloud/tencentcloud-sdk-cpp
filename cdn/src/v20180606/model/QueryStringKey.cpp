...
CoreInternalOutcome QueryStringKey::Deserialize(const Value &value)
{
    string requestId = "";
...
void QueryStringKey::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_reorderHasBeenSet)
    {
        ...
        string key = "Reorder";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_reorder.c_str(), allocator).Move(), allocator);
    }
