...
CoreInternalOutcome CreateDiagnoseUrlResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DiagnoseLink") && !rsp["DiagnoseLink"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DiagnoseLink` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string CreateDiagnoseUrlResponse::ToJsonString() const
{
