...
CoreInternalOutcome StorageOptionOverview::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("CFSOptions") && !value["CFSOptions"].IsNull())
    {
        if (!value["CFSOptions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `StorageOptionOverview.CFSOptions` is not array type"));

        const rapidjson::Value &tmpValue = value["CFSOptions"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            CFSOptionOverview item;
    ...
    if (value.HasMember("GooseFSOptions") && !value["GooseFSOptions"].IsNull())
    {
        if (!value["GooseFSOptions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `StorageOptionOverview.GooseFSOptions` is not array type"));

        const rapidjson::Value &tmpValue = value["GooseFSOptions"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            GooseFSOptionOverview item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
...
void StorageOptionOverview::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<CFSOptionOverview> StorageOptionOverview::GetCFSOptions() const
{
    return m_cFSOptions;
}

void StorageOptionOverview::SetCFSOptions(const vector<CFSOptionOverview>& _cFSOptions)
{
    m_cFSOptions = _cFSOptions;
    m_cFSOptionsHasBeenSet = true;
}

bool StorageOptionOverview::CFSOptionsHasBeenSet() const
{
    return m_cFSOptionsHasBeenSet;
}

vector<GooseFSOptionOverview> StorageOptionOverview::GetGooseFSOptions() const
{
    return m_gooseFSOptions;
}

void StorageOptionOverview::SetGooseFSOptions(const vector<GooseFSOptionOverview>& _gooseFSOptions)
{
    m_gooseFSOptions = _gooseFSOptions;
    m_gooseFSOptionsHasBeenSet = true;
}

bool StorageOptionOverview::GooseFSOptionsHasBeenSet() const
{
