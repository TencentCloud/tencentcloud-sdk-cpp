...
CoreInternalOutcome LoginNode::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("SystemDisk") && !value["SystemDisk"].IsNull())
    {
        if (!value["SystemDisk"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LoginNode.SystemDisk` is not array type"));
    ...
    {
        if (!value["InternetAccessible"].IsArray())
            return CoreInternalOutcome(Core::Error("response `LoginNode.InternetAccessible` is not array type"));

        const rapidjson::Value &tmpValue = value["InternetAccessible"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            InternetAccessible item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
            ...
            m_internetAccessible.push_back(item);
        }
...
void LoginNode::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_internetAccessibleHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_internetAccessible.begin(); itr != m_internetAccessible.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
...
vector<SystemDisk> LoginNode::GetSystemDisk() const
{
    return m_systemDisk;
}

void LoginNode::SetSystemDisk(const vector<SystemDisk>& _systemDisk)
{
...
vector<InternetAccessible> LoginNode::GetInternetAccessible() const
{
    return m_internetAccessible;
}

void LoginNode::SetInternetAccessible(const vector<InternetAccessible>& _internetAccessible)
{
