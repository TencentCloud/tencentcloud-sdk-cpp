...
CoreInternalOutcome DescribeOsInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("OsInfoSet") && !rsp["OsInfoSet"].IsNull())
    {
        if (!rsp["OsInfoSet"].IsArray())
            return CoreInternalOutcome(Error("response `OsInfoSet` is not array type"));
...
string DescribeOsInfoResponse::ToJsonString() const
{
...
vector<OsInfo> DescribeOsInfoResponse::GetOsInfoSet() const
{
    return m_osInfoSet;
}

bool DescribeOsInfoResponse::OsInfoSetHasBeenSet() const
{
