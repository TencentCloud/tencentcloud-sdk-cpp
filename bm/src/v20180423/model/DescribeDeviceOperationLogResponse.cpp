...
CoreInternalOutcome DescribeDeviceOperationLogResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DeviceOperationLogSet") && !rsp["DeviceOperationLogSet"].IsNull())
    {
        if (!rsp["DeviceOperationLogSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DeviceOperationLogSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["DeviceOperationLogSet"];
...
string DescribeDeviceOperationLogResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_deviceOperationLogSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DeviceOperationLogSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_deviceOperationLogSet.begin(); itr != m_deviceOperationLogSet.end(); ++itr, ++i)
        {
