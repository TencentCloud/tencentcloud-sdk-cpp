...
CoreInternalOutcome DeviceOperationLog::Deserialize(const Value &value)
{
    string requestId = "";
...
void DeviceOperationLog::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_taskDescriptionHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_taskDescription.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_opUinHasBeenSet)
    {
        ...
        string key = "OpUin";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_opUin.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_logDescriptionHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_logDescription.c_str(), allocator).Move(), allocator);
    }
