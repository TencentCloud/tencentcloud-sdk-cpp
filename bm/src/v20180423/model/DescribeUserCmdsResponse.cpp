...
CoreInternalOutcome DescribeUserCmdsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("UserCmds") && !rsp["UserCmds"].IsNull())
    {
        if (!rsp["UserCmds"].IsArray())
            return CoreInternalOutcome(Core::Error("response `UserCmds` is not array type"));
...
string DescribeUserCmdsResponse::ToJsonString() const
{
    ...
    if (m_userCmdsHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_userCmds.begin(); itr != m_userCmds.end(); ++itr, ++i)
        {
