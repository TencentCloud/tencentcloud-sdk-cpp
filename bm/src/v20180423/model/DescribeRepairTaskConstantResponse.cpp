...
CoreInternalOutcome DescribeRepairTaskConstantResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskTypeSet") && !rsp["TaskTypeSet"].IsNull())
    {
        if (!rsp["TaskTypeSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TaskTypeSet` is not array type"));
...
string DescribeRepairTaskConstantResponse::ToJsonString() const
{
    ...
    if (m_taskTypeSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_taskTypeSet.begin(); itr != m_taskTypeSet.end(); ++itr, ++i)
        {
...
vector<TaskType> DescribeRepairTaskConstantResponse::GetTaskTypeSet() const
{
    return m_taskTypeSet;
}

bool DescribeRepairTaskConstantResponse::TaskTypeSetHasBeenSet() const
{
