...
CoreInternalOutcome CreateClassResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ClassId") && !rsp["ClassId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ClassId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string CreateClassResponse::ToJsonString() const
{
...
uint64_t CreateClassResponse::GetClassId() const
{
    return m_classId;
}

bool CreateClassResponse::ClassIdHasBeenSet() const
{
