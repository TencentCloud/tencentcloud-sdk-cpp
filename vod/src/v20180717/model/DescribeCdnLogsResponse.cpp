...
CoreInternalOutcome DescribeCdnLogsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("OverseaCdnLogs") && !rsp["OverseaCdnLogs"].IsNull())
    {
        if (!rsp["OverseaCdnLogs"].IsArray())
            return CoreInternalOutcome(Core::Error("response `OverseaCdnLogs` is not array type"));
    ...
    if (rsp.HasMember("DomesticCdnLogs") && !rsp["DomesticCdnLogs"].IsNull())
    {
        if (!rsp["DomesticCdnLogs"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DomesticCdnLogs` is not array type"));
...
string DescribeCdnLogsResponse::ToJsonString() const
{
    ...
    if (m_overseaCdnLogsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "OverseaCdnLogs";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_overseaCdnLogs.begin(); itr != m_overseaCdnLogs.end(); ++itr, ++i)
        {
    ...
    if (m_domesticCdnLogsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DomesticCdnLogs";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_domesticCdnLogs.begin(); itr != m_domesticCdnLogs.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
