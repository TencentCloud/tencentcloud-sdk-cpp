...
CoreInternalOutcome DescribeCDNUsageDataResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DataInterval") && !rsp["DataInterval"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DataInterval` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string DescribeCDNUsageDataResponse::ToJsonString() const
{
    ...
    if (m_dataIntervalHasBeenSet)
    {
        ...
        value.AddMember(iKey, m_dataInterval, allocator);
    }
...
bool DescribeCDNUsageDataResponse::DataHasBeenSet() const
{
