...
CoreInternalOutcome CreateSubAppIdResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SubAppId") && !rsp["SubAppId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SubAppId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string CreateSubAppIdResponse::ToJsonString() const
{
