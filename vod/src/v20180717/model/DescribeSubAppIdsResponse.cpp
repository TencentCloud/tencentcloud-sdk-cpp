...
CoreInternalOutcome DescribeSubAppIdsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SubAppIdInfoSet") && !rsp["SubAppIdInfoSet"].IsNull())
    {
        if (!rsp["SubAppIdInfoSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SubAppIdInfoSet` is not array type"));
...
string DescribeSubAppIdsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_subAppIdInfoSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SubAppIdInfoSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_subAppIdInfoSet.begin(); itr != m_subAppIdInfoSet.end(); ++itr, ++i)
        {
