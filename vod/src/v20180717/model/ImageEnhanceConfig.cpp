...
ImageEnhanceConfig::ImageEnhanceConfig() :
    ...
    m_advancedSuperResolutionHasBeenSet(false),
...
CoreInternalOutcome ImageEnhanceConfig::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("AdvancedSuperResolution") && !value["AdvancedSuperResolution"].IsNull())
    {
        if (!value["AdvancedSuperResolution"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `ImageEnhanceConfig.AdvancedSuperResolution` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_advancedSuperResolution.Deserialize(value["AdvancedSuperResolution"]);
        if (!outcome.IsSuccess())
        {
            outcome.GetError().SetRequestId(requestId);
            return outcome;
        ...
        m_advancedSuperResolutionHasBeenSet = true;
    }
...
void ImageEnhanceConfig::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_advancedSuperResolutionHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AdvancedSuperResolution";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_advancedSuperResolution.ToJsonObject(value[key.c_str()], allocator);
    }
...
SuperResolutionInfo ImageEnhanceConfig::GetSuperResolution() const
{
    return m_superResolution;
}

void ImageEnhanceConfig::SetSuperResolution(const SuperResolutionInfo& _superResolution)
{
...
AdvancedSuperResolutionConfig ImageEnhanceConfig::GetAdvancedSuperResolution() const
{
    return m_advancedSuperResolution;
}

void ImageEnhanceConfig::SetAdvancedSuperResolution(const AdvancedSuperResolutionConfig& _advancedSuperResolution)
{
    m_advancedSuperResolution = _advancedSuperResolution;
    m_advancedSuperResolutionHasBeenSet = true;
}

bool ImageEnhanceConfig::AdvancedSuperResolutionHasBeenSet() const
{
    return m_advancedSuperResolutionHasBeenSet;
}
...
ImageQualityEnhanceInfo ImageEnhanceConfig::GetImageQualityEnhance() const
{
    return m_imageQualityEnhance;
}

void ImageEnhanceConfig::SetImageQualityEnhance(const ImageQualityEnhanceInfo& _imageQualityEnhance)
{
...
ColorEnhanceInfo ImageEnhanceConfig::GetColorEnhance() const
{
    return m_colorEnhance;
}

void ImageEnhanceConfig::SetColorEnhance(const ColorEnhanceInfo& _colorEnhance)
{
...
SharpEnhanceInfo ImageEnhanceConfig::GetSharpEnhance() const
{
    return m_sharpEnhance;
}

void ImageEnhanceConfig::SetSharpEnhance(const SharpEnhanceInfo& _sharpEnhance)
{
...
FaceEnhanceInfo ImageEnhanceConfig::GetFaceEnhance() const
{
    return m_faceEnhance;
}

void ImageEnhanceConfig::SetFaceEnhance(const FaceEnhanceInfo& _faceEnhance)
{
...
LowLightEnhanceInfo ImageEnhanceConfig::GetLowLightEnhance() const
{
    return m_lowLightEnhance;
}

void ImageEnhanceConfig::SetLowLightEnhance(const LowLightEnhanceInfo& _lowLightEnhance)
{
