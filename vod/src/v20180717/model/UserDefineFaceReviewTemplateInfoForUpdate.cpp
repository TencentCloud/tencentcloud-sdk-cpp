...
CoreInternalOutcome UserDefineFaceReviewTemplateInfoForUpdate::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Switch") && !value["Switch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UserDefineFaceReviewTemplateInfoForUpdate.Switch` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("LabelSet") && !value["LabelSet"].IsNull())
    {
        if (!value["LabelSet"].IsArray())
            return CoreInternalOutcome(Error("response `UserDefineFaceReviewTemplateInfoForUpdate.LabelSet` is not array type"));
    ...
    if (value.HasMember("BlockConfidence") && !value["BlockConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UserDefineFaceReviewTemplateInfoForUpdate.BlockConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ReviewConfidence") && !value["ReviewConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `UserDefineFaceReviewTemplateInfoForUpdate.ReviewConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void UserDefineFaceReviewTemplateInfoForUpdate::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
