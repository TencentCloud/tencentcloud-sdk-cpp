...
CoreInternalOutcome PullEventsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EventSet") && !rsp["EventSet"].IsNull())
    {
        if (!rsp["EventSet"].IsArray())
            return CoreInternalOutcome(Error("response `EventSet` is not array type"));
...
string PullEventsResponse::ToJsonString() const
{
...
vector<EventContent> PullEventsResponse::GetEventSet() const
{
    return m_eventSet;
}

bool PullEventsResponse::EventSetHasBeenSet() const
{
