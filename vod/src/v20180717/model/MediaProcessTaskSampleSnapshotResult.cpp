...
CoreInternalOutcome MediaProcessTaskSampleSnapshotResult::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Progress") && !value["Progress"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MediaProcessTaskSampleSnapshotResult.Progress` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t MediaProcessTaskSampleSnapshotResult::GetProgress() const
{
    return m_progress;
}

void MediaProcessTaskSampleSnapshotResult::SetProgress(const int64_t& _progress)
{
    m_progress = _progress;
    m_progressHasBeenSet = true;
}

bool MediaProcessTaskSampleSnapshotResult::ProgressHasBeenSet() const
{
