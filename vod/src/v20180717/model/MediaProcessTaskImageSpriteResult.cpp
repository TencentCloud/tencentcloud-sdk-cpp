...
CoreInternalOutcome MediaProcessTaskImageSpriteResult::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Progress") && !value["Progress"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MediaProcessTaskImageSpriteResult.Progress` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t MediaProcessTaskImageSpriteResult::GetProgress() const
{
    return m_progress;
}

void MediaProcessTaskImageSpriteResult::SetProgress(const int64_t& _progress)
{
    m_progress = _progress;
    m_progressHasBeenSet = true;
}

bool MediaProcessTaskImageSpriteResult::ProgressHasBeenSet() const
{
