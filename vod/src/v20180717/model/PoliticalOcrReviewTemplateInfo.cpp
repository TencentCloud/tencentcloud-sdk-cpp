...
CoreInternalOutcome PoliticalOcrReviewTemplateInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Switch") && !value["Switch"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `PoliticalOcrReviewTemplateInfo.Switch` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("BlockConfidence") && !value["BlockConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `PoliticalOcrReviewTemplateInfo.BlockConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("ReviewConfidence") && !value["ReviewConfidence"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `PoliticalOcrReviewTemplateInfo.ReviewConfidence` IsInt64=false incorrectly").SetRequestId(requestId));
        }
