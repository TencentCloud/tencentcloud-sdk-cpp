...
CoreInternalOutcome DescribeVoicesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Voices") && !rsp["Voices"].IsNull())
    {
        if (!rsp["Voices"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Voices` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Voices"];
...
string DescribeVoicesResponse::ToJsonString() const
{
    ...
    if (m_voicesHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_voices.begin(); itr != m_voices.end(); ++itr, ++i)
        {
...
bool DescribeVoicesResponse::VoicesHasBeenSet() const
{
    return m_voicesHasBeenSet;
}
