...
CoreInternalOutcome DescribeAndroidInstanceTasksStatusResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskStatusSet") && !rsp["TaskStatusSet"].IsNull())
    {
        if (!rsp["TaskStatusSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TaskStatusSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TaskStatusSet"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            AndroidInstanceTaskStatus item;
            ...
            m_taskStatusSet.push_back(item);
        }
...
string DescribeAndroidInstanceTasksStatusResponse::ToJsonString() const
{
    ...
    if (m_taskStatusSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_taskStatusSet.begin(); itr != m_taskStatusSet.end(); ++itr, ++i)
        {
...
vector<AndroidInstanceTaskStatus> DescribeAndroidInstanceTasksStatusResponse::GetTaskStatusSet() const
{
