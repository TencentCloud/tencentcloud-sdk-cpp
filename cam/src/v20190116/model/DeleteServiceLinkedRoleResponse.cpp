...
CoreInternalOutcome DeleteServiceLinkedRoleResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DeletionTaskId") && !rsp["DeletionTaskId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DeletionTaskId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string DeleteServiceLinkedRoleResponse::ToJsonString() const
{
