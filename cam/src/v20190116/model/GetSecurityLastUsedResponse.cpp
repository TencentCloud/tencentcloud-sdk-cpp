...
CoreInternalOutcome GetSecurityLastUsedResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SecretIdLastUsedRows") && !rsp["SecretIdLastUsedRows"].IsNull())
    {
        if (!rsp["SecretIdLastUsedRows"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SecretIdLastUsedRows` is not array type"));
...
string GetSecurityLastUsedResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_secretIdLastUsedRowsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SecretIdLastUsedRows";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_secretIdLastUsedRows.begin(); itr != m_secretIdLastUsedRows.end(); ++itr, ++i)
        {
