...
CoreInternalOutcome ListAttachedUserAllPoliciesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PolicyList") && !rsp["PolicyList"].IsNull())
    {
        if (!rsp["PolicyList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PolicyList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["PolicyList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            AttachedUserPolicy item;
...
string ListAttachedUserAllPoliciesResponse::ToJsonString() const
{
...
vector<AttachedUserPolicy> ListAttachedUserAllPoliciesResponse::GetPolicyList() const
{
    return m_policyList;
}

bool ListAttachedUserAllPoliciesResponse::PolicyListHasBeenSet() const
{
    return m_policyListHasBeenSet;
}

uint64_t ListAttachedUserAllPoliciesResponse::GetTotalNum() const
{
    return m_totalNum;
}

bool ListAttachedUserAllPoliciesResponse::TotalNumHasBeenSet() const
{
