...
CoreInternalOutcome StrategyInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tags") && !value["Tags"].IsNull())
    {
        if (!value["Tags"].IsArray())
            return CoreInternalOutcome(Core::Error("response `StrategyInfo.Tags` is not array type"));
...
vector<Tag> StrategyInfo::GetTags() const
{
    return m_tags;
}

void StrategyInfo::SetTags(const vector<Tag>& _tags)
{
    m_tags = _tags;
    m_tagsHasBeenSet = true;
}
