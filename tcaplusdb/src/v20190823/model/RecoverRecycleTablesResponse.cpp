...
CoreInternalOutcome RecoverRecycleTablesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TableResults") && !rsp["TableResults"].IsNull())
    {
        if (!rsp["TableResults"].IsArray())
            return CoreInternalOutcome(Error("response `TableResults` is not array type"));
...
string RecoverRecycleTablesResponse::ToJsonString() const
{
...
uint64_t RecoverRecycleTablesResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool RecoverRecycleTablesResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<TableResultNew> RecoverRecycleTablesResponse::GetTableResults() const
{
    return m_tableResults;
}

bool RecoverRecycleTablesResponse::TableResultsHasBeenSet() const
{
