...
CoreInternalOutcome ModifyCensorshipResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["Uins"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Uins` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Uins"];
    ...
    if (rsp.HasMember("Censorship") && !rsp["Censorship"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Censorship` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string ModifyCensorshipResponse::ToJsonString() const
{
