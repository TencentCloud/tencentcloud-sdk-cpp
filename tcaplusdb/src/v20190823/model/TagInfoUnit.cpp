...
CoreInternalOutcome TagInfoUnit::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TagKey") && !value["TagKey"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TagInfoUnit.TagKey` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("TagValue") && !value["TagValue"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TagInfoUnit.TagValue` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void TagInfoUnit::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
