...
CoreInternalOutcome DescribePrometheusTemplateSyncResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Targets") && !rsp["Targets"].IsNull())
    {
        if (!rsp["Targets"].IsArray())
            return CoreInternalOutcome(Error("response `Targets` is not array type"));
...
string DescribePrometheusTemplateSyncResponse::ToJsonString() const
{
...
vector<PrometheusTemplateSyncTarget> DescribePrometheusTemplateSyncResponse::GetTargets() const
{
    return m_targets;
}

bool DescribePrometheusTemplateSyncResponse::TargetsHasBeenSet() const
{
