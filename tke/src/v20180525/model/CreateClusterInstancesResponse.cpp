...
CoreInternalOutcome CreateClusterInstancesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("InstanceIdSet") && !rsp["InstanceIdSet"].IsNull())
    {
        if (!rsp["InstanceIdSet"].IsArray())
            return CoreInternalOutcome(Error("response `InstanceIdSet` is not array type"));
...
string CreateClusterInstancesResponse::ToJsonString() const
{
...
vector<string> CreateClusterInstancesResponse::GetInstanceIdSet() const
{
    return m_instanceIdSet;
}

bool CreateClusterInstancesResponse::InstanceIdSetHasBeenSet() const
{
