...
CoreInternalOutcome ClusterCredential::Deserialize(const Value &value)
{
    string requestId = "";
...
void ClusterCredential::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_cACertHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_cACert.c_str(), allocator).Move(), allocator);
    }
