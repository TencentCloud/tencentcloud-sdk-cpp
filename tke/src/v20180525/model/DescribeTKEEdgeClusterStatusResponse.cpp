...
CoreInternalOutcome DescribeTKEEdgeClusterStatusResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Conditions") && !rsp["Conditions"].IsNull())
    {
        if (!rsp["Conditions"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Conditions` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Conditions"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ClusterCondition item;
...
string DescribeTKEEdgeClusterStatusResponse::ToJsonString() const
{
...
string DescribeTKEEdgeClusterStatusResponse::GetPhase() const
{
    return m_phase;
}

bool DescribeTKEEdgeClusterStatusResponse::PhaseHasBeenSet() const
{
    return m_phaseHasBeenSet;
}

vector<ClusterCondition> DescribeTKEEdgeClusterStatusResponse::GetConditions() const
{
    return m_conditions;
}

bool DescribeTKEEdgeClusterStatusResponse::ConditionsHasBeenSet() const
{
