...
CoreInternalOutcome DescribeClusterNodePoolDetailResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NodePool") && !rsp["NodePool"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `NodePool` is not object type").SetRequestId(requestId));
        }
...
string DescribeClusterNodePoolDetailResponse::ToJsonString() const
{
    ...
    if (m_nodePoolHasBeenSet)
    {
        ...
        m_nodePool.ToJsonObject(value[key.c_str()], allocator);
    }
