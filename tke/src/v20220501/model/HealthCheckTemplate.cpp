...
CoreInternalOutcome HealthCheckTemplate::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Rules") && !value["Rules"].IsNull())
    {
        if (!value["Rules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `HealthCheckTemplate.Rules` is not array type"));

        const rapidjson::Value &tmpValue = value["Rules"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            HealthCheckTemplateRule item;
...
vector<HealthCheckTemplateRule> HealthCheckTemplate::GetRules() const
{
    return m_rules;
}

void HealthCheckTemplate::SetRules(const vector<HealthCheckTemplateRule>& _rules)
{
    m_rules = _rules;
    m_rulesHasBeenSet = true;
}

bool HealthCheckTemplate::RulesHasBeenSet() const
{
