...
CoreInternalOutcome DescribeComplianceDetailResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CheckConfigDetail") && !rsp["CheckConfigDetail"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CheckConfigDetail` is not object type").SetRequestId(requestId));
        }
...
string DescribeComplianceDetailResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_checkConfigDetailHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "CheckConfigDetail";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_checkConfigDetail.ToJsonObject(value[key.c_str()], allocator);
    }
