...
CoreInternalOutcome Command::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("DeviceKind") && !value["DeviceKind"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Command.DeviceKind` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string Command::GetDeviceKind() const
{
    return m_deviceKind;
}

void Command::SetDeviceKind(const string& _deviceKind)
{
    m_deviceKind = _deviceKind;
    m_deviceKindHasBeenSet = true;
}

bool Command::DeviceKindHasBeenSet() const
{
