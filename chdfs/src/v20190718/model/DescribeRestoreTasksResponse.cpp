...
CoreInternalOutcome DescribeRestoreTasksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("RestoreTasks") && !rsp["RestoreTasks"].IsNull())
    {
        if (!rsp["RestoreTasks"].IsArray())
            return CoreInternalOutcome(Core::Error("response `RestoreTasks` is not array type"));

        const rapidjson::Value &tmpValue = rsp["RestoreTasks"];
...
string DescribeRestoreTasksResponse::ToJsonString() const
{
...
vector<RestoreTask> DescribeRestoreTasksResponse::GetRestoreTasks() const
{
