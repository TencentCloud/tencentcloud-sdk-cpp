...
CoreInternalOutcome DescribeDeviceResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["Device"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `Device` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_device.Deserialize(rsp["Device"]);
...
DeviceInfo DescribeDeviceResponse::GetDevice() const
{
    return m_device;
}

bool DescribeDeviceResponse::DeviceHasBeenSet() const
{
