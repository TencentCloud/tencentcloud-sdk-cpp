...
CoreInternalOutcome PublishRRPCMessageResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("MessageId") && !rsp["MessageId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `MessageId` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
string PublishRRPCMessageResponse::ToJsonString() const
{
