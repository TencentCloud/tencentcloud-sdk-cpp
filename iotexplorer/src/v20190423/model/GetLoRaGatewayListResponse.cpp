...
CoreInternalOutcome GetLoRaGatewayListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Gateways") && !rsp["Gateways"].IsNull())
    {
        if (!rsp["Gateways"].IsArray())
            return CoreInternalOutcome(Error("response `Gateways` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Gateways"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            LoRaGatewayItem item;
...
string GetLoRaGatewayListResponse::ToJsonString() const
{
...
uint64_t GetLoRaGatewayListResponse::GetTotal() const
{
    return m_total;
}

bool GetLoRaGatewayListResponse::TotalHasBeenSet() const
{
    return m_totalHasBeenSet;
}

vector<LoRaGatewayItem> GetLoRaGatewayListResponse::GetGateways() const
{
    return m_gateways;
}

bool GetLoRaGatewayListResponse::GatewaysHasBeenSet() const
{
