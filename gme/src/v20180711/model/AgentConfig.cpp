...
AgentConfig::AgentConfig() :
    ...
    m_initLLMMessageHasBeenSet(false),
...
CoreInternalOutcome AgentConfig::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("InitLLMMessage") && !value["InitLLMMessage"].IsNull())
    {
        if (!value["InitLLMMessage"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `AgentConfig.InitLLMMessage` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_initLLMMessage = string(value["InitLLMMessage"].GetString());
        m_initLLMMessageHasBeenSet = true;
    }
...
void AgentConfig::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_initLLMMessageHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "InitLLMMessage";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_initLLMMessage.c_str(), allocator).Move(), allocator);
    }
...
string AgentConfig::GetInitLLMMessage() const
{
    return m_initLLMMessage;
}

void AgentConfig::SetInitLLMMessage(const string& _initLLMMessage)
{
    m_initLLMMessage = _initLLMMessage;
    m_initLLMMessageHasBeenSet = true;
}

bool AgentConfig::InitLLMMessageHasBeenSet() const
{
    return m_initLLMMessageHasBeenSet;
}
