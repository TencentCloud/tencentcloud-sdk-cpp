...
#ifndef TENCENTCLOUD_GME_V20180711_MODEL_AGENTCONFIG_H_
#define TENCENTCLOUD_GME_V20180711_MODEL_AGENTCONFIG_H_
...
#include <tencentcloud/gme/v20180711/model/AmbientSound.h>
#include <tencentcloud/gme/v20180711/model/VoicePrint.h>
            ...
            namespace Model
            {
                ...
                */
                ...
                public:
                    ...
                    /**
                     * 获取机器人的UserId，用于进房发起任务。【注意】这个UserId不能与当前房间内的主播观众UserId重复。如果一个房间发起多个任务时，机器人的UserId也不能相互重复，否则会中断前一个任务。需要保证机器人UserId在房间内唯一。
                     * @return UserId 机器人的UserId，用于进房发起任务。【注意】这个UserId不能与当前房间内的主播观众UserId重复。如果一个房间发起多个任务时，机器人的UserId也不能相互重复，否则会中断前一个任务。需要保证机器人UserId在房间内唯一。
                     * 
                     */
                    std::string GetUserId() const;

                    /**
                     * 设置机器人的UserId，用于进房发起任务。【注意】这个UserId不能与当前房间内的主播观众UserId重复。如果一个房间发起多个任务时，机器人的UserId也不能相互重复，否则会中断前一个任务。需要保证机器人UserId在房间内唯一。
                    ...
                    /**
                     * 获取机器人UserId对应的校验签名，即UserId和UserSig相当于机器人进房的登录密码。
                     * @return UserSig 机器人UserId对应的校验签名，即UserId和UserSig相当于机器人进房的登录密码。
                     * 
                     */
                    std::string GetUserSig() const;

                    /**
                     * 设置机器人UserId对应的校验签名，即UserId和UserSig相当于机器人进房的登录密码。
...
默认值为空，表示不进行过滤。
                    ...
                    /**
                     * 获取与WelcomeMessage参数互斥，当该参数有值时，WelcomeMessage将失效。\n在对话开始后把该消息送到大模型来获取欢迎语。	
                     * @return InitLLMMessage 与WelcomeMessage参数互斥，当该参数有值时，WelcomeMessage将失效。\n在对话开始后把该消息送到大模型来获取欢迎语。	
                     * 
                     */
                    std::string GetInitLLMMessage() const;

                    /**
                     * 设置与WelcomeMessage参数互斥，当该参数有值时，WelcomeMessage将失效。\n在对话开始后把该消息送到大模型来获取欢迎语。	
                     * @param _initLLMMessage 与WelcomeMessage参数互斥，当该参数有值时，WelcomeMessage将失效。\n在对话开始后把该消息送到大模型来获取欢迎语。	
                     * 
                     */
                    void SetInitLLMMessage(const std::string& _initLLMMessage);

                    /**
                     * 判断参数 InitLLMMessage 是否已赋值
                     * @return InitLLMMessage 是否已赋值
                     * 
                     */
                    bool InitLLMMessageHasBeenSet() const;
                    ...
                    /**
                     * 获取机器人字幕显示模式。 - 0表示尽快显示，不会和音频播放进行同步。此时字幕全量下发，后面的字幕会包含前面的字幕。 - 1表示句子级别的实时显示，会和音频播放进行同步，只有当前句子对应的音频播放完后，下一条字幕才会下发。此时字幕增量下发，端上需要把前后的字幕进行拼接才是完整字幕。	
                     * @return SubtitleMode 机器人字幕显示模式。 - 0表示尽快显示，不会和音频播放进行同步。此时字幕全量下发，后面的字幕会包含前面的字幕。 - 1表示句子级别的实时显示，会和音频播放进行同步，只有当前句子对应的音频播放完后，下一条字幕才会下发。此时字幕增量下发，端上需要把前后的字幕进行拼接才是完整字幕。	
                     * 
                     */
                    uint64_t GetSubtitleMode() const;

                    /**
                     * 设置机器人字幕显示模式。 - 0表示尽快显示，不会和音频播放进行同步。此时字幕全量下发，后面的字幕会包含前面的字幕。 - 1表示句子级别的实时显示，会和音频播放进行同步，只有当前句子对应的音频播放完后，下一条字幕才会下发。此时字幕增量下发，端上需要把前后的字幕进行拼接才是完整字幕。	
...
默认值为空，表示不进行过滤。
                     ...
                     */
                    std::string m_initLLMMessage;
                    bool m_initLLMMessageHasBeenSet;
...
#endif // !TENCENTCLOUD_GME_V20180711_MODEL_AGENTCONFIG_H_
