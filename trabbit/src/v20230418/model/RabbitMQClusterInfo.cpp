...
CoreInternalOutcome RabbitMQClusterInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("MessageRetainTime") && !value["MessageRetainTime"].IsNull())
    {
        if (!value["MessageRetainTime"].IsUint64())
        {
            return CoreInternalOutcome(Core::Error("response `RabbitMQClusterInfo.MessageRetainTime` IsUint64=false incorrectly").SetRequestId(requestId));
        }
        m_messageRetainTime = value["MessageRetainTime"].GetUint64();
...
void RabbitMQClusterInfo::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_messageRetainTimeHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
...
uint64_t RabbitMQClusterInfo::GetMessageRetainTime() const
{
    return m_messageRetainTime;
}

void RabbitMQClusterInfo::SetMessageRetainTime(const uint64_t& _messageRetainTime)
{
    m_messageRetainTime = _messageRetainTime;
    m_messageRetainTimeHasBeenSet = true;
}
