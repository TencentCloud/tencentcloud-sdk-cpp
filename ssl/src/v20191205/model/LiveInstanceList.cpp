...
CoreInternalOutcome LiveInstanceList::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Error") && !value["Error"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LiveInstanceList.Error` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string LiveInstanceList::GetError() const
{
    return m_error;
}

void LiveInstanceList::SetError(const string& _error)
{
    m_error = _error;
    m_errorHasBeenSet = true;
}

bool LiveInstanceList::ErrorHasBeenSet() const
{
