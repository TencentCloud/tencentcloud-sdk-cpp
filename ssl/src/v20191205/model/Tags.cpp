...
CoreInternalOutcome Tags::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TagKey") && !value["TagKey"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Tags.TagKey` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("TagValue") && !value["TagValue"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Tags.TagValue` IsString=false incorrectly").SetRequestId(requestId));
        }
...
void Tags::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
