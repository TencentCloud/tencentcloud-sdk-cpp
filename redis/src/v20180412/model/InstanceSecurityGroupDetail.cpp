...
CoreInternalOutcome InstanceSecurityGroupDetail::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("SecurityGroupDetails") && !value["SecurityGroupDetails"].IsNull())
    {
        ...
        const Value &tmpValue = value["SecurityGroupDetails"];
...
void InstanceSecurityGroupDetail::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
