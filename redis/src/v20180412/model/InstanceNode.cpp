...
CoreInternalOutcome InstanceNode::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("InstanceClusterNode") && !value["InstanceClusterNode"].IsNull())
    {
        ...
        const Value &tmpValue = value["InstanceClusterNode"];
...
void InstanceNode::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
