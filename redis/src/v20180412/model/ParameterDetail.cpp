...
CoreInternalOutcome ParameterDetail::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Max") && !value["Max"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ParameterDetail.Max` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Min") && !value["Min"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ParameterDetail.Min` IsString=false incorrectly").SetRequestId(requestId));
        }
