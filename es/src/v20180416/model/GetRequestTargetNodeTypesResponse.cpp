...
CoreInternalOutcome GetRequestTargetNodeTypesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TargetNodeTypes") && !rsp["TargetNodeTypes"].IsNull())
    {
        if (!rsp["TargetNodeTypes"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TargetNodeTypes` is not array type"));
...
string GetRequestTargetNodeTypesResponse::ToJsonString() const
{
...
vector<string> GetRequestTargetNodeTypesResponse::GetTargetNodeTypes() const
{
    return m_targetNodeTypes;
}

bool GetRequestTargetNodeTypesResponse::TargetNodeTypesHasBeenSet() const
{
