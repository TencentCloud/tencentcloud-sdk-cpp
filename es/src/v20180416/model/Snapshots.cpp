...
CoreInternalOutcome Snapshots::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("MultiAz") && !value["MultiAz"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Snapshots.MultiAz` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t Snapshots::GetMultiAz() const
{
    return m_multiAz;
}

void Snapshots::SetMultiAz(const uint64_t& _multiAz)
{
    m_multiAz = _multiAz;
    m_multiAzHasBeenSet = true;
}
