...
ChatCompletionsResponse::ChatCompletionsResponse() :
    ...
    m_onlineSearchContentHasBeenSet(false)
{
}
...
CoreInternalOutcome ChatCompletionsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("OnlineSearchContent") && !rsp["OnlineSearchContent"].IsNull())
    {
        if (!rsp["OnlineSearchContent"].IsArray())
            return CoreInternalOutcome(Core::Error("response `OnlineSearchContent` is not array type"));

        const rapidjson::Value &tmpValue = rsp["OnlineSearchContent"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            WebContent item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            ...
            m_onlineSearchContent.push_back(item);
        }
        m_onlineSearchContentHasBeenSet = true;
    }
...
string ChatCompletionsResponse::ToJsonString() const
{
    ...
    if (m_onlineSearchContentHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "OnlineSearchContent";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_onlineSearchContent.begin(); itr != m_onlineSearchContent.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
...
TokenUsage ChatCompletionsResponse::GetUsage() const
{
    return m_usage;
}
...
vector<WebContent> ChatCompletionsResponse::GetOnlineSearchContent() const
{
    return m_onlineSearchContent;
}

bool ChatCompletionsResponse::OnlineSearchContentHasBeenSet() const
{
    return m_onlineSearchContentHasBeenSet;
}
