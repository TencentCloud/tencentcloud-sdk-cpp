...
CoreInternalOutcome DescribeDbauditInstanceTypeResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DbauditTypesSet") && !rsp["DbauditTypesSet"].IsNull())
    {
        if (!rsp["DbauditTypesSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DbauditTypesSet` is not array type"));
...
string DescribeDbauditInstanceTypeResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_dbauditTypesSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DbauditTypesSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_dbauditTypesSet.begin(); itr != m_dbauditTypesSet.end(); ++itr, ++i)
        {
