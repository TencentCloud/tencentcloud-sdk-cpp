...
CoreInternalOutcome RegionInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("RegionState") && !value["RegionState"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `RegionInfo.RegionState` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t RegionInfo::GetRegionState() const
{
    return m_regionState;
}

void RegionInfo::SetRegionState(const int64_t& _regionState)
{
