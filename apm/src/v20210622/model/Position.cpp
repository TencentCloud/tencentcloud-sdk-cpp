...
CoreInternalOutcome Position::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("X") && !value["X"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Position.X` IsLosslessDouble=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Y") && !value["Y"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Position.Y` IsLosslessDouble=false incorrectly").SetRequestId(requestId));
        }
...
bool Position::XHasBeenSet() const
{
...
bool Position::YHasBeenSet() const
{
