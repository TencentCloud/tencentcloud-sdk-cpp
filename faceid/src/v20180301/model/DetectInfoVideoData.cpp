...
CoreInternalOutcome DetectInfoVideoData::Deserialize(const Value &value)
{
    string requestId = "";
...
void DetectInfoVideoData::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_livenessVideoHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_livenessVideo.c_str(), allocator).Move(), allocator);
    }
