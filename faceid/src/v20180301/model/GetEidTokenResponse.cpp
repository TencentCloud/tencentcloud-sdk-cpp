...
CoreInternalOutcome GetEidTokenResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EidToken") && !rsp["EidToken"].IsNull())
    {
        if (!rsp["EidToken"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `EidToken` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_eidToken = string(rsp["EidToken"].GetString());
...
string GetEidTokenResponse::ToJsonString() const
{
    ...
    if (m_eidTokenHasBeenSet)
    {
        ...
        value.AddMember(iKey, rapidjson::Value(m_eidToken.c_str(), allocator).Move(), allocator);
    }
...
string GetEidTokenResponse::GetEidToken() const
{
    return m_eidToken;
}
