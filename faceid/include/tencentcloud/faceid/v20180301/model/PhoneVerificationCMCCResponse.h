...
#ifndef TENCENTCLOUD_FACEID_V20180301_MODEL_PHONEVERIFICATIONCMCCRESPONSE_H_
#define TENCENTCLOUD_FACEID_V20180301_MODEL_PHONEVERIFICATIONCMCCRESPONSE_H_
    ...
    namespace Faceid
    {
        namespace V20180301
        {
            namespace Model
            {
                ...
                */
                class PhoneVerificationCMCCResponse : public AbstractModel
                {
                public:
                    PhoneVerificationCMCCResponse();
                    ~PhoneVerificationCMCCResponse() = default;
                    CoreInternalOutcome Deserialize(const std::string &payload);
                    std::string ToJsonString() const;


                    /**
                     * 获取<p>认证结果码，收费情况如下。- 收费结果码：0: 认证通过。-4: 信息不一致（手机号已实名，但姓名和身份证号与实名信息不一致）。- 不收费结果码：-6: 手机号码不合法。-7: 身份证号码有误。-8: 姓名校验不通过。-9: 没有记录。-11: 验证中心服务繁忙。-13：该号段不支持验证</p>
...
#endif // !TENCENTCLOUD_FACEID_V20180301_MODEL_PHONEVERIFICATIONCMCCRESPONSE_H_
