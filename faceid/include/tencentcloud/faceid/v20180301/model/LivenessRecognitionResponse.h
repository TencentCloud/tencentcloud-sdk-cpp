...
#ifndef TENCENTCLOUD_FACEID_V20180301_MODEL_LIVENESSRECOGNITIONRESPONSE_H_
#define TENCENTCLOUD_FACEID_V20180301_MODEL_LIVENESSRECOGNITIONRESPONSE_H_
    ...
    namespace Faceid
    {
        namespace V20180301
        {
            namespace Model
            {
                ...
                */
                class LivenessRecognitionResponse : public AbstractModel
                {
                public:
                    LivenessRecognitionResponse();
                    ~LivenessRecognitionResponse() = default;
...
注意：此字段可能返回 null，表示取不到有效值。
                    ...
                    /**
                     * 获取<p>相似度。- 取值范围 [0.00, 100.00]。- 推荐相似度大于等于70时可判断为同一人，（采用固定阈值70，误通过率为万分之一，不支持调整阈值）</p>
                     * @return Sim <p>相似度。- 取值范围 [0.00, 100.00]。- 推荐相似度大于等于70时可判断为同一人，（采用固定阈值70，误通过率为万分之一，不支持调整阈值）</p>
                     * 
                     */
                    double GetSim() const;
                    ...
                    /**
                     * 获取<p>最佳截图列表，仅在配置了返回多张最佳截图时返回。</p>
注意：此字段可能返回 null，表示取不到有效值。
                     * @return BestFrameList <p>最佳截图列表，仅在配置了返回多张最佳截图时返回。</p>
注意：此字段可能返回 null，表示取不到有效值。
                     * 
                     */
...
#endif // !TENCENTCLOUD_FACEID_V20180301_MODEL_LIVENESSRECOGNITIONRESPONSE_H_
