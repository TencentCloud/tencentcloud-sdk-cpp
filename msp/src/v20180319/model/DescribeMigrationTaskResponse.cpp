...
CoreInternalOutcome DescribeMigrationTaskResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskStatus") && !rsp["TaskStatus"].IsNull())
    {
        if (!rsp["TaskStatus"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TaskStatus` is not array type"));
...
string DescribeMigrationTaskResponse::ToJsonString() const
{
    ...
    if (m_taskStatusHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_taskStatus.begin(); itr != m_taskStatus.end(); ++itr, ++i)
        {
...
vector<TaskStatus> DescribeMigrationTaskResponse::GetTaskStatus() const
{
    return m_taskStatus;
}

bool DescribeMigrationTaskResponse::TaskStatusHasBeenSet() const
{
