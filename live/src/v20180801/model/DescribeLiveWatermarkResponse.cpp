...
CoreInternalOutcome DescribeLiveWatermarkResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Watermark") && !rsp["Watermark"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Watermark` is not object type").SetRequestId(requestId));
        }
...
string DescribeLiveWatermarkResponse::ToJsonString() const
{
...
WatermarkInfo DescribeLiveWatermarkResponse::GetWatermark() const
{
    return m_watermark;
}

bool DescribeLiveWatermarkResponse::WatermarkHasBeenSet() const
{
