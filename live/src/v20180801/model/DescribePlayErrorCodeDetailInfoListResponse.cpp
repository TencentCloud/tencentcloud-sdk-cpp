...
CoreInternalOutcome DescribePlayErrorCodeDetailInfoListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("HttpCodeList") && !rsp["HttpCodeList"].IsNull())
    {
        if (!rsp["HttpCodeList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `HttpCodeList` is not array type"));
...
string DescribePlayErrorCodeDetailInfoListResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_httpCodeListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "HttpCodeList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_httpCodeList.begin(); itr != m_httpCodeList.end(); ++itr, ++i)
        {
