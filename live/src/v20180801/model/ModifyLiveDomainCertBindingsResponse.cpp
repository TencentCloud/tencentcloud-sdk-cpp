...
CoreInternalOutcome ModifyLiveDomainCertBindingsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("MismatchedDomainNames") && !rsp["MismatchedDomainNames"].IsNull())
    {
        if (!rsp["MismatchedDomainNames"].IsArray())
            return CoreInternalOutcome(Core::Error("response `MismatchedDomainNames` is not array type"));

        const rapidjson::Value &tmpValue = rsp["MismatchedDomainNames"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_mismatchedDomainNames.push_back((*itr).GetString());
        }
...
string ModifyLiveDomainCertBindingsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_mismatchedDomainNamesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "MismatchedDomainNames";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_mismatchedDomainNames.begin(); itr != m_mismatchedDomainNames.end(); ++itr)
        {
...
vector<string> ModifyLiveDomainCertBindingsResponse::GetMismatchedDomainNames() const
{
    return m_mismatchedDomainNames;
}
