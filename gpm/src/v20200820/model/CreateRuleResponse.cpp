...
CoreInternalOutcome CreateRuleResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["RuleInfo"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `RuleInfo` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_ruleInfo.Deserialize(rsp["RuleInfo"]);
...
string CreateRuleResponse::ToJsonString() const
{
    ...
    if (m_ruleInfoHasBeenSet)
    {
        ...
        m_ruleInfo.ToJsonObject(value[key.c_str()], allocator);
    }
...
RuleInfo CreateRuleResponse::GetRuleInfo() const
{
    return m_ruleInfo;
}

bool CreateRuleResponse::RuleInfoHasBeenSet() const
{
