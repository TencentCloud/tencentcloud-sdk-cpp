...
CoreInternalOutcome MatchTicket::Deserialize(const Value &value)
{
    string requestId = "";
...
void MatchTicket::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_matchResultHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_matchResult.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_statusMessageHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_statusMessage.c_str(), allocator).Move(), allocator);
    }
