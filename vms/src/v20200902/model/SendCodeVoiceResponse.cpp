...
CoreInternalOutcome SendCodeVoiceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SendStatus") && !rsp["SendStatus"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SendStatus` is not object type").SetRequestId(requestId));
        }
...
string SendCodeVoiceResponse::ToJsonString() const
{
    ...
    if (m_sendStatusHasBeenSet)
    {
        ...
        m_sendStatus.ToJsonObject(value[key.c_str()], allocator);
    }
...
SendStatus SendCodeVoiceResponse::GetSendStatus() const
{
    return m_sendStatus;
}

bool SendCodeVoiceResponse::SendStatusHasBeenSet() const
{
