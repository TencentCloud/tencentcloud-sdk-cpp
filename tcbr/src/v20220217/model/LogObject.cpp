...
CoreInternalOutcome LogObject::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("PkgLogId") && !value["PkgLogId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `LogObject.PkgLogId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string LogObject::GetPkgLogId() const
{
    return m_pkgLogId;
}

void LogObject::SetPkgLogId(const string& _pkgLogId)
{
    m_pkgLogId = _pkgLogId;
    m_pkgLogIdHasBeenSet = true;
}

bool LogObject::PkgLogIdHasBeenSet() const
{
