...
CoreInternalOutcome DescribeCloudRunServersResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ServerList") && !rsp["ServerList"].IsNull())
    {
        if (!rsp["ServerList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ServerList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ServerList"];
...
string DescribeCloudRunServersResponse::ToJsonString() const
{
...
vector<ServerBaseInfo> DescribeCloudRunServersResponse::GetServerList() const
{
    return m_serverList;
}
