...
CoreInternalOutcome DescribeEnvBaseInfoResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("EnvBaseInfo") && !rsp["EnvBaseInfo"].IsNull())
    {
        if (!rsp["EnvBaseInfo"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `EnvBaseInfo` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_envBaseInfo.Deserialize(rsp["EnvBaseInfo"]);
...
string DescribeEnvBaseInfoResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_envBaseInfoHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "EnvBaseInfo";
...
EnvBaseInfo DescribeEnvBaseInfoResponse::GetEnvBaseInfo() const
{
    return m_envBaseInfo;
}
