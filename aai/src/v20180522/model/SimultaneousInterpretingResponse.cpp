...
CoreInternalOutcome SimultaneousInterpretingResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NmtText") && !rsp["NmtText"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `NmtText` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string SimultaneousInterpretingResponse::ToJsonString() const
{
    ...
    if (m_nmtTextHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "NmtText";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_nmtText.c_str(), allocator).Move(), allocator);
    }
