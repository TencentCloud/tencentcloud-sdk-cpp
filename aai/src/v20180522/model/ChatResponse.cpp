...
CoreInternalOutcome ChatResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Answer") && !rsp["Answer"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Answer` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ChatResponse::ToJsonString() const
{
...
string ChatResponse::GetAnswer() const
{
    return m_answer;
}

bool ChatResponse::AnswerHasBeenSet() const
{
