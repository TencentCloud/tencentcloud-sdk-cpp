...
CoreInternalOutcome AuditLogFile::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("FileSize") && !value["FileSize"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuditLogFile.FileSize` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Progress") && !value["Progress"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AuditLogFile.Progress` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t AuditLogFile::GetFileSize() const
{
    return m_fileSize;
}

void AuditLogFile::SetFileSize(const uint64_t& _fileSize)
{
...
uint64_t AuditLogFile::GetProgress() const
{
    return m_progress;
}

void AuditLogFile::SetProgress(const uint64_t& _progress)
{
