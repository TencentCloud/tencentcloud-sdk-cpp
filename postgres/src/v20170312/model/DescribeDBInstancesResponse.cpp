...
CoreInternalOutcome DescribeDBInstancesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DBInstanceSet") && !rsp["DBInstanceSet"].IsNull())
    {
        if (!rsp["DBInstanceSet"].IsArray())
            return CoreInternalOutcome(Error("response `DBInstanceSet` is not array type"));
...
string DescribeDBInstancesResponse::ToJsonString() const
{
...
vector<DBInstance> DescribeDBInstancesResponse::GetDBInstanceSet() const
{
    return m_dBInstanceSet;
}

bool DescribeDBInstancesResponse::DBInstanceSetHasBeenSet() const
{
