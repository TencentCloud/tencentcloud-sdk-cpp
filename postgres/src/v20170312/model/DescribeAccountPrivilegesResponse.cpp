...
CoreInternalOutcome DescribeAccountPrivilegesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PrivilegeSet") && !rsp["PrivilegeSet"].IsNull())
    {
        if (!rsp["PrivilegeSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PrivilegeSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["PrivilegeSet"];
...
string DescribeAccountPrivilegesResponse::ToJsonString() const
{
    ...
    if (m_privilegeSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_privilegeSet.begin(); itr != m_privilegeSet.end(); ++itr, ++i)
        {
...
vector<DatabasePrivilege> DescribeAccountPrivilegesResponse::GetPrivilegeSet() const
{
    return m_privilegeSet;
}

bool DescribeAccountPrivilegesResponse::PrivilegeSetHasBeenSet() const
{
