...
CoreInternalOutcome DescribeDBInstanceSecurityGroupsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SecurityGroupSet") && !rsp["SecurityGroupSet"].IsNull())
    {
        if (!rsp["SecurityGroupSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SecurityGroupSet` is not array type"));
...
string DescribeDBInstanceSecurityGroupsResponse::ToJsonString() const
{
    ...
    if (m_securityGroupSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_securityGroupSet.begin(); itr != m_securityGroupSet.end(); ++itr, ++i)
        {
...
vector<SecurityGroup> DescribeDBInstanceSecurityGroupsResponse::GetSecurityGroupSet() const
{
    return m_securityGroupSet;
}

bool DescribeDBInstanceSecurityGroupsResponse::SecurityGroupSetHasBeenSet() const
{
