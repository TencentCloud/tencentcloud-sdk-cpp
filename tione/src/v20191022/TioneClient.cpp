...
TioneClient::UpdateNotebookLifecycleScriptOutcome TioneClient::UpdateNotebookLifecycleScript(const UpdateNotebookLifecycleScriptRequest &request)
{
    auto outcome = MakeRequest(request, "UpdateNotebookLifecycleScript");
    if (outcome.IsSuccess())
    {
        auto r = outcome.GetResult();
        string payload = string(r.Body(), r.BodySize());
        UpdateNotebookLifecycleScriptResponse rsp = UpdateNotebookLifecycleScriptResponse();
        auto o = rsp.Deserialize(payload);
        if (o.IsSuccess())
            return UpdateNotebookLifecycleScriptOutcome(rsp);
        else
            return UpdateNotebookLifecycleScriptOutcome(o.GetError());
    }
    else
    {
        return UpdateNotebookLifecycleScriptOutcome(outcome.GetError());
    }
}

void TioneClient::UpdateNotebookLifecycleScriptAsync(const UpdateNotebookLifecycleScriptRequest& request, const UpdateNotebookLifecycleScriptAsyncHandler& handler, const std::shared_ptr<const AsyncCallerContext>& context)
{
    auto fn = [this, request, handler, context]()
    {
        handler(this, request, this->UpdateNotebookLifecycleScript(request), context);
    };

    Executor::GetInstance()->Submit(new Runnable(fn));
}

TioneClient::UpdateNotebookLifecycleScriptOutcomeCallable TioneClient::UpdateNotebookLifecycleScriptCallable(const UpdateNotebookLifecycleScriptRequest &request)
{
    auto task = std::make_shared<std::packaged_task<UpdateNotebookLifecycleScriptOutcome()>>(
        [this, request]()
        {
            return this->UpdateNotebookLifecycleScript(request);
        }
    );
