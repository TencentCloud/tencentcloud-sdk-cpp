...
CoreInternalOutcome ImageSecret::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("SecretId") && !value["SecretId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ImageSecret.SecretId` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ImageSecret::GetSecretId() const
{
    return m_secretId;
}

void ImageSecret::SetSecretId(const string& _secretId)
{
    m_secretId = _secretId;
    m_secretIdHasBeenSet = true;
}

bool ImageSecret::SecretIdHasBeenSet() const
{
