...
CoreInternalOutcome ImageUrl::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Url") && !value["Url"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ImageUrl.Url` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string ImageUrl::GetUrl() const
{
    return m_url;
}

void ImageUrl::SetUrl(const string& _url)
{
    m_url = _url;
    m_urlHasBeenSet = true;
}

bool ImageUrl::UrlHasBeenSet() const
{
