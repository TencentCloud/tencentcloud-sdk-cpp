...
CoreInternalOutcome DescribeTrainingTaskPodsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("PodNames") && !rsp["PodNames"].IsNull())
    {
        if (!rsp["PodNames"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PodNames` is not array type"));

        const rapidjson::Value &tmpValue = rsp["PodNames"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_podNames.push_back((*itr).GetString());
        }
        m_podNamesHasBeenSet = true;
    }
...
string DescribeTrainingTaskPodsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_podNamesHasBeenSet)
    {
        ...
        for (auto itr = m_podNames.begin(); itr != m_podNames.end(); ++itr)
        {
...
vector<string> DescribeTrainingTaskPodsResponse::GetPodNames() const
{
    return m_podNames;
}

bool DescribeTrainingTaskPodsResponse::PodNamesHasBeenSet() const
{
    return m_podNamesHasBeenSet;
}

uint64_t DescribeTrainingTaskPodsResponse::GetTotalCount() const
{
    return m_totalCount;
}
