...
CoreInternalOutcome DescribeCustomRulesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CustomRules") && !rsp["CustomRules"].IsNull())
    {
        if (!rsp["CustomRules"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CustomRules` is not array type"));

        const rapidjson::Value &tmpValue = rsp["CustomRules"];
...
vector<CustomRule> DescribeCustomRulesResponse::GetCustomRules() const
{
    return m_customRules;
}

bool DescribeCustomRulesResponse::CustomRulesHasBeenSet() const
{
    return m_customRulesHasBeenSet;
}

uint64_t DescribeCustomRulesResponse::GetTotalCount() const
{
    return m_totalCount;
}
