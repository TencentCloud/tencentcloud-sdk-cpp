...
CoreInternalOutcome DescribeTraceCodeByIdResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TraceCode") && !rsp["TraceCode"].IsNull())
    {
        if (!rsp["TraceCode"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `TraceCode` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_traceCode.Deserialize(rsp["TraceCode"]);
...
string DescribeTraceCodeByIdResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_traceCodeHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TraceCode";
...
TraceCode DescribeTraceCodeByIdResponse::GetTraceCode() const
{
    return m_traceCode;
}
