...
CoreInternalOutcome Filters::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["ExactMatch"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `Filters.ExactMatch` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_exactMatch = string(value["ExactMatch"].GetString());
...
void Filters::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_exactMatchHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_exactMatch.c_str(), allocator).Move(), allocator);
    }
...
string Filters::GetExactMatch() const
{
    return m_exactMatch;
}

void Filters::SetExactMatch(const string& _exactMatch)
{
    m_exactMatch = _exactMatch;
    m_exactMatchHasBeenSet = true;
}

bool Filters::ExactMatchHasBeenSet() const
{
