...
CoreInternalOutcome OsName::Deserialize(const rapidjson::Value &value)
{
    ...
    {
        if (!value["MachineOSType"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `OsName.MachineOSType` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_machineOSType = value["MachineOSType"].GetInt64();
...
int64_t OsName::GetMachineOSType() const
{
    return m_machineOSType;
}

void OsName::SetMachineOSType(const int64_t& _machineOSType)
{
