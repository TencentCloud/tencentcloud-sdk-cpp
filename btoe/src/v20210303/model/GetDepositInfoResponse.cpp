...
CoreInternalOutcome GetDepositInfoResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["BlockchainHeight"].IsInt64())
        {
            return CoreInternalOutcome(Core::Error("response `BlockchainHeight` IsInt64=false incorrectly").SetRequestId(requestId));
        }
        m_blockchainHeight = rsp["BlockchainHeight"].GetInt64();
...
int64_t GetDepositInfoResponse::GetBlockchainHeight() const
{
    return m_blockchainHeight;
}

bool GetDepositInfoResponse::BlockchainHeightHasBeenSet() const
{
