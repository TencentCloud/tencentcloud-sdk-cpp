...
CoreInternalOutcome OutputManageMarketingRiskValue::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("PostTime") && !value["PostTime"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `OutputManageMarketingRiskValue.PostTime` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
int64_t OutputManageMarketingRiskValue::GetPostTime() const
{
    return m_postTime;
}

void OutputManageMarketingRiskValue::SetPostTime(const int64_t& _postTime)
{
