...
CoreInternalOutcome UploadIdResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TaskData") && !rsp["TaskData"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TaskData` is not object type").SetRequestId(requestId));
        }
...
string UploadIdResponse::ToJsonString() const
{
...
TaskData UploadIdResponse::GetTaskData() const
{
    return m_taskData;
}

bool UploadIdResponse::TaskDataHasBeenSet() const
{
