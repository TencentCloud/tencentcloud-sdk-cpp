...
CoreInternalOutcome DescribeClusterDetailResponse::Deserialize(const string &payload)
{
    ...
    {
        if (!rsp["Cluster"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `Cluster` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_cluster.Deserialize(rsp["Cluster"]);
...
ClusterDetail DescribeClusterDetailResponse::GetCluster() const
{
    return m_cluster;
}

bool DescribeClusterDetailResponse::ClusterHasBeenSet() const
{
