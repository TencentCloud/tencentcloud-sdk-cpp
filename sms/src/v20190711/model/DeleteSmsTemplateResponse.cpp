...
CoreInternalOutcome DeleteSmsTemplateResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("DeleteTemplateStatus") && !rsp["DeleteTemplateStatus"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `DeleteTemplateStatus` is not object type").SetRequestId(requestId));
        }
...
string DeleteSmsTemplateResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_deleteTemplateStatusHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "DeleteTemplateStatus";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_deleteTemplateStatus.ToJsonObject(value[key.c_str()], allocator);
    }
