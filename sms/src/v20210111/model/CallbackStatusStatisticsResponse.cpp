...
CoreInternalOutcome CallbackStatusStatisticsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CallbackStatusStatistics") && !rsp["CallbackStatusStatistics"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CallbackStatusStatistics` is not object type").SetRequestId(requestId));
        }
...
string CallbackStatusStatisticsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_callbackStatusStatisticsHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "CallbackStatusStatistics";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_callbackStatusStatistics.ToJsonObject(value[key.c_str()], allocator);
    }
