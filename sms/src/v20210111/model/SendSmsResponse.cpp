...
CoreInternalOutcome SendSmsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SendStatusSet") && !rsp["SendStatusSet"].IsNull())
    {
        if (!rsp["SendStatusSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `SendStatusSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["SendStatusSet"];
...
string SendSmsResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_sendStatusSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "SendStatusSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_sendStatusSet.begin(); itr != m_sendStatusSet.end(); ++itr, ++i)
        {
