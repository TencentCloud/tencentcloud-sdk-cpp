...
CoreInternalOutcome CateInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("IsLeaf") && !value["IsLeaf"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `CateInfo.IsLeaf` IsBool=false incorrectly").SetRequestId(requestId));
        }
...
bool CateInfo::GetIsLeaf() const
{
    return m_isLeaf;
}

void CateInfo::SetIsLeaf(const bool& _isLeaf)
{
    m_isLeaf = _isLeaf;
    m_isLeafHasBeenSet = true;
}

bool CateInfo::IsLeafHasBeenSet() const
{
