...
CoreInternalOutcome AgentToolInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("ToolSource") && !value["ToolSource"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AgentToolInfo.ToolSource` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("AuthConfigStatus") && !value["AuthConfigStatus"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AgentToolInfo.AuthConfigStatus` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t AgentToolInfo::GetToolSource() const
{
    return m_toolSource;
}

void AgentToolInfo::SetToolSource(const uint64_t& _toolSource)
{
...
int64_t AgentToolInfo::GetAuthConfigStatus() const
{
    return m_authConfigStatus;
}

void AgentToolInfo::SetAuthConfigStatus(const int64_t& _authConfigStatus)
{
