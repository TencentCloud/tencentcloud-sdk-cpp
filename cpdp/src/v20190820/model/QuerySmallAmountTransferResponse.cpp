...
CoreInternalOutcome QuerySmallAmountTransferResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ReturnStatus") && !rsp["ReturnStatus"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ReturnStatus` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string QuerySmallAmountTransferResponse::ToJsonString() const
{
    ...
    if (m_returnStatusHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ReturnStatus";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_returnStatus.c_str(), allocator).Move(), allocator);
    }
