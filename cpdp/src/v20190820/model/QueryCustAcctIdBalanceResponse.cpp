...
CoreInternalOutcome QueryCustAcctIdBalanceResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AcctArray") && !rsp["AcctArray"].IsNull())
    {
        if (!rsp["AcctArray"].IsArray())
            return CoreInternalOutcome(Core::Error("response `AcctArray` is not array type"));
...
string QueryCustAcctIdBalanceResponse::ToJsonString() const
{
    ...
    if (m_acctArrayHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AcctArray";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_acctArray.begin(); itr != m_acctArray.end(); ++itr, ++i)
        {
