...
CoreInternalOutcome RegisterBehaviorResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ReplenishSuccessFlag") && !rsp["ReplenishSuccessFlag"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ReplenishSuccessFlag` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("RegisterInfo") && !rsp["RegisterInfo"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `RegisterInfo` is not object type").SetRequestId(requestId));
        }
...
string RegisterBehaviorResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_replenishSuccessFlagHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ReplenishSuccessFlag";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(m_replenishSuccessFlag.c_str(), allocator).Move(), allocator);
    }
