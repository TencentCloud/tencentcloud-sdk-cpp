...
CoreInternalOutcome QueryMerchantResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("MerchantName") && !rsp["MerchantName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `MerchantName` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("BusinessPayFlag") && !rsp["BusinessPayFlag"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `BusinessPayFlag` IsString=false incorrectly").SetRequestId(requestId));
        }
