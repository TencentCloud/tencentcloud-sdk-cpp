...
CoreInternalOutcome ExternalContractUserInfo::Deserialize(const Value &value)
{
    string requestId = "";
...
void ExternalContractUserInfo::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_externalUserTypeHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_externalUserType.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_externalUserIdHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_externalUserId.c_str(), allocator).Move(), allocator);
    }
