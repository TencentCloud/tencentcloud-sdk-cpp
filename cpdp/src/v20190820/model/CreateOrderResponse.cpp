...
CoreInternalOutcome CreateOrderResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TraceNo") && !rsp["TraceNo"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TraceNo` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("PayUrl") && !rsp["PayUrl"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `PayUrl` IsString=false incorrectly").SetRequestId(requestId));
        }
