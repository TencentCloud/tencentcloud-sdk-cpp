...
CoreInternalOutcome TerminateContractResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ContractTerminateData") && !rsp["ContractTerminateData"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ContractTerminateData` is not object type").SetRequestId(requestId));
        }
...
string TerminateContractResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_contractTerminateDataHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "ContractTerminateData";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_contractTerminateData.ToJsonObject(value[key.c_str()], allocator);
    }
