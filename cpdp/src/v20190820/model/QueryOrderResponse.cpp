...
CoreInternalOutcome QueryOrderResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("OrderList") && !rsp["OrderList"].IsNull())
    {
        if (!rsp["OrderList"].IsArray())
            return CoreInternalOutcome(Error("response `OrderList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["OrderList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            QueryOrderOutOrderList item;
...
string QueryOrderResponse::ToJsonString() const
{
...
int64_t QueryOrderResponse::GetTotalNum() const
{
    return m_totalNum;
}

bool QueryOrderResponse::TotalNumHasBeenSet() const
{
    return m_totalNumHasBeenSet;
}

vector<QueryOrderOutOrderList> QueryOrderResponse::GetOrderList() const
{
    return m_orderList;
}

bool QueryOrderResponse::OrderListHasBeenSet() const
{
