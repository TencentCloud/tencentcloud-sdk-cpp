...
CoreInternalOutcome SpecConfigInfo::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("NodeCount") && !value["NodeCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `SpecConfigInfo.NodeCount` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
uint64_t SpecConfigInfo::GetNodeCount() const
{
    return m_nodeCount;
}

void SpecConfigInfo::SetNodeCount(const uint64_t& _nodeCount)
{
