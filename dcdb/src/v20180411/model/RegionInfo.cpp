...
CoreInternalOutcome RegionInfo::Deserialize(const Value &value)
{
    ...
    if (value.HasMember("AvailableChoice") && !value["AvailableChoice"].IsNull())
    {
        ...
        const Value &tmpValue = value["AvailableChoice"];
        for (Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            ShardZoneChooseInfo item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
...
vector<ShardZoneChooseInfo> RegionInfo::GetAvailableChoice() const
{
    return m_availableChoice;
}

void RegionInfo::SetAvailableChoice(const vector<ShardZoneChooseInfo>& _availableChoice)
{
