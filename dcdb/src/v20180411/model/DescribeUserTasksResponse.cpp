...
CoreInternalOutcome DescribeUserTasksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FlowSet") && !rsp["FlowSet"].IsNull())
    {
        if (!rsp["FlowSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `FlowSet` is not array type"));

        const rapidjson::Value &tmpValue = rsp["FlowSet"];
...
string DescribeUserTasksResponse::ToJsonString() const
{
    ...
    if (m_flowSetHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_flowSet.begin(); itr != m_flowSet.end(); ++itr, ++i)
        {
