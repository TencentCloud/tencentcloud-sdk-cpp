...
CoreInternalOutcome Capacity::Deserialize(const Value &value)
{
    string requestId = "";
...
void Capacity::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_cTCCTokenHasBeenSet)
    {
        ...
        value.AddMember(iKey, Value(m_cTCCToken.c_str(), allocator).Move(), allocator);
    }
