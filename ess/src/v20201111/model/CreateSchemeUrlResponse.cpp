...
CoreInternalOutcome CreateSchemeUrlResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("SchemeUrl") && !rsp["SchemeUrl"].IsNull())
    {
        if (!rsp["SchemeUrl"].IsString())
        {
            return CoreInternalOutcome(Core::Error("response `SchemeUrl` IsString=false incorrectly").SetRequestId(requestId));
        }
        m_schemeUrl = string(rsp["SchemeUrl"].GetString());
...
string CreateSchemeUrlResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_schemeUrlHasBeenSet)
    {
...
string CreateSchemeUrlResponse::GetSchemeUrl() const
{
    return m_schemeUrl;
}
