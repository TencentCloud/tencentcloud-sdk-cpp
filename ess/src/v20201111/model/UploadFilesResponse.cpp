...
CoreInternalOutcome UploadFilesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FileIds") && !rsp["FileIds"].IsNull())
    {
        if (!rsp["FileIds"].IsArray())
            return CoreInternalOutcome(Core::Error("response `FileIds` is not array type"));

        const rapidjson::Value &tmpValue = rsp["FileIds"];
...
string UploadFilesResponse::ToJsonString() const
{
