...
注意：
RegisterInfo 中的UnifiedSocialCreditCode值不为空的时候，才可设置为不可编辑。
                     * @return UnifiedSocialCreditCodeSame 是否允许编辑企业注册时统一社会信用代码。
<br/>true:不允许编辑。
<br/>false:允许编辑（默认值）。
<br/>
...
RegisterInfo 中的UnifiedSocialCreditCode值不为空的时候，才可设置为不可编辑。
                     * 
                     */
                    bool GetUnifiedSocialCreditCodeSame() const;
...
RegisterInfo 中的UnifiedSocialCreditCode值不为空的时候，才可设置为不可编辑。
                     * 
                     */
                    void SetUnifiedSocialCreditCodeSame(const bool& _unifiedSocialCreditCodeSame);

                    /**
                     * 判断参数 UnifiedSocialCreditCodeSame 是否已赋值
                     * @return UnifiedSocialCreditCodeSame 是否已赋值
                     * 
                     */
                    bool UnifiedSocialCreditCodeSameHasBeenSet() const;
...
RegisterInfo 中的UnifiedSocialCreditCode值不为空的时候，才可设置为不可编辑。
                     */
                    bool m_unifiedSocialCreditCodeSame;
                    bool m_unifiedSocialCreditCodeSameHasBeenSet;
