...
CoreInternalOutcome DescribeAutoScaleStrategiesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TimeBasedAutoScaleStrategies") && !rsp["TimeBasedAutoScaleStrategies"].IsNull())
    {
        if (!rsp["TimeBasedAutoScaleStrategies"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TimeBasedAutoScaleStrategies` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TimeBasedAutoScaleStrategies"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            TimeAutoScaleStrategy item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
            if (!outcome.IsSuccess())
            {
                outcome.GetError().SetRequestId(requestId);
            ...
            m_timeBasedAutoScaleStrategies.push_back(item);
        }
        m_timeBasedAutoScaleStrategiesHasBeenSet = true;
    }
...
string DescribeAutoScaleStrategiesResponse::ToJsonString() const
{
    ...
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TimeBasedAutoScaleStrategies";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_timeBasedAutoScaleStrategies.begin(); itr != m_timeBasedAutoScaleStrategies.end(); ++itr, ++i)
        {
            value[key.c_str()].PushBack(rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
            (*itr).ToJsonObject(value[key.c_str()][i], allocator);
        }
...
{
    return m_timeBasedAutoScaleStrategies;
}

bool DescribeAutoScaleStrategiesResponse::TimeBasedAutoScaleStrategiesHasBeenSet() const
{
    return m_timeBasedAutoScaleStrategiesHasBeenSet;
}
