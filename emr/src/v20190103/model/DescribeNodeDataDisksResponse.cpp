...
CoreInternalOutcome DescribeNodeDataDisksResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("CBSList") && !rsp["CBSList"].IsNull())
    {
        if (!rsp["CBSList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `CBSList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["CBSList"];
...
string DescribeNodeDataDisksResponse::ToJsonString() const
{
    ...
    if (m_cBSListHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_cBSList.begin(); itr != m_cBSList.end(); ++itr, ++i)
        {
...
int64_t DescribeNodeDataDisksResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeNodeDataDisksResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<CBSInstance> DescribeNodeDataDisksResponse::GetCBSList() const
{
    return m_cBSList;
}
