...
CoreInternalOutcome Order::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Direction") && !value["Direction"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Order.Direction` IsString=false incorrectly").SetRequestId(requestId));
        }
...
string Order::GetDirection() const
{
    return m_direction;
}

void Order::SetDirection(const string& _direction)
{
    m_direction = _direction;
    m_directionHasBeenSet = true;
}

bool Order::DirectionHasBeenSet() const
{
