...
CoreInternalOutcome AddFairPlayPemResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("FairPlayPemId") && !rsp["FairPlayPemId"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `FairPlayPemId` IsUint64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("Priority") && !rsp["Priority"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `Priority` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
string AddFairPlayPemResponse::ToJsonString() const
{
...
uint64_t AddFairPlayPemResponse::GetFairPlayPemId() const
{
    return m_fairPlayPemId;
}

bool AddFairPlayPemResponse::FairPlayPemIdHasBeenSet() const
{
    return m_fairPlayPemIdHasBeenSet;
}

uint64_t AddFairPlayPemResponse::GetPriority() const
{
    return m_priority;
}

bool AddFairPlayPemResponse::PriorityHasBeenSet() const
{
