...
CoreInternalOutcome ListAIWorkbenchSkillsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Skills") && !rsp["Skills"].IsNull())
    {
        if (!rsp["Skills"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Skills` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Skills"];
...
string ListAIWorkbenchSkillsResponse::ToJsonString() const
{
    ...
    if (m_skillsHasBeenSet)
    {
        ...
        int i=0;
        for (auto itr = m_skills.begin(); itr != m_skills.end(); ++itr, ++i)
        {
...
vector<SkillInfo> ListAIWorkbenchSkillsResponse::GetSkills() const
{
    return m_skills;
}

bool ListAIWorkbenchSkillsResponse::SkillsHasBeenSet() const
{
    return m_skillsHasBeenSet;
}

PageByNumResult ListAIWorkbenchSkillsResponse::GetPageResult() const
{
    return m_pageResult;
}

bool ListAIWorkbenchSkillsResponse::PageResultHasBeenSet() const
{
