...
CoreInternalOutcome MetricData::Deserialize(const Value &value)
{
    ...
    if (value.HasMember("MetricName") && !value["MetricName"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `MetricData.MetricName` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Points") && !value["Points"].IsNull())
    {
        if (!value["Points"].IsArray())
            return CoreInternalOutcome(Error("response `MetricData.Points` is not array type"));

        const Value &tmpValue = value["Points"];
