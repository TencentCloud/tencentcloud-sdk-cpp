...
CoreInternalOutcome DescribePolicyConditionListConfigManualPeriod::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Default") && !value["Default"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `DescribePolicyConditionListConfigManualPeriod.Default` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Keys") && !value["Keys"].IsNull())
    {
        if (!value["Keys"].IsArray())
            return CoreInternalOutcome(Error("response `DescribePolicyConditionListConfigManualPeriod.Keys` is not array type"));
    ...
    if (value.HasMember("Need") && !value["Need"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `DescribePolicyConditionListConfigManualPeriod.Need` IsBool=false incorrectly").SetRequestId(requestId));
        }
...
void DescribePolicyConditionListConfigManualPeriod::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
