...
CoreInternalOutcome DescribeAccidentEventListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Alarms") && !rsp["Alarms"].IsNull())
    {
        if (!rsp["Alarms"].IsArray())
            return CoreInternalOutcome(Error("response `Alarms` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Alarms"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            DescribeAccidentEventListAlarms item;
...
string DescribeAccidentEventListResponse::ToJsonString() const
{
...
vector<DescribeAccidentEventListAlarms> DescribeAccidentEventListResponse::GetAlarms() const
{
    return m_alarms;
}

bool DescribeAccidentEventListResponse::AlarmsHasBeenSet() const
{
    return m_alarmsHasBeenSet;
}

int64_t DescribeAccidentEventListResponse::GetTotal() const
{
    return m_total;
}

bool DescribeAccidentEventListResponse::TotalHasBeenSet() const
{
