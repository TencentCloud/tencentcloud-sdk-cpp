...
CoreInternalOutcome CreateInstancePreResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ReturnCode") && !rsp["ReturnCode"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ReturnCode` IsString=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("ReturnMessage") && !rsp["ReturnMessage"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ReturnMessage` IsString=false incorrectly").SetRequestId(requestId));
        }
