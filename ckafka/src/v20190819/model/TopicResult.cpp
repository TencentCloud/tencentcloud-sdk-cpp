...
CoreInternalOutcome TopicResult::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TopicList") && !value["TopicList"].IsNull())
    {
        if (!value["TopicList"].IsArray())
            return CoreInternalOutcome(Error("response `TopicResult.TopicList` is not array type"));
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TopicResult.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
...
void TopicResult::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
