...
CoreInternalOutcome DescribeConnectResource::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("Tags") && !value["Tags"].IsNull())
    {
        if (!value["Tags"].IsArray())
            return CoreInternalOutcome(Core::Error("response `DescribeConnectResource.Tags` is not array type"));
...
void DescribeConnectResource::SetTags(const vector<Tag>& _tags)
{
    m_tags = _tags;
    m_tagsHasBeenSet = true;
}

bool DescribeConnectResource::TagsHasBeenSet() const
{
