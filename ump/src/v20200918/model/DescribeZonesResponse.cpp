...
CoreInternalOutcome DescribeZonesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Zones") && !rsp["Zones"].IsNull())
    {
        if (!rsp["Zones"].IsArray())
            return CoreInternalOutcome(Error("response `Zones` is not array type"));
...
string DescribeZonesResponse::ToJsonString() const
{
...
vector<ZoneConfig> DescribeZonesResponse::GetZones() const
{
    return m_zones;
}

bool DescribeZonesResponse::ZonesHasBeenSet() const
{
