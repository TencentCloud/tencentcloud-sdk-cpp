...
CoreInternalOutcome DescribeAITaskResultResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("AudioResult") && !rsp["AudioResult"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `AudioResult` is not object type").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("ImageResult") && !rsp["ImageResult"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `ImageResult` is not object type").SetRequestId(requestId));
        }
    ...
    if (rsp.HasMember("VideoResult") && !rsp["VideoResult"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `VideoResult` is not object type").SetRequestId(requestId));
        }
...
string DescribeAITaskResultResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_audioResultHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "AudioResult";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_audioResult.ToJsonObject(value[key.c_str()], allocator);
    }
    ...
    if (m_imageResultHasBeenSet)
    {
        ...
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_imageResult.ToJsonObject(value[key.c_str()], allocator);
    }

    if (m_videoResultHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "VideoResult";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kObjectType).Move(), allocator);
        m_videoResult.ToJsonObject(value[key.c_str()], allocator);
    }
