...
CoreInternalOutcome DescribeVocabLibResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("VocabLibNameSet") && !rsp["VocabLibNameSet"].IsNull())
    {
        if (!rsp["VocabLibNameSet"].IsArray())
            return CoreInternalOutcome(Core::Error("response `VocabLibNameSet` is not array type"));
...
string DescribeVocabLibResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_vocabLibNameSetHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "VocabLibNameSet";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_vocabLibNameSet.begin(); itr != m_vocabLibNameSet.end(); ++itr)
        {
