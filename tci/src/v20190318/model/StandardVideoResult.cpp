...
CoreInternalOutcome StandardVideoResult::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("HighlightsInfo") && !value["HighlightsInfo"].IsNull())
    {
        ...
        const Value &tmpValue = value["HighlightsInfo"];
...
void StandardVideoResult::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
