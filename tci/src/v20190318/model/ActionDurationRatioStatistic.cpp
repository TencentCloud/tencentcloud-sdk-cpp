...
CoreInternalOutcome ActionDurationRatioStatistic::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("Ratio") && !value["Ratio"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `ActionDurationRatioStatistic.Ratio` IsDouble=false incorrectly").SetRequestId(requestId));
        }
...
void ActionDurationRatioStatistic::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
