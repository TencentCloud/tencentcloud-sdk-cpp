...
CoreInternalOutcome DescribeAllUserGroupResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Groups") && !rsp["Groups"].IsNull())
    {
        if (!rsp["Groups"].IsArray())
            return CoreInternalOutcome(Error("response `Groups` is not array type"));
...
string DescribeAllUserGroupResponse::ToJsonString() const
{
...
int64_t DescribeAllUserGroupResponse::GetTotalCount() const
{
    return m_totalCount;
}

bool DescribeAllUserGroupResponse::TotalCountHasBeenSet() const
{
    return m_totalCountHasBeenSet;
}

vector<GroupItem> DescribeAllUserGroupResponse::GetGroups() const
{
    return m_groups;
}

bool DescribeAllUserGroupResponse::GroupsHasBeenSet() const
{
