...
CoreInternalOutcome DescribeTopSpaceTableTimeSeriesResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("TopSpaceTableTimeSeries") && !rsp["TopSpaceTableTimeSeries"].IsNull())
    {
        if (!rsp["TopSpaceTableTimeSeries"].IsArray())
            return CoreInternalOutcome(Core::Error("response `TopSpaceTableTimeSeries` is not array type"));

        const rapidjson::Value &tmpValue = rsp["TopSpaceTableTimeSeries"];
...
string DescribeTopSpaceTableTimeSeriesResponse::ToJsonString() const
{
    rapidjson::Document value;
    value.SetObject();
    rapidjson::Document::AllocatorType& allocator = value.GetAllocator();

    if (m_topSpaceTableTimeSeriesHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "TopSpaceTableTimeSeries";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        int i=0;
        for (auto itr = m_topSpaceTableTimeSeries.begin(); itr != m_topSpaceTableTimeSeries.end(); ++itr, ++i)
        {
