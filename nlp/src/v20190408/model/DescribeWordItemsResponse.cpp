...
CoreInternalOutcome DescribeWordItemsResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("WordItems") && !rsp["WordItems"].IsNull())
    {
        if (!rsp["WordItems"].IsArray())
            return CoreInternalOutcome(Core::Error("response `WordItems` is not array type"));

        const rapidjson::Value &tmpValue = rsp["WordItems"];
...
string DescribeWordItemsResponse::ToJsonString() const
{
