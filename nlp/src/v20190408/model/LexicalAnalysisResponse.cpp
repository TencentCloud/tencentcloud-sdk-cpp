...
CoreInternalOutcome LexicalAnalysisResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("NerTokens") && !rsp["NerTokens"].IsNull())
    {
        if (!rsp["NerTokens"].IsArray())
            return CoreInternalOutcome(Error("response `NerTokens` is not array type"));

        const Value &tmpValue = rsp["NerTokens"];
    ...
    if (rsp.HasMember("PosTokens") && !rsp["PosTokens"].IsNull())
    {
        if (!rsp["PosTokens"].IsArray())
            return CoreInternalOutcome(Error("response `PosTokens` is not array type"));

        const Value &tmpValue = rsp["PosTokens"];
        for (Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            PosToken item;
            CoreInternalOutcome outcome = item.Deserialize(*itr);
