...
CoreInternalOutcome SentenceEmbeddingResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("Vector") && !rsp["Vector"].IsNull())
    {
        if (!rsp["Vector"].IsArray())
            return CoreInternalOutcome(Core::Error("response `Vector` is not array type"));

        const rapidjson::Value &tmpValue = rsp["Vector"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_vector.push_back((*itr).GetDouble());
        }
    ...
    if (rsp.HasMember("Dimension") && !rsp["Dimension"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Core::Error("response `Dimension` IsUint64=false incorrectly").SetRequestId(requestId));
        }
...
bool SentenceEmbeddingResponse::VectorHasBeenSet() const
{
    return m_vectorHasBeenSet;
}

uint64_t SentenceEmbeddingResponse::GetDimension() const
{
    return m_dimension;
}

bool SentenceEmbeddingResponse::DimensionHasBeenSet() const
{
