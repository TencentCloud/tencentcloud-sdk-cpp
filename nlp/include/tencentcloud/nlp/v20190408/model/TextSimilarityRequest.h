            ...
            namespace Model
            {
                ...
                */
                ...
                public:
                    ...
                    /**
                     * 获取需要与源句子计算相似度的一个或多个目标句子（仅支持UTF-8格式，目标句子的数量不超过100个，每个句子不超过500字）
...
注意：每成功计算1个目标句子与源句子的相似度算1次调用
                    ...
                    /**
                     * 设置需要与源句子计算相似度的一个或多个目标句子（仅支持UTF-8格式，目标句子的数量不超过100个，每个句子不超过500字）
注意：每成功计算1个目标句子与源句子的相似度算1次调用
