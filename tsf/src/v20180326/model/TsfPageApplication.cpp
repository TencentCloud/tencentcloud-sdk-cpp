...
CoreInternalOutcome TsfPageApplication::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TsfPageApplication.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Content") && !value["Content"].IsNull())
    {
        if (!value["Content"].IsArray())
            return CoreInternalOutcome(Error("response `TsfPageApplication.Content` is not array type"));
...
void TsfPageApplication::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
