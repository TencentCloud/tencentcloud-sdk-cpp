...
CoreInternalOutcome ApiUseStatisticsEntity::Deserialize(const Value &value)
{
    string requestId = "";
...
void ApiUseStatisticsEntity::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
    ...
    if (m_countHasBeenSet)
    {
        ...
        string key = "Count";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_count.c_str(), allocator).Move(), allocator);
    }
    ...
    if (m_ratioHasBeenSet)
    {
        ...
        string key = "Ratio";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, Value(m_ratio.c_str(), allocator).Move(), allocator);
    }
