...
CoreInternalOutcome TsfPageConfig::Deserialize(const Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("TotalCount") && !value["TotalCount"].IsNull())
    {
        ...
        {
            return CoreInternalOutcome(Error("response `TsfPageConfig.TotalCount` IsInt64=false incorrectly").SetRequestId(requestId));
        }
    ...
    if (value.HasMember("Content") && !value["Content"].IsNull())
    {
        if (!value["Content"].IsArray())
            return CoreInternalOutcome(Error("response `TsfPageConfig.Content` is not array type"));
...
void TsfPageConfig::ToJsonObject(Value &value, Document::AllocatorType& allocator) const
{
