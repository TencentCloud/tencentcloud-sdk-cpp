...
CoreInternalOutcome DescribeSkillReferenceListResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("ReferenceList") && !rsp["ReferenceList"].IsNull())
    {
        if (!rsp["ReferenceList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ReferenceList` is not array type"));

        const rapidjson::Value &tmpValue = rsp["ReferenceList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            SkillReferenceGroup item;
...
string DescribeSkillReferenceListResponse::ToJsonString() const
{
...
vector<SkillReferenceGroup> DescribeSkillReferenceListResponse::GetReferenceList() const
{
