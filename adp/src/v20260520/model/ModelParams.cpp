...
CoreInternalOutcome ModelParams::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("StopSequenceList") && !value["StopSequenceList"].IsNull())
    {
        if (!value["StopSequenceList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `ModelParams.StopSequenceList` is not array type"));

        const rapidjson::Value &tmpValue = value["StopSequenceList"];
        for (rapidjson::Value::ConstValueIterator itr = tmpValue.Begin(); itr != tmpValue.End(); ++itr)
        {
            m_stopSequenceList.push_back((*itr).GetString());
        }
        m_stopSequenceListHasBeenSet = true;
    }
...
void ModelParams::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
    ...
    if (m_stopSequenceListHasBeenSet)
    {
        rapidjson::Value iKey(rapidjson::kStringType);
        string key = "StopSequenceList";
        iKey.SetString(key.c_str(), allocator);
        value.AddMember(iKey, rapidjson::Value(rapidjson::kArrayType).Move(), allocator);

        for (auto itr = m_stopSequenceList.begin(); itr != m_stopSequenceList.end(); ++itr)
        {
...
vector<string> ModelParams::GetStopSequenceList() const
{
    return m_stopSequenceList;
}

void ModelParams::SetStopSequenceList(const vector<string>& _stopSequenceList)
{
    m_stopSequenceList = _stopSequenceList;
    m_stopSequenceListHasBeenSet = true;
}

bool ModelParams::StopSequenceListHasBeenSet() const
{
    return m_stopSequenceListHasBeenSet;
}
