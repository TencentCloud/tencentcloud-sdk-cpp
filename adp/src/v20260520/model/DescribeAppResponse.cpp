...
CoreInternalOutcome DescribeAppResponse::Deserialize(const string &payload)
{
    ...
    if (rsp.HasMember("App") && !rsp["App"].IsNull())
    {
        if (!rsp["App"].IsObject())
        {
            return CoreInternalOutcome(Core::Error("response `App` is not object type").SetRequestId(requestId));
        }

        CoreInternalOutcome outcome = m_app.Deserialize(rsp["App"]);
...
App DescribeAppResponse::GetApp() const
{
    return m_app;
}

bool DescribeAppResponse::AppHasBeenSet() const
{
