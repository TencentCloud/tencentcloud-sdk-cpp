...
CoreInternalOutcome InputBoxConfig::Deserialize(const rapidjson::Value &value)
{
    ...
    if (value.HasMember("InputBoxButtons") && !value["InputBoxButtons"].IsNull())
    {
        ...
        {
            m_inputBoxButtons.push_back((*itr).GetInt64());
        }
...
vector<int64_t> InputBoxConfig::GetInputBoxButtons() const
{
    return m_inputBoxButtons;
}

void InputBoxConfig::SetInputBoxButtons(const vector<int64_t>& _inputBoxButtons)
{
