...
CoreInternalOutcome PlatformUsageSummary::Deserialize(const rapidjson::Value &value)
{
    string requestId = "";
    ...
    if (value.HasMember("ResourceConsumptionList") && !value["ResourceConsumptionList"].IsNull())
    {
        if (!value["ResourceConsumptionList"].IsArray())
            return CoreInternalOutcome(Core::Error("response `PlatformUsageSummary.ResourceConsumptionList` is not array type"));
...
void PlatformUsageSummary::ToJsonObject(rapidjson::Value &value, rapidjson::Document::AllocatorType& allocator) const
{
...
vector<ResourceConsumption> PlatformUsageSummary::GetResourceConsumptionList() const
{
    return m_resourceConsumptionList;
}

void PlatformUsageSummary::SetResourceConsumptionList(const vector<ResourceConsumption>& _resourceConsumptionList)
{
    m_resourceConsumptionList = _resourceConsumptionList;
    m_resourceConsumptionListHasBeenSet = true;
}

bool PlatformUsageSummary::ResourceConsumptionListHasBeenSet() const
{
